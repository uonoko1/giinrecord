...
# 「今は引用しているから安全」ではなく、**形そのものを拒む**（実測: `~` を許したままだと、
# 当時の見本 15 形のうち `/~/pbi505/` だけが素通りした）。
#
...
# **片方の釘だけを外しても、$ROOT の外には 1 件も出ない。**
# もう一方が捕まえ、**exit 1 で止める**（それぞれ実測で確認した）:
#   - `..` の判定だけ消す        → assert_under_root が
#     「プローブ置き先が docroot の外に出る」で捕まえ、**何も作られない**
#   - assert_under_root を殺す   → allowlist が「.. は traversal になる」で捕まえる
#   - assert_safe_uri の `exit 1` だけ消す → assert_under_root の `exit 1` が止める
# ただし**「落ちるか」と「安全か」は別**なので、probe-safety は
# **拒否メッセージを名指しで**要求する。そのため上の 3 つは probe-safety では**落ちる**
# （「別の釘が捕まえた」ことまで見える。#485「経路が2つあるものは別々に釘打つ」）。
#
# **両方を同時に外すと、元の穴がそのまま戻る**: `location /../../rev502-escaped/` で
# **17 passed, 0 failed（exit 0）／/tmp/rev502-escaped が残る**（実測）。
# probe-safety はこれを **6 見本・16 アサーション**で落とす（うち 1 件は
# 「作業ディレクトリの外に 1 件できた」= 実際の脱出の検出）。
