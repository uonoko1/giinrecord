 ...
 * よって「委員だった期間」を推定せず、会議 1 回ごとの事実として記録する。
 *
 * **期間は作らない（ETL も Web も）。** 出席した最初の日と最後の日を範囲として見せると、
 * その間ずっとその役職だったという、一次資料に無い主張になる（辞任→再任で間が空く場合を表現できない）。
 * Web は「出席 N 回」「最初の出席 …」「最新の出席 …」と、**出席の事実**として出す。
 * 詳細は `docs/DATA_CONTRACT.md`「委員会の役職」。
