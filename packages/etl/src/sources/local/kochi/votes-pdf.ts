...
/**
 * 番号の欄（NFKC 後）。「第1号」「議発第12号」「報第1号」「376報第1号」（第376回からの継続審査）など。
 *
 * **枝番（`請第1-1号`）も通す**（Issue #901）。**請願の番号には枝番が付き、12月定例会に集中している。**
 * **枝番を捨てて `請第1号` に丸めない**——**1 会期に 4 行ある枝が 2 種に潰れ、
 * 代わりに `rollcalls.ts` が「出た順」の `-1` / `-2` を足すので、県の枝番と見分けが付かなくなる**（#569）。
 *
 * **区切りは PDF の原文では `－` U+FF0D だが、`numberText` は NFKC 済みなので `-` U+002D になる**
 * （だからここは `-` だけを見る）。
 *
 * **母数**（2026-09-20、index の賛否 PDF 65 本すべての番号の欄 2,040 個・360 種を全数収集）:
 * `第N号` 1,465 / `議発第N号` 265 / `N報第N号` 204 / `報第N号` 41 / `N第N号` 20 /
 * **`請第N-N号` 36** / `請第N号` 5。**末尾に余り（`m[2] !== ""`）が出る形は 0 個。**
 */
const NUMBER = /^(.*?第[0-9]+(?:-[0-9]+)?号)(.*)$/;
