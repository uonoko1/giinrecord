...
/**
 * **PDF の氏名が、名簿の氏名から字が落ちた形になっているか**（Issue #778。#749 の機序 ②）。
 *
 * **字形の揺れ・空白・異体字セレクタは `localNameKey` が畳んだ後で見る**ので、ここが true なら
 * **「畳んでもなお、名簿にある字が PDF の側で足りない」**という事実だけが残る。
 *
 * ## **なぜ「キーが違う」ではなく「部分列で、かつ短い」なのか**
 *
 * **#750（青森）・#759（秋田）・#768（佐賀）は「キーが違う」で書いていた。**
 * **3 県の中ではそれで同じ結果になる**——**この 3 県は `matchBySubsequence` で寄せるので、
 * 「寄った」かつ「キーが違う」なら、それは必ず部分列だから**（`matchBySubsequence` は
 * 完全一致か部分列でしか寄せない）。**`matchByExact`（島根）も、寄ればキーは必ず同じ。**
 *
 * **違いが出るのは鳥取（`matchBySurnamePrefix`）だけ**——
 * **鳥取の PDF は `○○議員`（姓だけ）なので、名簿と一致しないのが正常である。**
 * **「キーが違う」で 11 県に広げると、本番 `data/assemblies/pref-31/` の 35 人**
 * **全員が鳴る**（`入江議員` → `入江 誠` など。2026-09-13 実測）——
 * **守りが増えるのではなく、全部が鳴って誰も見なくなる**（#771 が閾値 1.7 で測ったのと同じ壊れ方）。
 *
 * **`議員` の 2 字は名簿の氏名に無いので部分列にならず、この判定なら鳴らない。**
 * **鳥取で字が落ちてもこの判定は鳴らない**——**姓しか公表されていないので、
 * そもそも名の側の欠落は誰にも見えない。そのことを「鳴らない」で表す**（鳴らせない、が事実）。
 *
 * ## **ここは「直せる」とは言わない**
 *
 * **返すのは「落ちている」という事実だけ**で、**落ちた字が何だったかは推定しない**（#569／#674）。
 * **票は本人に付いたまま残す**——**寄せ方は変えない**（#617／#648 の部分列一致は、
 * 実データの欠落で別人に決まらないと測ってある）。
 *
 * ## **長さの検査は、今のデータでは効いていない**（#778 の変異テストで分かったこと。隠さずに書く）
 *
 * **`[...a].length < [...b].length` を外しても、テストは 1 件も落ちなかった**（等価変異）。
 * **総当たりで確かめた**: `data/members/index.json` の 1,225 名から作れる **1,499,386 組**に対し、
 * **この検査を外しても答えが変わる組は 0。**
 *
 * **理由**: `a !== b` を先に外してあるので、**残るのは「長さが違う部分列」だけ**であり、
 * 長さの検査は本来は冗長である。**唯一ずれうるのは `isSubsequence` が
 * `a[i]`（UTF-16 単位）で進むのに `b` を符号位置で回すところ**だが、
 * **そのずれは `isSubsequence` を `false` に倒す方向にしか働かない**ので、
 * **長さの検査が「余計に鳴らせる」ことは無い。**
 * **そのうえ `localNameKey` を通した後に BMP 外の字が残る氏名は本番に 0 名**（1,225 名で実測。
 * 本番で唯一の BMP 外の氏名の字 `𠮷` は `ITAIJI` が `吉` に畳む）。
 *
 * **それでも残すのは、この関数の意味が「短くなった」だから**——
 * **`isSubsequence` の実装が将来直った（符号位置で進むようになった）ときに、
 * ここが黙って「同じ長さでも鳴る」側に変わらないようにする。**
 * **「今は効いていない」ことを書いた上で残す**（効いていないものを効いているように書かない）。
 */
export function isLossyName(nameText: string, rosterName: string): boolean {
  const a = localNameKey(nameText);
  const b = localNameKey(rosterName);
  if (a === "" || a === b) return false;
  return [...a].length < [...b].length && isSubsequence(a, b);
}
