 ...
 *   - **読めない 3 本**（`Kg265_250424`・`Kg274_250628`・`Kg337_sanpi-261127`）:
 *     **#718 はこの 3 本を「文字層なしの画像 PDF・読めないのが正しい結果」と書いていたが、
 *     実測（#922、2026-09-19）では 3 本のうち 2 本に文字層がある。**
 *       - `Kg265_250424`: showText 774 回。**CMap 未指定で 711 回が「グリフ 0」だった**（CMap を渡すと空 0）
 *       - `Kg274_250628`: showText 1477 回。**CMap 未指定で 1339 回が「グリフ 0」だった**（CMap を渡すと空 0）
 *       - `Kg337_sanpi-261127`: **showText 0 回。この 1 本だけが本当に文字層を持たない**
 *         （中身は `constructPath` 820 個の図形。CMap を渡しても 1 文字も変わらない）
 *     **文字が読めるようになっても、この 2 本はまだ表として組めない**——
 *     **`rotate=90` の縦置きページ**で（読める本は `rotate=0` の横置き）、罫線の縦横が入れ替わって見える
 *     （`Kg265` は vlines 92 / hlines 54。読める `Kg220` は 59 / 88）。**回転したページの表の組み立ては未対応。**
 *     **「読めないのが正しい」とは書かない**——1 本崩れた断定なので、測った結果だけを書く。
 *
