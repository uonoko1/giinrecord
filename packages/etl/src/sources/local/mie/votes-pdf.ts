 ...
 *
 * ### **`cluster()` の鎖は伸び続けない**（**当初「危ない」と書いたが、測ったら違った**）
 *
 * **`cluster()` は単純な単連結ではない**（`pdf-table.ts`）——
 * **まとめるたびに代表値が `(prev + v) / 2` に動く**ので、**鎖は途中で割れる。**
 *
 * **実測（等間隔に `tol` ずつ離した n 個を入れる）**:
 *
 * | n | クラスタ | 全体の幅 |
 * |---:|---:|---:|
 * | 3 | **2** | `2 tol` |
 ...
 *
 * **だいたい 2 個ごとに割れる。**
 * **三重のフィクスチャ 31 本 / 43 ページ / 3,752 クラスタで、
 * 幅が `tol` を超えたクラスタは 0 件**（いちばん広いもので `span/tol = 0.377`）。
 * **行間 8.4pt を `tol` 刻みで橋渡しするには 11 万個の中間アイテムが要る**が、
 * **実ページは最大 2,256 個で桁が 2 つ足りない。**
 *
 ...
 *
 * ### **`tol` は「群の最大 h」なので、実効の余裕はこれより小さい**（**測った**）
 *
 * **`tol` を一群でひとつにした**（対ごとにすると比較が非対称になる）**結果、
 * **その群にいちばん大きい文字が 1 つあれば、小さい文字にとっての `tol` も一緒に膨らむ。**
 *
 * **実測（151 本 / 並べ替えの群 22,792 / 差 > 0 の隣接対 91,709）**:
 *
 * | | 実測 |
 * |---|---|
 * | 1 つの群の中の `h` の比（max / min）のいちばん大きいもの | **×3.00**（`000252426.pdf`、`h` 2.88〜8.64） |
 * | **実効の余裕**（本物の差 ÷ **その群の** `tol`）のいちばん小さいもの | **×11.95** |
 * |  | （`000073620.pdf`。対の `h` は 8.76 だが群の max は 13.20） |
 *
 * **つまり上の表の「×18.00」は、対ごとの `h` で測った値である。**
 * **実装が実際に使う `tol`（群の最大 `h`）で測ると ×11.95 まで落ちる。**
 * **三重では ×11.95 でも十分だが、他県へ持ち出すときは
 * 上の表の数字をそのまま余裕とみなしてはいけない**——
 * **佐賀は対ごとで既に ×1.04 なので、群の最大 `h` で膨らめばその時点で潰れる。**
 *
...
function joinedLines(items: Item[]): { y: number; text: string }[] {
  ...
  // **151 本で 2,681 対**（内訳: `joinedLines` 2,633 / `cellText` 45 / `leftHeader` 3 / `joinVertical` 0）。
  //
  // **`cellText` の 45 対は 1 本（`000073636.pdf`）に全部入っている**——
  // **この本は直す前は読めておらず（見出しの照合で止まる）、その 45 対は出力に届いていなかった。**
  // **直すとこの本が読めるようになり、45 対は「正しい向きで」出る**
  // （実測: `105号議案第` → `議案第105号`、`決可` → `可決`）。
  // **つまり `cellText` は「誤差が届かない」のではなく「届く本が読めていなかった」だけである。**
