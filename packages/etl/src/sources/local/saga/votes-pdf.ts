 ...
 * **±0.35pt のような絶対値で切ると令和7年2月版で 1 列足りなくなる**——
 * **その本は議員の列の 1 本目だけ 15.00pt で、残りが 14.76pt なので、
 * 差 0.24pt は通るが、末尾の 14.58pt との差 0.42pt で切れてしまう**（実測。
 * **そのとき記号 37 個に対して列が 36 本になり、6 ページ 62 行が丸ごと読めなくなる**）。
 *
 ...
 *   下の余裕 0.0598             上の余裕 0.0070   ← 上は 8.7% しかない
 * ```
 *
 ...
 *
 * **この 2 本では、ずれた並びのセルがすべて `不明` に落ちた**ので、
 * **この値では「別人の票」までは出ていない**（測った事実。安全だという意味ではない）。
 * **`不明` に落ちるかどうかは罫線と記号の x の偶然**であり、
 * **設計として 1 つずれを止めているものは何も無い**——#569 の重いほうの形そのもの。
 *
