...
/**
 * **「上から下、同じ高さなら左から右」で並べるときの比較**（Issue #999）。
 *
 * ## 何が問題だったか
 *
 * **`(b.y - a.y || a.x - b.x)` は、`b.y - a.y` が 0 でなければ x を見ない。**
 * **浮動小数の丸めで y が 1e-13 pt だけ違うと、同じ行なのに x を見ずに左右が決まる。**
 *
 * **揺れの出どころは推測ではない**——**`mie/glyphs.ts` の `Td`/`TD`/`T*` は
 * `ly += dy * sy` で行頭を積算する**ので、**同じ高さに戻ってきても、
 * どの経路を通ったかで最下位ビットが違う。**
 *
 * **実測（2026-09-24、三重の index の賛否 PDF 151 本すべて。母数＝並べ替えに渡った隣接対 766,154）:**
 *
 * | `d = a.y - b.y` の大きさ（文字の高さ `h` に対する比） | 対 | 性質 |
 * |---|---:|---|
 * | `d = 0` | 675,255 | 同じ y。x で並ぶ（問題なし） |
 * | `1e-16 h` 〜 `1e-13 h` | **4,739** | **double の積算の丸め誤差**（同じ行） |
 * | **`1e-12 h` 〜 `1e-9 h`** | **0** | ← **4 桁ぶん空いている** |
 * | `1e-8 h` 〜 `1e-6 h` | **457** | **PDF が座標を単精度で持つ本の丸め誤差**（同じ行。`000073636.pdf` 1 本だけ） |
 * | **`1e-5 h` の帯** | **0** | ← **ここが空いているので境目に置ける** |
 * | `1e-4 h` 以上 | 85,703 | **本物の差**（わざとずらした要素・別の行） |
 *
 * **誤差の側と本物の側のあいだに、`1e-5 h` の帯がまるごと空いている。**
 * **だから許容差は `h * 1e-5` に置く**——
 * **実測で「誤差のいちばん大きいもの」（`1e-6 h`）の 10 倍、
 * 「本物のいちばん小さいもの」（`1e-4 h`）の 1/10 である。**
 *
 * **`h` に比例させる理由**: **誤差は座標の大きさに比例して増える**（丸めは相対量）。
 * **絶対値で決めると、拡大率の違う本（実測: `Tm` の倍率は 1 倍から 5 倍まである）で
 * 効いたり効かなかったりする。**
 *
 * ## 何に効くか（**票には効かない**。#569 の重さで確かめた）
 *
 * **実測（151 本）: 誤差でひっくり返る対 2,629 対のうち、
 * **2,627 対が `joinedLines`（表題と凡例を探すためだけの行）、2 対が左 8 列の見出しの照合**で、
 * **`cellText`（件名・番号・人数・結果）と `joinVertical`（議員の氏名）では 0 対。**
 * **表決のセルはそもそもどの並べ替えも通らない**——**`bandIndex(voteCols, cx)` の x だけで置く。**
 *
 * **「今のデータでは票に届かない」であって「原理的に届かない」ではない**のは
 * `cellText` / `joinVertical` のほうだけで、**表決のセルは構造上この比較を通らない。**
 *
 * ## 既存の出力は変わらない（**実測**）
 *
 * **151 本を、許容差 `0`（＝今の実装）と `1e-9 / 1e-6 / 1e-4 / 1e-3 / 1e-2` の絶対値で通して、
 * 表題・会期名・凡例・議員（氏名と会派）・全行（番号・件名・日付・人数・結果・全セル）を突き合わせた:
 * **読めた 111 本のうち値が変わった本は 0 本**（どの許容差でも）。
 * **読めなくなった本も 0 本。**
 */
export const byRowThenColumn = (a: Item, b: Item): number => {
  // **許容差は「行の高さの 1e-5」**（上の実測表。誤差の帯と本物の帯のあいだの空白に置いてある）
  const tol = Math.max(a.h, b.h, 1) * 1e-5;
  return (Math.abs(a.y - b.y) <= tol ? 0 : b.y - a.y) || a.x - b.x;
};
