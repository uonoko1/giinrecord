 ...
 * **当たらなければ番号は空**（推定しない）。
 */
const NUMBER_CELL = /(?:議案|認定|承認|報告|請願|陳情|発議|意見書案|決議案|同意|諮問|人事案件|[一-鿿]{0,8})第[０-９0-9]{1,4}号/;
 ...
 * ## **許容 1.9 倍には下限があり、上限は無い**（**154 本で測った。2026-09-13**）
 * **秋田の列は右端の小会派で 1.33 倍に広がる**ので、**1.33 より大きくなければならない。**
 * **一方、帯の外は 7 倍以上離れているので、上は広くても構わない。**
 *
 * | 許容 | 列の数と合う行（154 本 3,985 行） | 1.9 と芯が違う行 |
 * |---|---:|---:|
 * | **1.2** | **2,013** | **1,971** |
 * | 1.5 | 3,946 | 38 |
 * | **1.9（これ）** | **3,983** | — |
 ...
 * | 3.5 | 3,983 | **0** |
 *
 * **1.9 → 2.5 / 3.5 の変異は等価である**（**当てて確かめた。57 件すべて緑**）。
 * **守っているのは「1.33 より大きく、帯の外までは届かない」という幅であって、
 * 1.9 という数そのものではない。**
 * **1.5 でも 38 行だけ落ちる**ので、**1.33 のすぐ上では足りない**——
 * **1.9 を採ったのは、実測の広がり 1.33 から余裕を取った値である。**
...
function readLeftCells(items: readonly Item[]): { number: string; title: string; dateText: string; method: string; result: string; counts?: { yes: number; no: number; voting: number } } {
  ...
  // **番号と件名が 1 アイテムに同居する本もある**（実測 `041222hyoketsu.pdf`:
  // `議案第 178 号 秋田県情報公開条例の一部を改正する条例案` が 1 アイテム。**この本の 60 行すべて**）。
  // **だから「番号を含むアイテムまで」を繋いだあと、そのアイテムの中でも番号の後ろを件名に回す。**
  // **「番号の後ろを回さない」と件名が空になる**（実測: `041222` `060220` `R41102` の全行で空になった）。
  let numberEnd = -1;
  let acc = "";
  ...
  const numMatch = numberEnd >= 0 ? acc.match(NUMBER_CELL) : null;
  // **通し番号は番号ではない**（`34議案第78号` の `34`）ので、`第N号` の部分だけを採る。
  // ## **`知事提出` は縦書きで、行の境をまたぐ**（実測 2026-09-13。**直していない**）
  // **`h231202giketu.pdf` では `知事提出` が x=45 に縦書きで 4 アイテムに割れ**
  // （`知` y=616.8 / `事` y=607.6 / `提` y=598.4 / `出` y=589.1）**、
  // 行の錨（議決月日 y=607.6）から見て `知` が 1 つ上の行に近い。**
  // **その結果 `number` が `事提出認定第２号` になる**（`知` が落ちる。**1 行**）。
  // **`認定第２号` は正しく入っている**ので、**議案の同一性は保たれている**（id も件名も正しい）。
  // **直さない**——**「縦書きのラベルを行に割り振る」規則を足すと、
  // 行の錨の付け方そのものを変えることになり、154 本すべてに影響する。**
  // **`知事提出` は種別の前に付く飾りで、議案を一意にするのに要らない。**
  // **`number` の原文がそうなっていることを、ここに書いて残す。**
  //
  // **`知事提出議案第45号` のように種別の前に出す語がある**ので、
  // **`NUMBER_CELL` は `第N号` の前の漢字も 6 文字まで拾う**（`知事提出議案` は 6 文字、`知事提出認定` も 6 文字だが、
  // **全角数字の本では `第２号` の前が `知事提出認定` で 6 文字なのに 8 文字まで要る**——
  // **`NUMBER_CELL` は最短一致ではないので、8 にしておくと `知` が落ちない**）
  const number = numMatch ? numMatch[0] : "";
  // **番号のアイテムの中で、番号より後ろの部分**（同居している本。上のコメント）
  const afterNumberInItem = numMatch ? acc.slice(numMatch.index! + numMatch[0].length) : "";
  // **件名は「番号のアイテムの中の残り」＋「それより後ろのアイテム」**
  // （**y の降順・行の中は左から**。折り返しがここで正しく繋がる）
  const titleItems = numberEnd >= 0 ? leftItems.slice(numberEnd + 1) : leftItems;
  const fallbackTitle = (numberEnd >= 0 ? afterNumberInItem : "") + joinText(titleItems) + inDateBefore;
  // **錨より右 = 表決方法 → 議決結果 → 数**。
 ...
 * **k 番目は定義上 k 番目の列に落ちる。**
 * ## **「x が列の境界の中にあるか」の確認は、154 本では 1 度も効かない**（**測った。2026-09-13**）
 * **154 本 3,983 行 169,267 記号で、`k` 番目が `k` 番目の列に入らなかった行は 0**（実測）。
 * **この枝を消す変異を当てても、6 本のフィクスチャは 1 セルも動かない**
 * （**落ちるのは `readVoteCells` の単体テストだけ**）。
 * **つまりこれは「今のところ効いていない守り」である。**
 *
 * **それでも残す**——**列の作り方が変わったとき**（罫線から作るように戻す、
 * 中心を平均でなく中央値から作る、など）**に、ずれが積もったことをここで捕まえられる。**
 * **消すと、ずれても気づけない**（記号の個数は合ったままなので、`unknownCells` も `unmatched` も動かない）。
 * **「効いていない」という事実を、測った数字ごとここに残す。**
 */
 ...
 *
 * ## **このモデルは、フィクスチャ 6 本では「効いていない」**（**測った。2026-09-13**）
 *
 * **`isHalf` を常に `false` にする（＝全部を全角幅と見て等分する）変異を当てて測った**（結合アイテムの
 * 記号 **2,259 個**、セル幅で正規化した中心のずれ）:
 *
 * | 半角の送り幅 | 中心のずれ（最大） | **列が変わった記号** |
 * |---|---:|---:|
 * | **0.5（これ）** | — | — |
 * | **1.0（等分）** | **0.2454 セル** | **0** |
 * | 0.371（#753 が 1 本で実測した値） | 0.0928 セル | **0** |
 *
 * **どれも半セルに届かないので、6 本では 1 記号も列が変わらない。**
 * **`splitRowItem` の単体テストは落ちるが、6 本のフィクスチャは 1 セルも動かない。**
 * **これは「検算が緩い」のではなく、この実装では等価である**——
 * **列の中心を「その列に落ちた記号の x の平均」から作っている**（`findMemberColumns`）ので、
 * **記号は定義上その中心の近くにあり、両側に半セルぶんの余裕がある。**
 *
 * **それでも半角モデルを残す理由:**
 *   - **典型的な誤差が小さい**（0.5 なら 0、等分なら最大 0.245 セル）。
 *     **議員が増えて列が狭くなれば、先に破綻するのは等分のほうである。**
 *   - **#753 は「半角の送り幅は全角の半分ではない」と 1 本で実測している**（0.371em）。
 *     **0.371 と 0.5 のどちらが正しいかは分かっていない**——**フォントの送り幅を網羅的に測っていない。**
 *     **どちらでも落ちる列が同じなら、モデルの単純なほうを採る。**
