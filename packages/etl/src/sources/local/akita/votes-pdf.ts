 ...
 *
 * ## **前提: `marks` は cx の昇順に並んでいること**（Issue #1002 で明文化）
 *
 * **この関数は「隣り合う要素の差」を列の間隔として読む**ので、
 * **並んでいない `marks` を渡すと間隔の意味が壊れる。**
 * **今の呼び手は 2 つとも `voteRows()` の返り値**（`marks.sort((a, c) => a.cx - c.cx)` 済み）**なので満たしている。**
 * **満たさない入力を渡す呼び手を足すなら、渡す前に並べること。**
 *
 * **破れたときに何が起きるかを測った**（#1002。6 本 223 行 × 200 通りの混ぜ方 = **44,600 試行**）:
 *
 * | 段階 | 試行 |
 * |---|---:|
 * | 混ぜた試行（母数） | **44,600** |
 * | **`step <= 0` で「並べ替えずにそのまま返る」枝に入った** | **20,444** |
 * | **芯の長さが議員の数と一致した**（長さの検査を抜けた） | **1,192** |
 * | **`readVoteCells` の `columnOf(...) === k` の検算まで通った** | **0** |
 * | **別人の票になった** | **0** |
 *
 * **「長さが合う」ところまでは 1,192 回到達する。止めているのは `readVoteCells` の
 * `columnOf` の検算だけである**——**あの検査は「正しい入力では 1 度も効かない」と
 * そちらの docblock に書いてあるが、前提が破れた入力では 1,192 回効いている。**
 * **消してはいけない。**
 *
 * **余裕**: 落ちた 1,192 回のうち **1,188 回（99.7%）が `k = 0`**、残り 4 回が `k = 1`。
 * **それより後ろまで通ったものは 0**（＝いちばん最初の記号で捕まえている）。
 */
...
/**
 * **左の欄のアイテムを「いちばん近い錨」に配るときの、距離の上限**（行の高さの何倍まで許すか。#829）。
 *
 * ## **上限が無いと、ページ下端のページ番号が票の行に入る**（**本番の欠陥。#819 が見つけた**）
 *
 * `readRows` は左の欄のアイテムを**いちばん近い錨（議決月日の y）に配る**が、**上限が無かった。**
 * **ページ下端に 1 つだけ立つページ番号（`1` `2` `3`）は、左の欄の x の範囲に入り、
 * 氏名帯より下にあるので、必ずどれかの行に配られる。**
 * `readLeftCells` は「錨より右の数字を左から 3 つ」で `counts` を作るので、そこに 1 桁が混ざると:
 * - その行の数字が **2 つ**（**反対 0 の行は反対者数の欄が空**）→ **3 つになり `counts` が捏造される**
 * - その行の数字が **3 つ**（ふつう）→ **4 つになり `counts` が丸ごと落ちる**
 *
 * ## **上限は実測から決めた**（**フィクスチャ 6 本・213 行・左の欄のアイテム 1,667 個**。2026-09-13）
 *
 * 配られたアイテムの `|Δy| / 行の高さ` を全部数えた:
 *
 * | `|Δy|/行の高さ` | 個数 |
 * |---|---:|
 * | 0.0〜0.5 | 1,638 |
 * | 0.5〜1.0 | 18 |
 * | 1.0〜1.5 | **0** |
 * | 1.5〜2.0 | **1**（`h231202giketu` p1 の縦書き `知事提出` の `出`。**ratio 1.884**） |
 * | 2.0〜2.5 | **0** |
 * | 2.5〜 | **10**（**全部ページ番号の `1` `2` `3`**。ratio 3.000 / 3.886 / 4.400 / 4.986 / 30.523 / 43.536 / 50.986 / 58.743 / 64.636 / 68.768） |
 *
 * **本物の最大 1.884 と、ページ番号の最小 3.000 のあいだが空いている。**
 * **その真ん中の 2.5 を採る**——**1.884 側に寄せると縦書きの `知事提出` を落とし、
 * 3.000 側に寄せると `h291222giketu` p1 の `1`（ちょうど 3.000）を取りこぼす。**
 *
 * **`>` と `>=` のどちらでも結果は同じ**——**1.884 と 2.5 と 3.000 のあいだに、実測でアイテムが 1 つも無い。**
 * **「ちょうど境界」の行が無いので、この 2 つを取り違えても出力は変わらない**（境界の向きを頼りにしていない）。
 *
 * **錨の選び方の上限（`* 3`）とは別の値である**——**あちらは「票の行から錨までの距離」、
 * こちらは「錨からアイテムまでの距離」。同じ 3 にすると、ちょうど 3.000 のページ番号が通る。**
 *
 * **効果（フィクスチャ 6 本 213 行を全件突き合わせた。2026-09-13）**:
 * **10 行が直り、他の 203 行は 1 文字も変わらない**（`number` `title` `dateText` `method` `result` `counts` `cells` すべて）。
 * **捏造 5・喪失 5** の内訳は `akita-votes-pdf.test.ts` の「#829」のテストに実物で置いてある。
 */
const ROW_ITEM_MAX_ROWS = 2.5;
...
function readRows(page: PageGeometry, mc: MemberColumns, pageNo: number): VotePdfRow[] {
  ...
  for (const it of page.items) {
    ...
    if (r < 0) continue;
    // **いちばん近い錨でも遠すぎるものは、どの行にも入れない**（#829。下の `ROW_ITEM_MAX_ROWS`）
    if (Math.abs(it.cy - nearest) > Math.max(rows[r].h, 1) * ROW_ITEM_MAX_ROWS) continue;
    rowItems[r].push(it);
  }
 ...
 *
 * ## **反対者数の欄が空の行がある**（**`--sessions 29` に広げて初めて出た。#901**）
 *
 ...
 *
 * **`--sessions 5`（旧・本番）の範囲には 2 つの行が 1 行も無く、
 * `--sessions 29` で 231 / 785 行（29.4%）が出た**——**その 231 行は `counts` が空になる。**
 *
 * ## **「空欄は 0」と読んではいけない**（**#901 で測って確かめた。誘惑が実在した**）
 *
 * **`--sessions 29` の範囲（index 0..28）だけを見ると、
 * 数が 2 つの 550 / 550 行で `表決者数 == 賛成者数` が成り立つ**ので、
 * **「空欄 = 反対 0」と読みたくなる**（**`mc.left − 行の右端` を字の高さで割った比も
 * きれいに分かれる**——**数 3 つの行は 0.2〜0.8、数 2 つの行は 2.3〜3.6**）。
 *
 * **だが 154 本すべてで測ると、その読み方は 180 行で嘘になる**（実測。#901）:
 *
 * | 本 | 行 | PDF の数 | **記号の実数** |
 * |---|---|---|---|
 * | `h231202giketu.pdf` | 認定第２号 | `44` `42`（**反対者数の欄は空**） | **○ 42 / × 2** |
 * | `h260708giketsu.pdf` | 議案第178号 | `43` `40`（同上） | **○ 40 / × 3** |
 * | `h261002giketu.pdf` | 請願第38号 | `43` `11`（同上） | **○ 11 / × 32** |
 *
 * **反対者が居るのに欄が空の行がある。** **「空欄 = 0」と読むと、
 * `counts.no` に 0 を書いた 180 行が出て、そのうち大半は嘘になる**（#569 の重いほう）。
 * **`h261002giketu.pdf` の 請願第38号 は、反対 32 人を 0 人と公表することになる。**
 *
 * **見出し（`反 対 者 数` の縦書き）の位置は 154 本とも取れるが、
 * 「空欄が 0 なのか、書き忘れなのか」は PDF のどこにも書かれていない。**
 * **区別する手がかりが原文に無いので、`counts` を作らないのが正しい。**
 *
 * **つまり 231 行に `counts` が無いのは、実装の不具合ではなく一次資料がそう書いていないからである。**
 * **その 231 行は `local-count-mismatches.test.ts` の突き合わせ（#826）の外に出る**——
 * **突き合わせる相手が公表されていないので、「合っている」とも「食い違っている」とも言えない**（#757）。
 *
 * **数がそれ以外の個数なら counts 無し**（0 と書かない。**原文に無い数を作らない**）。
 *
 ...
 *
 * ## **訂正: 「1 度も効かない」のは正しい入力での話である**（Issue #1002 で測った）
 *
 * **`voteRowCore` は「`marks` が cx の昇順」を前提にしている。**
 * **その前提が破れた入力では、この枝は 1,192 回効いている**
 * （223 行 × 200 通りの混ぜ方 = 44,600 試行のうち、**長さの検査を抜けてここまで来たのが 1,192**。
 * **そのすべてをここで落としており、別人の票になったのは 0**）。
 *
 * **つまりこれは「今のところ効いていない守り」ではなく、
 * 「前提が破れたときに別人の票を止める唯一の守り」である。**
 *
 * **消すとどうなるかを測った**（**この行を消す変異を当て、同じ 44,600 試行を通した**）:
 *
 * | | 素のコード | **この行を消す** |
 * |---|---:|---:|
 * | 全部 `不明` | 44,600 | 43,408 |
 ...
 * | **誤ったセル** | **0** | **6,742** |
 *
 * **消すと 1,181 行が別人の票になる。** **消してはいけない。**
 * **`voteRowCore` の docblock に前提と表を書いた。**
 */
