 ...
 *
 * ## **高知 = 5 の根拠**（2026-09-20、index の賛否 PDF 65 本すべてを取得して実測）
 *
 * **高知で効いている上限は名簿でも「読めるか」でもなく、`glyphs.ts` の 1 つの安全弁である。**
 *
 * | `--sessions` | 足す会期 | 採決（累計） | セル（累計） | 止まる理由 |
 * |---:|---|---:|---:|---|
 * | 2 | 2026-06 / 2026-02 | **104** | 3,744 | **（本番の値）** |
 * | 3 | ＋2025-12 | **179** | 6,369 | **直す前はここで止まっていた**（請願の枝番 `請第1-1号`） |
 * | 4 | ＋2025-09 | **197** | 7,017 | — |
 * | **5** | **＋2025-06** | **221** | **7,881** | — |
 * | 6 | ＋2025-02 | — | — | **`rotated/scaled text matrix [1,0,0,1.0000200271606445] not supported`** |
 * | 7〜14 | 2024-12 〜 2023-05臨時 | （読める） | | **2025-02 の 1 本しか間に挟まっていない** |
 *
 * **`--sessions 6` の 2025-02 は、text matrix の `a` か `d` が `1.0000200271606445`**
 * （**実測: `[1.00002,0,0,1]` が 3,664 回、`[1,0,0,1.00002]` が 1,096 回。`/Rotate` は 0**）。
 * **`glyphs.ts` は `a !== 1 || d !== 1` で止める**——**`showText` が `a` / `d` を
 * 字送りに一切掛けていないので、通せば 0.002% ずれた位置で文字を読むことになる。**
 * **正しい足し方を実データで検証していないので、通さない側に倒す**（#700 / #707 と同じ判断）。
 * **これを直すのは別の PBI である**（#913 の 1 番と同じ性格の作業）。
 *
 * **一般選挙の境はもっと先にある**——**会期ごとに PDF に出る氏名の集合を 31 本ぶん数えると、
 * 2023-02 と 2023-05臨時 のあいだに IN 10 / OUT 9 の不連続が 1 回だけ出る**（**その次は 2019-05臨時 の
 * IN 8 / OUT 8**）。**それ以外の移り変わりは 0〜3 人で、任期中の辞職・補選の規模である。**
 * **`--sessions 14`（2023-05臨時 まで）が任期の内側で、5 はそのずっと手前である。**
 *
 * **`rosterAsOf` 2026-07-30 から最古の採決 2025-06-27 まで 398 日**（**上限 1,461 日の 27%。#928 は鳴らない**）。
 * **寄らなかった氏名は 0 → 3 人**（武石利彦・田所裕介・橋本敏男。**いずれも候補 0 人＝名簿に同姓同名すらいない**）。
 *
 * ## **秋田 = 29 の根拠**（2026-09-20、index の賛否 PDF 154 本すべてを取得して実測）
 *
 * ### **秋田だけ数の単位が違う**——**29 は「29 本会議日」であって「29 定例会」ではない**
 * **賛否 PDF が 1 本会議日に 1 本**なので（`akita/index.ts`）、**`--sessions N` は N 本の PDF を指す。**
 * **1 つの定例会に PDF が 7 本ある年もある**ので、**他県の 4 と秋田の 29 は同じ桁の話ではない。**
 *
 * ### **秋田は読める本が尽きない**——**154 / 154 本すべてが読める**（実測。失敗 0）
 * **三重（151 中 84）・徳島（28 中 7）と違い、`parseVotePdf` は 1 本も落とさない。**
 * **だから止める位置を決めるのは「読めるか」ではなく「名簿が当たるか」だけである。**
 *
 * ### **一般選挙の境は 29 本目と 30 本目のあいだ**（**選挙の日付を外から持ち込まず、氏名の集合だけで見た**）
 *
 * | 本 | 議決日 | 氏名 | 1 つ古い本と共通 | **IN** | **OUT** |
 * |---:|---|---:|---:|---:|---:|
 * | 27 | 2023-07-28 | 41 | 41 | 0 | 0 |
 * | 28 | 2023-07-06 | 41 | 41 | 0 | 0 |
 * | **29** | **2023-05-16** | **41** | **34** | **7** | **9** |
 * | 30 | 2023-03-10 | **43** | 43 | 0 | 0 |
 * | 31〜36 | 2023-02-20〜2022-09-20 | 43 | 43 | 0 | 0 |
 *
 * **29 本目と 30 本目のあいだに IN 7 / OUT 9 の不連続があり、議員数も 41 と 43 で変わる。**
 * **154 本のうち、5 人以上の入れ替わりが出る境はここ 1 か所だけである**（他の 153 回の移り変わりは
 * **0〜2 人**で、任期中の辞職・補選の規模。**空白の詰め方が違うだけの 6 人**——
 * `佐 々 木雄 太` と `佐々木 雄太` のような——**は、空白を除けば同じ氏名なので数えていない**）。
 *
 * ### **検算**（154 本すべての氏名の集合を数えた）
 * **`--sessions 29` の範囲に出る氏名は 43 通り。そのうち 41 が今の名簿と完全一致し、
 * 名簿の 41 人は 1 人残らずこの範囲に現れる**（「名簿にいるのに票が無い」人が 0 人）。
 * **寄らない 2 人は `沼谷 純`（2025-04-30 まで）と `鈴木 健太`（2024-11-01 まで）で、
 * どちらも任期の途中で退いた議員である**——**票は `memberId` 空のまま残す**（#529）。
 *
 * ### **`#928` の検査は、この境を捕まえない**（**余地があることは安全ではない**）
 * **`rosterAsOf` 2026-07-24 から 2023-05-16 まで 1,165 日、選挙の前の 2023-03-10 でも 1,232 日で、
 * どちらも上限 1,461 日の内側である。** **`--sessions 33`（2022-12-22、1,310 日）でも鳴らない。**
 * **止める位置を決めているのは #928 ではなく、氏名の集合に出た不連続のほうである。**
 *
 * ### **外の一次資料と合う**（**歴代正副議長一覧**）
 * **`議` の列は 785 採決中 784 で 1 人**（1 件だけ 0 人＝議長も投票した行。#615 が実測した形）。
 * **その 1 人は 2 人しかいない**——**北林 丈正（2023-05-16〜2025-04-30）と 工藤 嘉範（2025-05-14〜）。**
 * **県議会の「歴代正副議長一覧」と 1 日も食い違わない**:
 * **69代 北林丈正 令5.5.15〜令7.5.14 / 70代 工藤嘉範 令7.5.14〜**
 * （https://pref.akita.gsl-service.net/doc/2019042400028/ の `file_contents/250514.pdf`）。
 *
 * ## **青森 = 14 の根拠**（2026-09-21、index の議決結果 PDF 56 本すべてを取得して実測）
 *
 * ### **青森も読める本が尽きない**——**56 / 56 本すべてが読める**（実測。失敗 0）
 * **`parseVotePdf` も `toLocalRollCalls` も 1 本も落とさず、56 本の `unknownCells` の合計は 0。**
 * **だから止める位置を決めるのは「読めるか」ではなく「名簿が当たるか」だけである**（秋田と同じ形）。
 *
 * ### **一般選挙の境は 14 会期目と 15 会期目のあいだ**（**選挙の日付を外から持ち込まない**）
 *
 ...
 *
 * **5 人以上の入れ替わりが出る境は 56 会期のうち 3 か所**（**14↔15 / 32↔33 / 49↔50。
 * 順に 2023年4月・2019年4月・2015年4月の一般選挙**）。**他の 52 回は 0〜3 人で辞職・補選の規模。**
 *
 * **⚠ 見かけ上の入れ替わりのうち 6 回は `櫛引ユキ子` 1 人の字形違いだった**
 * （`櫛󠄁引`+IVS ／ `噰引`（U+5670 に化ける）／ `櫛引` ／ `引`（`櫛` が文字層に無い）。**#749 の機序 ①②③**）。
 * **`localNameKey` は IVS しか落とさないので、この 3 形は手で寄せて数え直した**——
 * **寄せても寄せなくても 14↔15 の境は動かない**（実測）。
 *
 * ### **`#928` の検査は、この境を捕まえない**——**鳴るのは 4 会期も後ろである**
 *
 ...
 *
 * ### **検算**（56 本すべての氏名の集合を数えた）
 * **`--sessions 14` の範囲に出る氏名は 51 通り。そのうち 45 が今の名簿と完全一致し、
 * 部分列でしか寄らなかった組は 0 組。** **寄らない 6 人**（`和田寛司` `噰引ユキ子` `阿部広悦`
 * `谷川政人` `工藤貴弘` `関良`）**は 6 人とも候補 0 人**——**名簿に同姓同名すら居ないので、
 * 別人に寄りようがない**（#569 の安全側）。**票は `memberId` 空のまま残す**（#529）。
 * **名簿 46 人のうちこの範囲に出ないのは `和田 寬司` 1 人だけで、これは `寛`/`寬` の別字**
 * （**畳んではいけない**。#636／#529）。
 *
 * ### **`--sessions 15` を採らない理由を、増えかたからも言う**
 * **寄らなかった氏名は 14 で 6 人（2,306 票 / 29,015 ＝ 7.95%）だが、15 で 17 人（3,476 票 / 32,447
 * ＝ 10.71%）に跳ねる**——**増えた 11 人は 2023-02 会期で退いた議員である。**
 * **19 では 20 人・6,614 票（16.28%）。** **「寄らない」は安全側だが、跳ねかたが境を裏づける。**
 * ## **鳥取 = 11 の根拠**（2026-09-21、index の 73 会期・賛否 PDF 123 本すべてを取得して実測）
 *
 * ### **母数**（#757）
 * **会期 index の `<a>` 88 本 → 会期 73 本**（**落とした 15 本は `href` の無いアンカー 14 本と「次のページ」1 本**）。
 * **73 会期のうち 1 会期は「まだ議決結果のリンクが無い」**（令和8年9月定例会。**会期中**）。
 * **残り 72 会期の議決結果ページから賛否 PDF が 123 本。**
 *
 * ### **直す前は 24 / 123 本しか読めなかった。直した後は 43 / 123 本**
 * **止めていたのは 4 つの欠陥で、どれも `--sessions 2` の 5 本には 1 件も無い形である**
 * （実測は `tottori-votes-pdf.test.ts`）:
 *
 * | 欠陥 | 最初に当たる会期 | 直し方 |
 * |---|---|---|
 * | **議案等番号のセルが一次資料で空**（決算認定に係る指摘事項） | **3 会期目**（令和7年12月） | **空のまま出す**（件名から拾って埋めない） |
 * | **見出しが `令和`/`7`/`年`/`2`/`月定例会` の 5 アイテムに割れている** | **6 会期目**（令和7年2月） | **同じ y の文字をつないでから探す** |
 * | **同じ凡例を同じページに 2〜3 回刷っている**（値は 1 文字も違わない） | **8 会期目**（令和6年9月） | **同じ値の重なりは通す。値が違えば今までどおり例外** |
 * | **件名が 5.5pt 字下げされていて件名が空になる** | **10 会期目**（令和6年2月） | **x だけで切らず、引用の本文（10pt 以上）が出たら打ち切る** |
 *
 * ### **どこで止まるか**
 *
 * | `--sessions` | 会期 | 採決（累計） | セル（累計） | 止まる理由 |
 * |---:|---|---:|---:|---|
 * | 2 | 令和8年6月 / 8年2月 | **118** | 4,130 | **（直す前の本番の値）** |
 * | 5 | ＋7年12月 / 7年9月 / 7年6月 | **225** | 7,875 | — |
 * | 8 | ＋7年2月 / 6年11月 / 6年9月 | **396** | 13,689 | — |
 * | **11** | **＋6年6月 / 6年2月 / 5年11月** | **572** | **19,623** | — |
 * | 12 | ＋令和5年9月 | — | — | **`page 1: group heading rule … not found`**（会派の見出しの罫線が無い） |
 *
 * **12 会期目（令和5年9月）の 3 本は 1 本も読めない。** **13 会期目（令和5年8月臨時会）は読めるが、
 * 12 会期目で例外になるのでそこまで行かない**（`index.ts` は会期を 1 つずつ読み、例外で全体が止まる）。
 * **これを直すのは別の PBI である**（**会派の見出しを罫線以外から取る必要がある**）。
 *
 * ### **11 は任期の内側である**（**選挙の日付を外から持ち込まず、氏名の集合だけで見た**）
 *
 * **読める 13 会期に PDF に出る氏名を会期ごとに数えると、不連続が 1 回も出ない:**
 *
 * **⚠ この表の「入れ替わり」は `seatsChanged` ではない**（#986）——
 * **ここは PDF に出る氏名の集合を直に引き算した IN / OUT である。**
 * **`seatsChanged` の実装は `min(rosterAbsent, unmatchedNames)`** なので、
 * **同じ境でも小さい値になりうる**（**佐賀は入れ替わり 9 の境を 4 としか言わない**）。
 * **下の会期ごとの `seatsChanged`（0〜2）と、この IN / OUT（0〜2）は別々に測った 2 つの信号で、
 * どちらも跳ねていない**——**片方だけを見て「境が無い」と言っていない。**
 *
 * | 会期 | 氏名 | 1 つ古い会期との入れ替わり（IN / OUT。`seatsChanged` ではない） |
 * |---|---:|---|
 * | 令和8年6月 〜 令和7年6月 | 35 | **0**（4 回とも） |
 * | **令和7年6月 ← 令和7年2月** | 35 ← 34 | **IN 1**（森議員） |
 * | 令和7年2月 〜 令和6年9月 | 34 | **0**（2 回とも） |
 * | **令和6年9月 ← 令和6年6月** | 34 ← 35 | **OUT 1**（内田隆議員） |
 * | **令和6年6月 ← 令和6年2月** | 35 ← 33 | **IN 2**（山本議員・玉木議員） |
 * | **令和6年2月 ← 令和5年11月** | 33 ← 34 | **OUT 1**（藤縄議員） |
 * | **令和5年11月 ← 令和5年8月臨時** | 34 ← 35 | **OUT 1**（平井議員） |
 * | 令和5年8月臨時 ← 令和5年6月 | 35 | **0** |
 *
 * **12 回の移り変わりはすべて 0〜2 人で、任期中の辞職・補選の規模である。**
 * **三重の 9 人・徳島の IN13/OUT11・秋田の IN7/OUT9 のような境は、読める範囲に 1 つも無い。**
 * **理由は単純で、いちばん古い読める会期（令和5年6月、2023-06-14 議決）が
 * 2023年4月の一般選挙の後だからである**——**読めるほうが先に尽きる。**
 *
 * **外の一次資料とも合う**: **県議会「歴代正副議長一覧」の 88代 浜崎晋一 は 令和5年5月10日 就任**
 * （https://www.pref.tottori.lg.jp/76208.htm ）。**読める会期はすべてその後にある。**
 *
 * ### **検算**
 * **読める 13 会期に出る氏名は 38 通り。そのうち 35 が今の名簿（35 人）に 1 対 1 で寄り、
 * 名簿の 35 人は 1 人残らずこの範囲に現れる**（「名簿にいるのに票が無い」人が 0 人）。
 * **寄らない 3 人は 内田隆議員・藤縄議員・平井議員 で、いずれも任期の途中で退いた議員である**
 * ——**票は `memberId` 空のまま残す**（#529）。
 *
 * **`内田隆議員` は特に見ておく価値がある**——**名簿に 内田 博長 が居るからである。**
 * **鳥取の PDF は同姓を名の 1 文字で書き分けており**（`内田隆議員` / `内田博議員`）、
 * **前方一致は `内田博長`.startsWith(`内田隆`) が偽なので寄らない。**
 * **ここが寄ってしまえば 176 採決が別人の記録として出る**（#569 の重いほう）。
 * **検査は `tottori-rollcalls.test.ts` に置いた。**
 *
 * ### **`#928` はここでは効いていない**
 * **`rosterAsOf` 2023-04-30 で、最新の採決 2026-06-29 まで 1,156 日**（上限 1,461 日の内側）。
 * **広げて増えるのは古い側なので、`daysAfter` は 1 日も動かない**（最新の採決は変わらない）。
 * **最古の採決 2023-12-04 は `rosterAsOf` の後なので `daysBefore` は負である。**
 * **止める位置を決めているのは #928 ではなく、12 会期目で読めなくなることのほうである。**
 *
 * ### **増えた採決に x と y の錨が当たっている**
 * **`local-x-anchor-rotation.test.ts` の鳥取の母数が 133 → 370 行に増え、
 * 4 回転（1 / 2 / −1 / −2）とも 370 / 370 行が落ちる**（**無改造では 0 行**）。
 * **増えた 4 本は 88代 浜崎晋一 と 89代 福田俊史（令和7年6月9日 就任）の両方をまたぐ。**
 *
 * ## **残る 1 議会（滋賀）を 2 のままにした理由**
 *
...
const LOCAL_SESSIONS_DEFAULT: Readonly<Record<string, number>> = {
  ...
  tokushima: 4,
  // **高知 pref-39**: 令和8年6月 / 8年2月 / 7年12月 / 7年9月 / 7年6月。**6 会期目（令和7年2月）は
  // text matrix が `1.0000200271606445` 倍で `glyphs.ts` が止める**（`showText` が `a`/`d` を
  // 字送りに掛けていないので、通すと 0.002% ずれた位置で読む。通さない側に倒す）。
  // **一般選挙の境（2023-02 ↔ 2023-05臨時、IN 10 / OUT 9）はずっと先で、名簿は効いていない**
  kochi: 5,
  // **秋田 pref-05**: **この数の単位だけ他の 10 議会と違う**——**「定例会」ではなく「本会議日」である**
  // （`akita/index.ts` の docblock。**賛否 PDF が 1 本会議日に 1 本**なので、`--sessions N` は
  // 「新しいほうから N 本の PDF」になる）。**29 = 2026-07-03 から 2023-05-16 までの 29 本会議日。**
  // **30 本目（2023-03-10）は 2023年4月の一般選挙の前**なので採らない（上の docblock）。
  akita: 29,
  // **青森 pref-02**: 令和8年6月第326回 〜 令和5年5月第96回臨時会（**1 会期 1 本なので 14 会期 = 14 本**）。
  // **15 会期目（令和5年2月第313回）は 2023年4月の一般選挙の前**で、氏名の集合が IN 11 / OUT 15 で
  // 不連続になる（上の docblock）。**56 / 56 本が読めるので「読めるか」は効いていない。**
  // **#928 の検査は 19 会期目まで鳴らない**——**境より 4 会期も後ろなので、当てにしていない。**
  aomori: 14,
  // **奈良 pref-29**: 令和8年6月 / 8年2月 / 7年12月 / 7年9月。**125 → 180 採決、5,000 → 7,200 セル。**
  // **5 会期目（令和7年6月）は「セルの `-` U+002D が凡例の `―` U+2015 に無い」で例外。**
  // **これを共有の字形表（`glyph-variants.ts`）で寄せてはいけない**——
  // **滋賀の凡例は同じ `-` U+002D を「欠席」の鍵として使っている**（`shiga-votes-pdf.test.ts` の
  // `Kg220_240424-sanpi.pdf`: 凡例 `{"-":"欠席","議":…,"退":…}`）。**奈良に合わせて `―` に寄せると
  // 滋賀の凡例から `-` が引けなくなる。** **同じ符号位置が議会ごとに違う意味を持つので、
  // これは「見た目が同じ別コードポイント」ではなく意味の衝突である**（実測は `nara-sessions-window.test.ts`）。
  // **6 会期目（令和7年2月）は議案等番号が空の行で別の例外になるので、直しても 1 会期しか伸びない。**
  // **一般選挙の境（2024-10-23 ↔ 2022-10-24、IN 17 / OUT 17）はさらに先で、名簿は効いていない**
  nara: 4,
  // **宮城 pref-04**: 第400 〜 第390回（令和8年6月 〜 令和5年11月定例会）。
  // **index の 77 本のうち読めるのは 13 本で、その 13 本は連続した先頭 13 会期にちょうど収まる**
  // （**第387回 以前の 64 本は 1 本も読めない**——凡例の見出しが `＜会派名＞` でない 61 本・
  //  見出しの数字が全角 42 本・文字層なし 3 本ほか。#871 を 2026-09-21 に独立に再現した）。
  // **止める位置を決めたのは「読めるか」ではなく「名簿が当たるか」である**——
  // **会期ごとに PDF に出る氏名の集合を 13 本ぶん数えると、第390回 ← 第389回 のあいだだけ
  //  IN 18 / OUT 19 の不連続が出る**（**2023-10 の一般選挙**。他の 11 回の移り変わりは 0〜2 人）。
  // **だから 12・13 本目（第389・388回）は入れない。** **`unmatched` も同じ境で 5 → 23 に跳ねる**（実測）。
  // **#928 はこの境を捕まえない**——**13 本目まで広げても `rosterAsOf` から 1,178 日で、上限 1,461 日の内側。**
  miyagi: 11,
  // **島根 pref-32**: 令和8年6月 / 8年2月 / 7年11月 / 7年9月 / 7年6月。**112 → 231 採決、3,920 → 8,085 セル。**
  // **島根は先行 8 県と止まる理由が違う**——**名簿ではなく一次資料の側が先に尽きる。**
  //   **会期 index は 108 あるが、議員別採決結果一覧 PDF を持つのは 14 会期（2023-06 〜 2026-06）だけ**（#874 が全数測定）。
  //   **15 会期目（2023-05 臨時会）には議員別 PDF が無く、会期ページに `h1` も無いので `parseSessionPage` が落ちる。**
  //   **一般選挙（2023-04）は 14 本すべてより手前**で、`rosterAsOf` は **2023-05-17**。
  //   **#928 の `rosterAsOf` 窓を「選挙の境の検出」に使ってはいけない**（#961）——
  //   **青森では境が 14↔15 会期なのに鳴ったのは 19 会期目、秋田では一度も鳴らなかった。**
  //   **島根も同じで、14 本の議決日は `rosterAsOf` から 50 〜 1,142 日、上限 1,461 の内側で 1 度も鳴らない**（実測）。
  //   **効く信号は `unmatchedNames` と、会期ごとの氏名集合の不連続である。**
  //   **氏名の集合も跳ねない**: **PDF に出る氏名は 35 〜 36 名で、差は `出川桃子` 1 人だけ**
  //   （2023-06 〜 2024-09 に居て、2025-06 以降に居ない。**辞職の規模であって一般選挙の境ではない**）。
  //   **その 1 人は名簿に無いので `matchByExact` が寄せず `unmatched.json` に落ちる**——**別人には付かない**（#569）。
  // **実際に止めているのは「読めるか」である**——**6 会期目（2025-05 臨時会）で `parseVotePdf` が落ちる。**
  //   **機序は #874 の分類 E**: **記号の行が 4 つあるのに議案番号が `第80号` の 1 つしか無い**（実測）ので、
  //   **`anchors` が 1 本しか立たず 4 行ぶんの記号が 1 行に集まる**（`two vote marks in one cell (col 0)`）。
  //   **常任委員の選任のように番号を持たない議案があるためで、閾値では直らない。この PBI では直していない。**
  // **5 会期ぶんの出力は `unknownCells` 0 / `unmatchedNames` 0**、
  // **公表された賛成者数・反対者数と `○`/`●` の個数が 231 行すべてで一致する**（母数つきの実測。#757）。
  shimane: 5,
  // **佐賀 pref-41**: 令和8年6月定 〜 令和5年5月臨（5月9日から11日まで）の 13 会期。
  // **23 → 366 採決、851 → 13,542 セル。**
  //
  // **母数**（2026-09-21、`runSaga` を実際に走らせて数えた。#757）:
  // **今の任期（令和5年4月30日〜令和9年4月29日。名簿ページの原文）に索引が持つ会期は 20 本。**
  // **そのうち読めたのが 13 本、読めなかったのが 7 本**——
  // **文字層が無い PDF 5 本**（令和7年11月定・7年6月定・6年11月定・6年9月定・6年6月定）と
  // **賛否 PDF のリンクがそもそも無い 2 本**（令和8年11月定・8年9月定。会期中）。
  // **「23 しか無い」のではなく「23 しか読めていなかった」**——
  // **20 本のうち 2 本しか見に行っていなかった**（下の欠陥）。
  //
  // ## **止める位置は 13**——**氏名の集合が 13↔14 で切れる**
  // **会期ごとに PDF に出る氏名の集合を数えると、令和5年5月臨 ← 令和5年2月定 のあいだだけ
  //  IN 4 / OUT 5 の不連続が出る**（**2023-04 の一般選挙**）:
  //   - **入る 4 人**（`中倉政義` `川﨑常博` `井上祐輔` `稲富正敏`）は**今の名簿に 1 人もいない**
  //   - **出る 5 人**（`指山清範` `青木一功` `石丸太郎` `酒井幸盛` `猪村利恵子`）は**全員いる**
  // **`unmatched` も同じ境で 2 → 6 に跳ねる**（実測）。**14 本目を入れると、
  //  引退した 4 人の票が名簿に寄らないまま出るか、同姓同名に当たれば別人に付く**（#569 の重いほう）。
  //
  // ## **#928 はこの境を捕まえない**——**先行 9 県でいちばん外れている**
  // **14 本目まで広げても `rosterAsOf`（2025-04-01）から 753 日で、上限 1,461 日の内側**
  // （**708 日も余っている**）。**青森（#959）は「境の 4 会期後ろで鳴る」だったが、
  //  佐賀は上限の半分そこそこで境を越える。** **日数は選挙がいつあったかを知らない**（#961）。
  //
  // ## **13 と 14 の違いは 2 会期ではなく 1 会期**（会期の並び）
  // **令和5年5月臨時会は 2 本ある**（5月9日から11日まで / 5月29日）。**どちらも選挙の後。**
  saga: 13,
  // **鳥取 pref-31**: 令和8年6月 〜 令和5年11月定例会（11 会期）。**118 → 572 採決、4,130 → 19,623 セル。**
  // **12 会期目（令和5年9月）は `group heading rule … not found` で 3 本とも読めない**
  // （会派の見出しを区切る罫線が無い。直すのは別の PBI）。
  // **名簿は効いていない**——**いちばん古い読める会期（令和5年6月）が 2023年4月の一般選挙の後で、
  // 読める 13 会期に一般選挙の規模の入れ替わり（IN/OUT が 5 人以上）が 1 回も出ない**（上の docblock）。
  // **寄らない 2 人（内田隆議員・藤縄議員）は任期の途中で退いた議員で、票は `memberId` 空のまま残る**（#529）。
  tottori: 11,
  // **滋賀 pref-25**: 令和8年7月 〜 令和5年5月招集会議（19 会期）。**14 → 163 採決、604 → 6,886 セル。**
  // **止める位置を決めたのは「読めるか」ではない**——**index の 146 本のうち 144 本が読める**
  // （読めないのは `Kg337_sanpi-261127`＝文字層なしと `Kg265_250424`＝`/Rotate 90` の 2 本だけで、
  //  どちらも 2013〜2014 年でこの窓のはるか外）。**20 会期目も例外では止まらない。**
  // **決めたのは名簿の境である**——**会期ごとに PDF に出る氏名の集合を 80 会期ぶん数えると、
  //  2023-02 ← 2023-05招集会議 のあいだに IN 12 / OUT 11 の不連続が 1 回だけ出る**
  // （**2023年4月の一般選挙**。他の 18 回の移り変わりは 0〜4 人で、任期中の辞職・補選の規模）。
  // **だから 20 会期目（2023-02）は入れない。**
  // **#928 はこの境を捕まえない**——**20 会期目まで広げても `rosterAsOf` から 1,315 日で、
  //  上限 1,461 日の内側（90%）。19 会期目でも 1,231 日（84%）。止めているのは氏名の集合のほう。**
  // **検算**: **この 19 会期に出る氏名は 50 通り。42 が今の名簿と完全一致し、
  //  名簿の 42 人は 1 人残らずこの範囲に現れる**（「名簿にいるのに票が無い」人が 0 人）。
  //  **寄らない 8 人はいずれもこの任期の途中で退いた議員で、票は `memberId` 空のまま残す**（#529。
  //  **8 人とも候補 0 人＝名簿に同姓同名すらいない**ので、別人に付く余地が無い）。
  // **広げる前に直した欠陥**: **会派帯の上端に表の外枠を使っていたので、
  //  その上の注記・凡例が会派名の頭に流れ込んでいた**（`votes-pdf.ts` の `readGroups`。
  //  **146 本中 25 本・825 列**。**`--sessions 2` の 2 本には 1 件も無い形**で、
  //  **9 会期目の `Kg835_0425sanpi2.pdf` で窓の中に入ってくる**）。
  shiga: 19,
};
