 ...
 *
 * ### **#982 で 12 本が読めるようになったが、三重の `4` は動かない**（2026-09-24 に測り直した）
 *
 * **PR #994 で index 151 本のうち読める本が 99 → 111 に増えた**（+12 本 / 245 行）。
 * **だが本番の採決は 733 件のまま 1 件も増えない。**
 * **`--sessions 4` で `runMie` を実際に走らせて数えた: 733 件。
 * 採決 ID を `data/` の 733 件と突き合わせて、差は 0 である**（片側だけの ID も 0）。
 *
 * **理由は「12 本が既定の射程の外にある」ことで、しかもその外し方が深い**——
 * **#901 が上の表で使ったのと同じ物差し（会期ごとの「PDF に出る氏名の集合」の入れ替わり）で
 * index の 27 会期まで測り直した**（`.measure/982/election-boundaries.ts`）:
 *
 * | 新しく読めた本 | 会期（index の何番目か） | **今の任期から見てまたぐ一般選挙** |
 * |---|---|---:|
 * | `001088734` `001088735` | r03（**7 番目**） | **1 回**（r05-1 で 9 人入れ替わり） |
 * | `001088736` `001088742` `001088744` `001088745` `001088746` `001088747` `001088748` | r02（**8 番目**） | **1 回** |
 * | `000072296` | h25（**17 番目**） | **3 回**（r05-1 / h31 / h27-1 がいずれも 9 人） |
 * | `000073629` | h24-1（**19 番目**） | **3 回** |
 * | `000073609` | h21-1（**27 番目**） | **4 回**（＋ h21-1-rinji で 13 人入れ替わり） |
 *
 * **いちばん近い本でも一般選挙を 1 回またぐ**ので、**`--sessions` をいくつにしても
 * 「今の任期に収まる」という既定の根拠を壊さずには取りに行けない。**
 * **`--sessions 8` にすれば 9 本が入るが、それは r05-1 の 9 人の入れ替わりをまたぐ**——
 * **引退した議員の票が、今の名簿の別人に付きうる**（#569 の重いほう。この docblock の冒頭の理由そのもの）。
 *
 * **つまり #982 の 12 本は「読めるが、出さない」が正しい。**
 * **`--sessions` を上げるのではなく、名簿に任期を持たせる**（`LocalMemberTerm` が
 * `asOf` しか持たない、という制約のほう）**が先である。それは別の PBI。**
 * **「読める」と「出す」は別**（#835）——**この表は、その区別が三重で実際に効いている記録である。**
 *
 ...
 * **実測 2026-09-24（本番 `data/` の 11 議会 / 119 会期 / 4,599 採決 / 198,221 票）:**
 *
 * | `seatsChanged` | 会期 |
 * |---:|---:|
 * | 0 | 48 |
 ...
 * | **6 以上** | **0** |
 *
 * **10 以上は 0 件**（**「数えていない」ではなく「11 議会 119 会期すべてを数えた上での 0」**。#757）。
 *
 * **`seatsChanged` の最大は 5 なので、10 は「任期の途中の辞職・補選では 1 度も出ていない幅の 2 倍」である。**
 * **⚠ この最大は #901 の進み方で動く**——**2026-09-21 の時点では 79 会期 / 最大 4 だったが、
 * #973 が滋賀を `--sessions 19` にして 119 会期 / 最大 5 になった。**
 * **「最大 4」と書いてある古い記述（Issue #986 の本文を含む）は、この日付より前のものである。**
 *
 ...
 *
 * **⚠ この 3 県は「いちばん小さい境」ではなかった**（#986 が 11 県すべてを測り直した）——
 * **秋田の境は 9、佐賀の境は 4 で、どちらもこの線の下にある**（下の表。**線の下は 秋田 9・佐賀 4**）。
 * **「線を 12 以上にすると青森（11）を取りこぼす」は正しいが、
 * 線が 10 のままでも既に 2 県を取りこぼしている。**
 * **上側の余裕（今の 11 県の最大 5 との差 5）より、下側は余裕ではなく取りこぼしである。**
 *
 ...
 *
 * ## ⚠ **今の「印 0 件」を「安全」と読まないこと**（#986 で測り直した）
 *
 * **11 県とも印が付いていないが、それは「境が無い」ではなく
 * 「担当者が境の手前で `--sessions` を止めている」という意味である。**
 * **この線が境を検出して止めた実績は 1 件も無い**（**止める道具ではないので当然である**）。
 *
 * ### **この線は、境の入れ替わりの総量の半分しか見ない**
 *
 * **`min(a, b) <= floor((a + b) / 2)` は恒等的に成り立ち、a = b のときですら等号である。**
 * **だから `seatsChanged` は `rosterAbsent + unmatchedNames` の半分を超えられない。**
 * **本番 11 議会 / 119 会期で、`seatsChanged` が総量と一致した会期は 0 件**
 * （**総量が 0 でない 77 会期の `seatsChanged / 総量` の最大は 0.50**。実測 2026-09-24）。
 *
 * **「名簿の定数が減っている県だけが縮む」ではない**（#986 が佐賀の担当者の見立てを追試した結果）。
 * **非対称（名簿 R ≠ 境の PDF の氏名の数 P）が決めるのは、`min` がどちらの側を採るかだけである。**
 *
 * ### **11 県の境を同じ式に通した結果**（#986。**母数 11 / 測れた境 8 / 測れない 3**）
 *
 * | 県 | `rosterAbsent` | `unmatchedNames` | 総量 | **`seatsChanged`** | この線 |
 * |---|---:|---:|---:|---:|---|
 * | 宮城 (04) | 19 | 23 | 42 | **19** | 鳴る |
 * | 滋賀 (25) | 17 | 18 | 35 | **17** | 鳴る |
 * | 奈良 (29) | 17 | 17 | 34 | **17** | 鳴る |
 * | 青森 (02) | 18 | 16 | 34 | **16** | 鳴る |
 * | 徳島 (36) | 13 | 13 | 26 | **13** | 鳴る |
 * | 高知 (39) | 13 | 12 | 25 | **12** | 鳴る |
 * | **秋田 (05)** | **9** | **11** | **20** | **9** | **鳴らない**（線の下 1） |
 * | **佐賀 (41)** | **5** | **4** | **9** | **4** | **鳴らない**（線の下 6） |
 *
 * **測れない 3 県**: **三重**（境は隣だが IN / OUT の内訳が記録されていない。総量「9 人」のみ）・
 * **鳥取**（12 会期目が読めず境まで届かない）・**島根**（境が一次資料に無い）。
 * **ただし三重は、総量 9 では上限が 4 なので、内訳を測らなくてもこの線には届かない**
 * （**線 10 に掛かるには総量 20 以上が要る**）。
 *
 * ### **それでも線を動かしていない**
 *
 * **秋田の境 9 と、今の 11 県の最大 5（滋賀）のあいだに 3 しか空きが無い。**
 * **佐賀（4）まで捕まえるには線を 4 にする必要があり、それは本番の最大と同じ値である**
 * ——**鳴りっぱなしになれば、鳴っていること自体を見なくなる**（#787 / #790）。
 * **`rosterAbsent` と `unmatchedNames` は `min` で潰す前の値が `meta.json` に既に在るので、
 * 取りこぼした境も、後から 2 つの数を直に見れば数えられる。**
 *
 * **実測は `test/local-seats-changed-boundaries.test.ts`。**
 */
