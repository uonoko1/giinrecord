 ...
 *
 * #### **ただし、今の本番には 1 件も出ていない**（**PO の検算を受けて追加で測った。2026-09-24**）
 *
 * | 出どころ | 件数 | 反転した形 |
 * |---|---:|---:|
 * | `data/members` の `group`（**名簿 HTML 由来**） | 7 種 | **0** |
 * | `data/assemblies/pref-24/rollcalls` の `votes[].group`（**PDF 由来**） | **34,590 件 / 8 種** | **0** |
 *
 * **`data/members` の `group` はそもそも PDF を通らない**——
 * `roster.ts` が名簿ページの `所属会派` の `<td>` を読み、
 * **選挙区ページと五十音順ページの 2 経路を突き合わせて、一致しなければ例外にする**（`roster.ts:175`）。
 *
 * **PDF 由来の `votes[].group` も、今は全部正しい。**
 * **理由は「読めているから」ではない**——
 * **反転した会派名を持つ本は 76 本あり、年は 2009〜2023 年に収まる**（実測）。
 * **一方、本番の `data/assemblies/pref-24/rollcalls` に入っているのは
 * `r05-2` / `r06` / `r07` / `r08` だけ**である。
 *
 * **`--sessions` の既定が三重は 4**（`local-cli.ts` の `defaultSessionsFor`）で、
 * **その 4 という数は、会派名とは全く別の #569 の理由で決まっている**——
 * > **一律の数にしないのは、名簿が「今の 1 枚」しか無く、一般選挙をまたぐと
 * > 引退した議員の票が今の別人に付きうるため**（`local-cli.ts` の docblock）
 *
 * **つまり反転した会派名は、正しさによってではなく、
 * 「別の理由で範囲を絞っていること」の副作用で本番から外れている。**
 *
 * **`--sessions 4` の根拠そのものは `test/local-sessions-default.test.ts` にある**（#901 / #996）——
 * **三重は一般選挙のたびに 9 人が入れ替わり、`--sessions 5` は r05-1 を足して一般選挙をまたぐ。**
 * **あちらは「なぜ 4 で止めるか」を測り、ここは「4 で止めることで何が外れているか」を測っている。**
 *
 * **だから 4-1 と 4-2 は「既定を広げるときの前提条件」である**——
 * **青森は #959 で既定を 2 → 14 に増やしたばかりだが、
 * 三重で同じことをするなら、その前に 4-1 と 4-2 を直す必要がある。**
 * **広げた瞬間に、反転した会派名と入れ替わった件名が本番に出る。**
 *
 ...
 *
 * #### **ただし、本番に出ている範囲には 1 対も無い**（**PO の検算を受けて追加で測った。2026-09-24**）
 *
 * | 母数 | 本数 / 行 | 揺れの対 |
 * |---|---:|---:|
 * | 読めている 111 本すべて | 111 本 / 2,805 行 | **1,238 対**（65 本） |
 * | **本番の範囲（2023年7月以降 ＝ `r05-2`〜`r08`）** | **24 本 / 714 行** | **0 対**（**0 本**） |
 *
 * **1,238 対はすべて 2023年6月以前の、本番に入っていない本に出ている。**
 * **会派名の反転（4-1）と同じ構図である**——
 * **`--sessions` の既定 4 が、別の #569 の理由で古い本を外しているおかげで当たっていない。**
 * **`--sessions` を増やせば、その瞬間に両方とも本番に出る。**
 *
