 ...
 *
 * **実測 2026-09-25**（`data/` を直に読んだ）:
 * **11 議会 / 119 会期 / 4,599 採決 / 198,221 票**（#901 で 11 議会すべてを広げ終えた）。
 *
 * | `seatsChanged` | 会期 |
 * |---:|---:|
 * | 0 | 48 |
 ...
 * | **10 以上は 0 件**（印が付く会期） | **0** |
 *
 * **合計 48+17+29+9+8+8 = 119**（**母数。下の `assert.deepEqual` と一致する**）。
 *
 * ## ⚠ **この表は 2026-09-25 まで 97 会期ぶんのまま取り残されていた**（#1007 のレビューが見つけた）
 *
 * **`0:32 1:12 2:28` で合計 97**——**2 行上の「119 会期」とも、下の `assert.deepEqual` の
 * `{0:48, 1:17, 2:29, …}` とも食い違っていた。** **assert は本番 `data/` を数え直すので
 * `#901` が会期を広げるたびに直ったが、docblock は誰も数え直さないので取り残された。**
 *
 * **しかも同じ表の正しい版が `packages/etl/src/local-assemblies.ts` の
 * `SEATS_CHANGED_FLAG` の docblock に在る**——**「同じ表が 2 か所にあって片方だけ直った」。**
 * **#1007 が直そうとしている病気そのものが、この欄を数えているファイルの中に在った。**
 *
 * **だからこのファイルは `seats-changed-line-numbers.test.ts` の `PLACES` に入れてある**
 * （**本番 `data/` の実測とずれたら落ちる**）。
 *
 * **`seatsChanged` の最大は 5**（#901 で滋賀を `--sessions 19` にして 4 → 5 に動いた。**印の線 10 の半分**）。
 * **滋賀の 20 会期目（2023年4月の一般選挙の向こう側）は `seatsChanged` 17 になる**——
 * **だから 19 で止めた**（#901。**印はこの境で鳴る**）。
 * **奈良が境をまたいだ場合は 17 になる**（#950 が測った 41 名中 17 名の入れ替わり）。
 */
test("#951 本番 data/: 11 議会 119 会期の seatsChanged 分布（最大 5、印が付く会期は 0 件）", async () => {
  ...
  assert.equal(rows.length, 119, "会期の合計");
  assert.equal(rows.reduce((s, r) => s + r.rollcalls, 0), 4_599, "採決の合計（#855 / #928 の母数と同じ）");
  assert.equal(rows.reduce((s, r) => s + r.votes, 0), 198_221, "票の合計（#928 の母数と同じ）");
  // **分布**（**「0 件でした」では、見ていなくても同じ顔をする**）
  const hist = new Map<number, number>();
  for (const r of rows) hist.set(r.seatsChanged, (hist.get(r.seatsChanged) ?? 0) + 1);
  assert.deepEqual(Object.fromEntries([...hist].sort((a, b) => a[0] - b[0])), { 0: 48, 1: 17, 2: 29, 3: 9, 4: 8, 5: 8 });
 ...
 */
test("#951 本番 data/: rosterSeen + rosterAbsent == counts.members（119 会期すべて）", async () => {
  const prefs = await localPrefs();
  let checked = 0;
  for (const p of prefs) {
...
test("#951 本番 data/: meta.sessionRosterCoverage が rollcalls/ + members/index.json から作り直した値と一致", async () => {
  ...
  assert.equal(sessions, 119, "作り直した会期の数（母数）");
});
 ...
 *
 * **`SEATS_CHANGED_FLAG` は「今までに実際に起きた幅（最大 5。滋賀。#973 で 4 → 5 に動いた）の外」に引いてある。**
...
test("#951 入れ替わりの幅ごとの印（0/1/4 は付かず、10/17/41 は付く）", () => {
  ...
  assert.deepEqual(got, {
    ...
    // **4 は #973 より前の 11 県の最大（三重）。今の最大は 5（滋賀）。ここで鳴ったら本番が赤くなる**
