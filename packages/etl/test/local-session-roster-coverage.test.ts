 ...
 * **11 議会 / 93 会期 / 3,996 採決 / 176,446 票**（#901 の島根で 79 → 82、**佐賀で 82 → 93 会期**）。
 *
 ...
 */
test("#951 本番 data/: 11 議会 93 会期の seatsChanged 分布（最大 4、印が付く会期は 0 件）", async () => {
  ...
  assert.equal(rows.length, 93, "会期の合計");
  assert.equal(rows.reduce((s, r) => s + r.rollcalls, 0), 3_996, "採決の合計（#855 / #928 の母数と同じ）");
  assert.equal(rows.reduce((s, r) => s + r.votes, 0), 176_446, "票の合計（#928 の母数と同じ）");
  // **分布**（**「0 件でした」では、見ていなくても同じ顔をする**）
  const hist = new Map<number, number>();
  for (const r of rows) hist.set(r.seatsChanged, (hist.get(r.seatsChanged) ?? 0) + 1);
  assert.deepEqual(Object.fromEntries([...hist].sort((a, b) => a[0] - b[0])), { 0: 43, 1: 15, 2: 27, 3: 6, 4: 2 });
 ...
 */
test("#951 本番 data/: rosterSeen + rosterAbsent == counts.members（93 会期すべて）", async () => {
  const prefs = await localPrefs();
  let checked = 0;
  for (const p of prefs) {
  ...
  assert.equal(checked, 93, "見た会期の数（母数）");
});
...
test("#951 本番 data/: meta.sessionRosterCoverage が rollcalls/ + members/index.json から作り直した値と一致", async () => {
  ...
  assert.equal(sessions, 93, "作り直した会期の数（母数）");
});
