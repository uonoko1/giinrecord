 ...
 *
 * **実測 2026-09-23**（`data/` を直に読んだ）:
 * **11 議会 / 102 会期 / 4,450 採決 / 191,939 票**（#901 の島根で 79 → 82、佐賀で 82 → 93、**鳥取で 93 → 102 会期**）。
 *
 * **島根・佐賀・鳥取は同じ日に別々の PR で入った。** **どの PR 単独の値も正しくない**——
 * **この数字は 3 つとも入った `data/` を読み直して測ったものである**（足し算で出していない）。
 *
 * | `seatsChanged` | 会期 | #901 の佐賀で |
 * |---:|---:|---|
 * | 0 | 43 | **+8** |
 * | 1 | 15 | **+3** |
 * | 2 | 27 | — |
 ...
 * | **10 以上（印が付く）** | **0** | — |
 *
 * **最大は 4**（三重 r05-2 と r06。母数 47 人）。**佐賀を 13 会期に広げても最大は動かない。**
 * **佐賀は 2 → 13 会期で +11**（**前の 2 会期はどちらも 1 だったので、0 が +8、1 が +3**）。
 * **佐賀の 1 は 5 会期ぶんあり、中身はすべて `猪村理恵子` / `桃崎裕介` の字の食い違いである**
 * （**`sourceConflict`。席は動いていない**）。
 *
 * **奈良が境をまたいだ場合は 17 になる**（#950 が測った 41 名中 17 名の入れ替わり）。
 *
 * ## ⚠ **線の下にも本物の境がある**（#901 佐賀が見つけた。**#961 への申し送り**）
 *
 * **佐賀の 14 会期目は一般選挙の境（2023-04）だが、`seatsChanged` は 4 にしかならない**——
 * **線 10 の下 6 である**（`saga-sessions-widen.test.ts` が一次資料から固定している）。
 *
 * **理由は 2 つ**:
 *   1. **`min(rosterAbsent, unmatchedNames)` が縮める**——**佐賀は 名簿 37 / PDF 36 人で、
 *      新顔 4・退いた 5 の非対称。** **総量 9 が `min` で 4 になる。**
 *   2. **入れ替わりの割合が小さい**（**24.3%**。青森 25.0% とほぼ同じなのに、
 *      青森は `rosterAbsent` と `unmatchedNames` が揃って 11 なので縮まない）。
 *
 * **だから `seatsChanged >= 10` を「広げてよいかの判定」に使ってはいけない。**
 * **後から数えるための記録としては有用だが、歯止めにはならない**（#951 の PO の判断どおり
 * 「鳴っても止めない」のはこの意味でも正しい）。
 */
test("#951 本番 data/: 11 議会 102 会期の seatsChanged 分布（最大 4、印が付く会期は 0 件）", async () => {
  ...
  assert.equal(rows.length, 102, "会期の合計");
  assert.equal(rows.reduce((s, r) => s + r.rollcalls, 0), 4_450, "採決の合計（#855 / #928 の母数と同じ）");
  assert.equal(rows.reduce((s, r) => s + r.votes, 0), 191_939, "票の合計（#928 の母数と同じ）");
  // **分布**（**「0 件でした」では、見ていなくても同じ顔をする**）
  const hist = new Map<number, number>();
  for (const r of rows) hist.set(r.seatsChanged, (hist.get(r.seatsChanged) ?? 0) + 1);
  assert.deepEqual(Object.fromEntries([...hist].sort((a, b) => a[0] - b[0])), { 0: 49, 1: 17, 2: 28, 3: 6, 4: 2 });
 ...
 */
test("#951 本番 data/: rosterSeen + rosterAbsent == counts.members（102 会期すべて）", async () => {
  const prefs = await localPrefs();
  let checked = 0;
  for (const p of prefs) {
  ...
  assert.equal(checked, 102, "見た会期の数（母数）");
});
...
test("#951 本番 data/: meta.sessionRosterCoverage が rollcalls/ + members/index.json から作り直した値と一致", async () => {
  ...
  assert.equal(sessions, 102, "作り直した会期の数（母数）");
});
