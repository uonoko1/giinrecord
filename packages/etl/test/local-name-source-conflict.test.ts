...
test("#711 否定的対照: 本番 data/ の unmatched 行の reason が計算し直した値と一致する", async () => {
  ...
  // **高知も 2 → 5 にして 0 → 3 行になった**（別の PR）——**武石利彦・田所裕介・橋本敏男。**
  // **3 人とも今の名簿 36 人に居ない、前に退いた議員で、理由の記載なし＝名簿に無い氏名。**
  // **`candidates` が 3 人とも 0 件**（**名簿に同姓同名すら居ないので、別人に寄せる余地が無い**）。
  // **`sourceConflict` は 1 行も増えていない**（徳島も高知も名簿が 1 ページで、突き合わせる相手が無い）。
  // **秋田も 5 → 29 本会議日にして 0 → 2 行になった**（別の PR）——**沼谷 純・鈴木 健太。**
  // **どちらも今の任期の途中で退いた議員で、理由の記載なし＝名簿に無い氏名。**
  // **`brokenGlyph` も `sourceConflict` も付かない**——
  // **秋田の名簿 41 人に同じ氏名も 1 文字違いの氏名も無いので、「同姓同名の別人」以前の問題である。**
  // **宮城も 2 → 11 にして 1 → 5 行になった**（#901）——**ゆさみゆき・中島 源陽（会派が変わるので 2 行）・
  // 渡辺 勝幸・渡辺 重益。** **4 人とも今の名簿 56 人に居ない、前に退いた議員で、理由の記載なし＝名簿に無い氏名。**
  // **`sourceConflict` は 1 行も増えていない**（宮城の名簿は 3 ページを突き合わせて作るが、食い違いは無い）。
  // **青森も 2 → 14 会期にして 3 → 10 行になった**（別の PR）——
  // **増えた 7 行は 4 人ぶん**（`阿部広悦` `谷川政人` `工藤貴弘` は会派が空の行と `自由民主党` の行で
  // 2 行ずつ、`関良` は 1 行）。**4 人とも今の任期の途中で退いた議員で、理由の記載なし＝名簿に無い氏名。**
  // **`candidates` は 4 人とも 0 件**（**名簿 46 人に同姓同名すら居ない**）。
  // **`sourceConflict` は 1 行も増えていない**——**青森の 3 行（`和田寛司` ×2・`噰引ユキ子` ×1）は
  // `--sessions 2` のときと同じで、広げて新しく「名簿と 1 文字違い」になった氏名は 1 つも無い。**
  // **#901 の鳥取で 31 → 33**（**内田隆議員・藤縄議員**。どちらも任期の途中で退いた議員）。
  // **2 行とも `reason` も `candidates` も無い**——**名簿に姓で始まる氏名が 1 人も居ない**形である。
  // **特に `内田隆議員` は、名簿に同姓の 内田 博長 が居てもなお候補 0 人**
  // （**鳥取の PDF は同姓を名の 1 文字で書き分けるので `内田博長`.startsWith(`内田隆`) が偽**）。
  // **ここが 1 人でも候補に挙がったら、別人の記録が出る一歩手前である**（#569）。
  assert.equal(rows, 33, `本番の unmatched は 33 行のはず。増減したらこの対照を測り直すこと（実測 2026-09-23）`);
