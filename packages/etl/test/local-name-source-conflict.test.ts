...
test("#711 否定的対照: 本番 data/ の unmatched 行の reason が計算し直した値と一致する", async () => {
  ...
  // **滋賀も 2 → 19 会期にして 1 → 10 行になった**（#901）——**増えた 9 行は 7 人ぶん。**
  // **`九里 学` と `大野 和三郎` は会派・字詰めの違いで 2 行ずつ**、
  // **`重田 剛` `河井 昭成` `角田 航也` `岩佐 弘明` `白井 幸則` が 1 行ずつ。**
  // **`白井 幸則` と `九里 学` は「広げたから出た」のではない**——
  // **県が 2026-09-13 → 09-23 のあいだに名簿から外したので、`--sessions 2` のままでも出る**（実測）。
