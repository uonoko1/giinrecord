...
//
// ---------------------------------------------------------------------------------------------
// NOT CLOSED, on purpose — これを信じる前に読むこと（実測値つき）
// ---------------------------------------------------------------------------------------------
// 作業合意: **防御は「不可能にすること」ではなく「隠れて通れなくすること」**。
// この検査が止めるのは「**そっと消す**」であって、「**書き換えると決めて書き換える**」ではない。
// 塞げていないものを、測った数字とともに置いておく:
//
//   1) **この検査自身の削除**（#533 で別に扱う。**この PR の範囲外**）
//      `packages/etl/package.json` の test も glob（`node --test --import tsx test/*.test.ts`）。
//      実測: このファイルを消す + `nginx-headers.test.sh` を消す → **etl 931 pass / 0 fail**
//      （origin/main の「無言の緑」と寸分違わない）。
//      **レイヤを1つ上げただけでは無限後退する。** 止めるなら glob をやめて明示列挙にするか、
//      テストファイル集合を数える検査を置くか。**このファイルは自分自身を守れない。**
//
//   2) **担当表・固定値を「両方」書き換えること**
//      `INVENTORY` と `INVENTORY_PINNED`、`SUBJECT_OWNERS` と `SUBJECT_OWNERS_PINNED` のように
//      **2 か所を揃えて書き換えれば通る**。二重化は**原理的な壁ではなく、diff に痕跡を残す仕掛け**。
...
//      **指示に従うだけで守りが外れた**ので、メッセージから「下げる」選択肢を消してある。
...
test("#513 経路1: 各 *.test.sh は検査項目を名指ししている（空にしても中身を抜いても落ちる）", () => {
  ...
  for (const { file, anchors, minAssertions: min } of INVENTORY) {
    ...
    //
    // **このメッセージが検査を黙らせないか、実際に測った**（#507）。
    // 最初の版は「**減らすなら台帳も直すこと**」と書いていた。その指示どおりに実行すると——
    // `monitor-probe.test.sh` の `test_case` を 40 本コメントアウトし、下限を 49 → 9 に下げると
    // **15 pass / 0 fail**。**指示に従うだけで、40 本の assertion が消えたことを誰も言わなくなった。**
    // だからメッセージから「下限を下げる」という選択肢を**消してある**。
    const n = assertionSites(s);
    if (n < min)
      findings.push(
        `${file}: 実際に走る assertion が ${n} 個（台帳は最低 ${min} 個と言っている）
      まず疑うのは**テストのほう**——本文をコメントに逃がしていないか、assertion ごと消えていないか。
      **意図して減らしたのでないなら、\`minAssertions\` を下げて黙らせないこと。**
      新しい書き方で assertionSites() が数え落としているだけなら、**下限を下げるのではなく**
      (a) 既存 14 本と同じ書き方（\`test_case\` / \`ok\` / \`bad\` / \`assert_*\` / \`PASS=\$((PASS+1))\`）にするか、
      (b) assertionSites() にその書き方を教える（数え方を直す）。
      本当にテストを小さくしたなら、下限と一緒に**なぜ小さくしてよいか**をこの台帳のコメントに書くこと。`,
      );
  }
  const skipped = INVENTORY.map((e) => e.file).filter(
    (f) => !audited.includes(f),
