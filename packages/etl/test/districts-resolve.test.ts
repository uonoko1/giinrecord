...
import { resolveDistricts, sangiinDistrict, shugiinDistrict } from "../src/sources/districts/resolve.ts";
...
test("parseHokkaidoBureaus: 14 局そろわなければ失敗する", () => {
  assert.throws(() => parseHokkaidoBureaus("<html><h2 id=\"a\"><a href=\"x\">空知総合振興局</a></h2><p>夕張市</p></html>"), /14/);
});
...
test("市区が丸ごと 1 つの区なら候補は 1 つ（千代田区 → 東京1）", () => {
  const r = resolveDistricts(only("東京都"), [parseDistrictText(text("tokyo"), "東京都")], bureaus);
...
test("振興局管内は北海道の所管市町村表で町村に展開する（市は別表に直接載る）。同名の町は郡を除いた名前の完全一致で区別する", () => {
  const r = resolveDistricts(only("北海道"), [parseDistrictText(text("hokkaido"), "北海道")], bureaus);
...
test("郡の括弧は町村の列挙: 東伯郡（三朝町）→ 1区、東伯郡（湯梨浜町…）→ 2区。郡だけなら郡の全町村", () => {
  const r = resolveDistricts(only("鳥取県"), [parseDistrictText(text("tottori"), "鳥取県")], bureaus);
...
test("外字 〓 は任意の 1 文字として照合し、県内で 1 つに絞れるときだけ紐づく（〓石市 → 釜石市）", () => {
  const iwate = "岩 手 県\n第１区 盛岡市\n第２区 〓石市\n";
  const r = resolveDistricts(only("岩手県"), [parseDistrictText(iwate, "岩手県")], bureaus);
...
test("区の再編で別表の区名が KEN_ALL に無い市（浜松市、2024-01-01）は再編の対応表で現在の区に展開し、複数の区にまたがれば候補を並べる", () => {
  const r = resolveDistricts(only("静岡県"), [parseDistrictText(text("shizuoka"), "静岡県")], bureaus);
...
test("別表の単位が KEN_ALL のどの市区町村にも紐づかなければ失敗する（推定しない）", () => {
  const iwate = "岩 手 県\n第１区 盛岡市、存在しない市\n";
  assert.throws(() => resolveDistricts(only("岩手県"), [parseDistrictText(iwate, "岩手県")], bureaus), /存在しない市/);
});

test("〓 で候補が 2 つ以上になれば失敗する", () => {
  const hokkaido = "北 海 道\n第１区 札幌市〓〓区\n";
  assert.throws(() => resolveDistricts(only("北海道").filter((r) => r.city.startsWith("札幌市")), [parseDistrictText(hokkaido, "北海道")], bureaus), /札幌市〓〓区.*札幌市厚別区/);
});

test("KEN_ALL にあるのに区が 1 つも付かない市区町村があれば失敗する（別表の読み落とし検知）", () => {
  const iwate = "岩 手 県\n第１区 盛岡市\n";
  assert.throws(() => resolveDistricts(only("岩手県"), [parseDistrictText(iwate, "岩手県")], bureaus), /釜石市/);
});

test("別表に無い都道府県の郵便番号があれば失敗する", () => {
  assert.throws(() => resolveDistricts(only("鳥取県"), [parseDistrictText(text("hokkaido"), "北海道")], bureaus), /鳥取県/);
});
