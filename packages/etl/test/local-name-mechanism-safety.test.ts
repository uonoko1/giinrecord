 ...
 *
 * **`total` は名簿の氏名から作るので、県が誰かを載せ替えれば動く。**
 * **実際に起きた**: **2026-09-23 に滋賀が 44 → 42 人になり 1,793 → 1,786 通りになった。**
 *
 * **本丸は `total` ではなく `wrong` が空であること**——
 * **「別人に決まる経路が無い」は、名簿が動いても保たれていなければならない。**
 * **`total` を直すときは、`wrong` が空のままであることを必ず見ること**（#943。数だけ合わせない）。
 */
test("#722 機序 1/3（1 文字欠落）: 別人に決まる例は無い", () => {
  const r = measure((key, i) => [...key.slice(0, i), ...key.slice(i + 1)].join(""));
  // 2026-09-13 に滋賀（pref-25、#741）の 44 名が入って 1,123 → 1,297（8 議会・329 名）、
  // **同日に青森（pref-02、#750）の 46 名が入って 1,297 → 1,479（9 議会・375 名）**になった。
  // **さらに秋田（pref-05、#759）の 41 名が入って 1,479 → 1,642（10 議会・416 名）**になった。
  // **2026-09-13 に佐賀（pref-41、#768）の 37 名が入って 1,642 → 1,793（11 議会・453 名）**になった。
  // **2026-09-23 に滋賀の名簿が 44 → 42 人になって 1,793 → 1,786（11 議会・451 名）**——
  // **県が `白井 幸則` と `九里 学` を名簿から外した**（#901 で広げたからではない）。
  // **母数が減っても `wrong` は空のままである。**
  // **佐賀は「一次資料どうしが 1 文字違いで食い違う」議会**（`猪村利恵子`/`猪村理恵子`、
  // `桃崎祐介`/`桃崎裕介`）**なので、1 文字欠落で別人に寄る経路が開いていないかが特に効く。**
  // **測り直した結果、佐賀を入れても `wrong` は空のままである**（other 0）。
  // **母数が増えても `wrong` が空のままであること**——**「別人に決まる経路が無い」は、
  // 名簿が増えるほど当たりにくくなる主張なので、増えた後でこそ意味がある。**
  // **測り直した結果、別人に決まる経路は 1 つも開いていない**（other 0 / wrong []）。
  // **青森は `櫛引ユキ子` が 3 通りに壊れる議会**（#749）なので、ここが 0 であることが特に効く。
  assert.equal(r.total, 1786, "母数が変わった＝名簿が変わっている。docblock ごと測り直すこと");
  assert.deepEqual(r.wrong, [], "**実データで起こりうる欠落で別人に決まる経路が開いた**（#569）");
  assert.equal(r.self, 1667, "本人に決まる数が変わった。名簿が変わっている");
});
...
test("#722 機序 4（別の字に化ける）: 別人に決まらず、全件 brokenGlyph として落ちる", () => {
  ...
  for (const replacement of ["□", "◇", "■", "?", "�"]) {
    ...
    assert.equal(r.total, 1786, `母数が変わった（${replacement}）`);
    assert.deepEqual(r.wrong, [], `**${replacement} に化けて別人に決まる経路が開いた**`);
    assert.equal(r.none, 1786, `${replacement} は全件 unmatched に落ちるはず`);
