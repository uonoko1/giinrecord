...
test("#722 機序 1/3（1 文字欠落）: 別人に決まる例は無い", () => {
  ...
  assert.equal(r.total, 1793, "母数が変わった＝名簿が変わっている。docblock ごと測り直すこと");
  assert.deepEqual(r.wrong, [], "**実データで起こりうる欠落で別人に決まる経路が開いた**（#569）");
  assert.equal(r.self, 1674, "本人に決まる数が変わった。名簿が変わっている");
});
...
test("#722 機序 4（別の字に化ける）: 別人に決まらず、全件 brokenGlyph として落ちる", () => {
  ...
  for (const replacement of ["□", "◇", "■", "?", "�"]) {
    ...
    assert.equal(r.total, 1793, `母数が変わった（${replacement}）`);
    assert.deepEqual(r.wrong, [], `**${replacement} に化けて別人に決まる経路が開いた**`);
    assert.equal(r.none, 1793, `${replacement} は全件 unmatched に落ちるはず`);
