...
/**
 * **会派帯の上に注記や凡例があると、それが会派名の頭に付く**（#901 で `--sessions` を
 * 2 → 19 に広げて出た）。
 *
 * **`readGroups` は帯の上端に `grid.top` を使っていた。** **`grid.top` は「表の外枠」**であって
 * 会派帯の上端ではない——**外枠と会派帯のあいだに見出しや注記の段がある本では、
 * その段の文字が会派名に流れ込む。**
 *
 * **実測（146 本すべて）**: **25 本・825 列**で会派が壊れていた。2 通りある:
 *   - `（議案についてはこのホームページの議案詳細情報をご覧ください。）自由民主党滋賀県議会議員団`
 *   - `「○」は賛成を、…表す。チームしが県議団`
 *
 * **`group` は票の 1 件ずつに書かれ、`unmatched.json` の鍵にもなる**ので、
 * **同じ議員の同じ会派が「別の会派」に見える**（`--sessions 19` で `unmatched` の
 * `重田 剛` と `白井 幸則` が、会派違いで 2 行ずつに割れていた。実測）。
 *
 * **直し方**: **帯の上端は「会派帯の下の罫線より上にある罫線のうち、いちばん低いもの」。**
 * **外枠まで飛ばさない。** 健全な本ではこれが `grid.top` と一致するので、出力は変わらない。
 */
test("#901 readGroups: 会派帯の上の注記を会派名に混ぜない（Kg835。20/42 列が壊れていた）", async () => {
  const pdf = await parseVotePdf(fixture("Kg835_0425sanpi2.pdf"));
  assert.equal(pdf.members.length, 42);
  // **母数を出す**（#757）——42 列を 1 つ残らず見る
  const groups = new Map<string, number>();
  for (const m of pdf.members) groups.set(m.group, (groups.get(m.group) ?? 0) + 1);
  assert.equal([...groups.values()].reduce((a, b) => a + b, 0), 42);
  // 注記（`（議案については…ご覧ください。）`）も凡例（`…を表す。`）も会派名に入らない
  for (const [g, n] of groups) {
    assert.ok(!/ご覧ください|表す。/.test(g), `会派名に注記が混ざっている: [${g}]（${n} 列）`);
    assert.notEqual(g, "", "会派名が空の列がある");
  }
  // **この PDF の会派の内訳**（原文どおり。合計 42）
  assert.deepEqual(Object.fromEntries([...groups].sort()), {
