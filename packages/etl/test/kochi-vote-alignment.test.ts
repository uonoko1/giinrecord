...
test("#876 欠陥④: `-` U+002D は**共有の**字形表では寄らない —— 2 本が落ちる（寄せると滋賀が壊れる）", async () => {
  ...
  //
  // ## **共有の表に足してはいけない**（#901 の宮城が一度足して、#950 の奈良の測定で取り消した）
  // **滋賀の凡例は `-` U+002D そのものを「欠席」の鍵として使っている**
  // （`Kg220_240424-sanpi.pdf` の凡例は `{"-":"欠席","議":"議長（表決権なし）","退":"退席"}`。実測）。
  // **共有の表で `-` → `－` に寄せると、その本の凡例から `-` が引けなくなる。**
  // **同じ符号位置が議会ごとに違う意味を持つので、これは「見た目が同じ別コードポイント」ではなく意味の衝突である。**
  // **宮城は議会の中だけで寄せた**（`miyagi/votes-pdf.ts` の `miyagiLegendKey`。
  // **宮城の 13 本の凡例に `-` U+002D は 1 度も出てこない**ので、宮城の中では衝突しない）。
  // **高知で同じことをしてよいかは測っていない**（高知の 2 本の凡例を見ていない）。**別の PBI である。**
});
