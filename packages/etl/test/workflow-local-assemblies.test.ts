...
test("local-assemblies.yml: 日次と同じイメージで local-cli.ts を ASSEMBLIES（miyagi=pref-04 tokushima=pref-36 tottori=pref-31 mie=pref-24 nara=pref-29 shimane=pref-32 kochi=pref-39 shiga=pref-25）の順に走らせ、パイプの失敗を拾う（shell: bash）", () => {
  assert.match(workflow, /ASSEMBLIES: "miyagi=pref-04 tokushima=pref-36 tottori=pref-31 mie=pref-24 nara=pref-29 shimane=pref-32 kochi=pref-39 shiga=pref-25"/, "三重（#203）・奈良（#202）・島根（#221）・高知（#220）も同じループで走る");
