...
test("local-assemblies.yml: 日次と同じイメージで local-cli.ts を ASSEMBLIES（miyagi=pref-04 tokushima=pref-36）の順に走らせ、パイプの失敗を拾う（shell: bash）", () => {
  assert.match(workflow, /ASSEMBLIES: "miyagi=pref-04 tokushima=pref-36"/);
