 ...
 */
const DENOMINATOR: Record<string, number> = { 宮城: 356, 奈良: 125, 鳥取: 133, 島根: 140, 徳島: 132 };

/** **交代当日の行**（`allowPrev` のとき「2 人のどちらか」しか言えない行）。**実測。** */
const HANDOVER_ROWS: Record<string, number> = { 宮城: 0, 奈良: 37, 鳥取: 0, 島根: 0, 徳島: 0 };

test("#906 母数を先に固定する: 5 県の判定できた行は 356 / 125 / 133 / 140 / 132（計 886）", () => {
  let total = 0;
 ...
 * **落ちた行数は 125 → 90 へ減る。** **「回転で落ちる行数が多いほど錨が強い」と読めない例である。**
 */
test("#906 奈良は `strict` でも `−1` で 90 / 125 しか落ちない（**ずらしたほうが一致が増える**）", () => {
  const byRot: Record<number, number> = {};
  for (const k of ROTATIONS) byRot[k] = probeNara(k, "strict").mismatch;
  assert.equal(byRot[1], 125, "+1 列回転");
  assert.equal(byRot[2], 125, "+2 列回転");
  assert.equal(byRot[-1], 90, "**−1 列回転: 90 / 125 しか落ちない**");
  assert.equal(byRot[-2], 125, "−2 列回転");
  // **無改造 35 落ち → `−1` で 90 落ち。** **交代当日の 37 行だけを見ると 35 → 2 へ「改善」している。**
  assert.equal(probeNara(0, "strict").mismatch, 35, "無改造");
  // **その 37 行だけを取り出すと、`−1` のほうが一致が多い**（#891 が高知で見つけたのと同じ向きの罠）。
  assert.ok(byRot[-1] - 35 < 125 - 35, "`−1` は他の回転より捕まえる行が少ない");
});
 ...
 */
test("#906 `allowPrev`: **奈良が `−1` で 35 行・`+1` で 2 行を取り逃がす**（高知と同じ機序）", () => {
  const nara: Record<number, number> = {};
  for (const k of ROTATIONS) nara[k] = probeNara(k, "allowPrev").mismatch;
  // **母数 125 のうち、落ちるべきなのに落ちない行**
  assert.equal(nara[1], 123, "+1 列回転: 2 行が生き残る");
  assert.equal(nara[2], 125, "+2 列回転: 穴は無い");
  assert.equal(nara[-1], 90, "**−1 列回転: 35 行が生き残る**");
  assert.equal(nara[-2], 125, "−2 列回転: 穴は無い");
