 ...
 * `"議"`(x=43.08, y=694.2800036621094) のように、
 * **差 2.563e-5 pt（`h`=8.64 に対して 3.0e-6）で x の逆順に並んでいた。**
 * **許容差を `1e-13` のような「double の誤差ぶんだけ」にすると、この本は救えない。**
 *
 * **x の順（43.08 → 173.28）と y の順（694.280029… が先）が食い違う値を選んである**——
 * **食い違わない値だと、直っていなくても通ってしまう**（そういうテストは何も主張していない）。
 */
