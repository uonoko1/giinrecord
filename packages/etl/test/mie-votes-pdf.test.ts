...
/* ---------- #901: 議案等番号の 2 つの別の形（`意見書案N号` / `意見書第N案`） ---------- */

/**
 * ## **`--sessions` を 2 から広げようとして最初に当たる壁**（Issue #901）
 *
 * **三重の既定は `--sessions 2`（令和8年・令和7年）で、本番に出ているのは 365 件である。**
 * **`--sessions 3` にすると ETL は 1 本の PDF で例外を投げて止まる**——
 * **`index.ts` は `parseVotePdf` の例外を握り潰さない**ので、**会期まるごと出ない。**
 *
 * **2026-09-20 に index の賛否 PDF 151 本すべてを取得して、`議案等番号` のセルの形を数えた**
 * （`.measure/901/numbers.ts`。**母数 3,700 個**）:
 *
 * | 形 | 件数 | `{種別}第N号` に当たるか |
 * |---|---:|---|
 * | `議案第N号` | 2,838 | ○ |
 ...
 * | `諮問第N号` | 5 | ○ |
 * | **`意見書案N号`**（`第` が無い） | **8** | **×** |
 * | **`意見書第N案`**（`第` と `号`/`案` が入れ替わる） | **2** | **×** |
 *
 * **通らないのは 2 種・のべ 10 個だけである**（3,700 個中 0.27%）。
 * **どちらも「種別」と「番号」の両方がセルに書いてあり、推定は要らない**（#569）——
 * **足りないのは読み方であって、事実ではない。**
 *
 * **この 10 個が 2 本の PDF に効いて、`--sessions 3` と `--sessions 4` を塞いでいた:**
 *   `001172850.pdf`（令和6年12月、`意見書第26案` `意見書第27案`）→ **r06 が出ない**
 *   `001086178.pdf`（令和5年6月、`意見書案３号` ほか）→ **r05-2 が出ない**
 *
 * **`kind` と `number` は採決 id（`pref-24-{会期}-{日付}-{kind}-{number}`）に入る**ので、
 * **同じ本の中で別の議案が同じ id にならないことを下で数える。**
 */
const dec2024 = await parseVotePdf(bytes("001172850.pdf")); // 令和6年定例会（12月）
const jun2023 = await parseVotePdf(bytes("001086178.pdf")); // 令和5年第2回定例会（6月）

test("#901 `意見書第N案`（第 と 号/案 が入れ替わる形）を、原文のまま kind と number に割る", () => {
  assert.equal(dec2024.sessionName, "令和６年定例会");
  assert.equal(dec2024.year, 2024);
  assert.equal(dec2024.month, 12);
  // **母数を先に出す**（#757）。行が減れば下の抽出は空回りする
  assert.equal(dec2024.rows.length, 41, `行 ${dec2024.rows.length}`);
  const odd = dec2024.rows.filter((r) => r.kind === "意見書" && /案$/.test(r.number));
  assert.equal(odd.length, 2, `\`意見書第N案\` の行 ${odd.length}`);
  assert.deepEqual(odd.map((r) => [r.kind, r.number, r.title]), [
    ["意見書", "第26案", "私学助成の充実を求める意見書案"],
    ["意見書", "第27案", "配合飼料価格安定制度の改善を求める意見書案"],
  ]);
  // **番号を作り直していない**（`第26号` に寄せていない。原文は `案` である。#569）
  assert.deepEqual(odd.map((r) => r.kind + r.number), ["意見書第26案", "意見書第27案"]);
});

test("#901 `意見書案N号`（`第` が無い形）を、原文のまま kind と number に割る", () => {
  assert.equal(jun2023.sessionName, "令和５年第２回定例会");
  assert.equal(jun2023.year, 2023);
  assert.equal(jun2023.month, 6);
  assert.equal(jun2023.rows.length, 16, `行 ${jun2023.rows.length}`);
  const odd = jun2023.rows.filter((r) => r.kind === "意見書案" && !/^第/.test(r.number));
  assert.equal(odd.length, 1, `\`意見書案N号\` の行 ${odd.length}`);
  assert.deepEqual(odd.map((r) => [r.kind, r.number, r.result]), [["意見書案", "３号", "否決"]]);
  // **`第` を足していない**（原文に無い文字を出力に入れない。#569）
  assert.equal(odd[0].kind + odd[0].number, "意見書案３号");
  // **同じ本の中に `意見書案第N号` も在る**——**同じ種別で 2 つの書き方が混在している。**
  // **「この本は `第` を使わない」ではないので、片方だけを見る実装にしない。**
  assert.deepEqual(
    jun2023.rows.filter((r) => r.kind === "意見書案").map((r) => r.number),
    ["第１号", "第２号", "３号"],
    "同じ本の 意見書案 3 行（2 つは `第N号`、1 つは `N号`）",
  );
});

test("#901 2 本とも、同じ本の中で (kind, number) が重複しない（採決 id が衝突しない）", () => {
  for (const [name, pdf] of [["001172850.pdf", dec2024], ["001086178.pdf", jun2023]] as const) {
    const keys = pdf.rows.map((r) => `${r.dateText}-${r.kind}-${r.number.normalize("NFKC")}`);
    assert.equal(new Set(keys).size, keys.length, `${name}: (議決月日, kind, number) が重複している`);
    assert.ok(keys.length > 0, `${name}: 行が 0`);
    // **`kind` も `number` も空でない**（`.+?` が空に落ちて全部が同じ id になる形を塞ぐ）
    assert.deepEqual(pdf.rows.filter((r) => r.kind === "" || r.number === "").map((r) => r.title), [], `${name}: kind か number が空`);
  }
});

/* ---------- #901: 会派見出しが 2 列に折り返すと、文字の並びが崩れる ---------- */

/**
 * ## **`--sessions 2` では 1 件も出ないが、広げると出る**（Issue #901）
 *
 * **会派見出しは結合セルで、名前が長いと縦書きが 2 列に折り返す。**
 * **日本語の縦書きは右の列から読む**が、`readMembers` は文字を
 * **`(b.y - a.y || a.x - b.x)`（上から下 → 左から右）** で並べていた。
 * **1 列で収まる見出しでは同じ結果になるが、2 列に折り返すと左右が 1 文字ずつ交互に混ざる。**
 *
 * **実測（2026-09-20、index の賛否 PDF 151 本すべて。`.measure/901/groupscan.ts`）:**
 * **読めた 87 本に会派見出しの原文が 21 種あり、うち 4 種が崩れていた**——
 *
 * | 出力されていた原文 | 本数 | 正しい原文 |
 * |---|---:|---|
 * | `運草動のい根が` | 36 | **`草の根運動いが`** |
 * | `運草動のみ根え` | 12 | **`草の根運動みえ`** |
 * | `運※動草いのが根` | 1 | （`※` 付きの同じ会派） |
 * | `※み1ん新なしのい党翼` | 1 | （`※` 付きの `新しい翼`） |
 *
 * **`草の根運動いが` が正しく出ていた本も 2 本ある**（1 列に収まった本）——
 * **同じ会派が、本によって 2 通りの文字列で出ていた。**
 *
 * **`--sessions 2`（令和8年・令和7年）の 13 本には 1 件も無い**ので、
 * **本番に出ている 365 件は影響を受けていない。**
 * **`--sessions 4` まで広げると 9 列 / 243 票（母数 34,590 = 0.70%）に効く**（すべて 稲森 稔尚）。
 *
 * **これは「記録が出ない」ではなく「別の文字列が出る」側である**（#569）——
 * **利用者から見て、会派名が間違っていることは検出できない。**
 * **広げる前に直す必要がある。**
 *
 * **直し方**: **見出しセルの中のグリフを x でクラスタに分け、列を右から左へ、
 * 各列は上から下へ読む。** **1 列の見出しでは今までと 1 バイトも変わらない。**
 */
const jun2023groups = [...new Set(jun2023.members.map((m) => m.group))];

test("#901 2 列に折り返した会派見出しを、右の列から読む（`草の根運動いが`）", () => {
  // **母数**（#757）: この本の会派は 7 つ
  assert.equal(jun2023groups.length, 6, `会派 ${jun2023groups.length}`);
  assert.ok(jun2023groups.includes("草の根運動いが"), `会派の原文: ${jun2023groups.join(" / ")}`);
  // **崩れた並びが出ていないこと**（同じ文字を別の順に並べたもの）
  assert.ok(!jun2023groups.includes("運草動のい根が"), `崩れた並びが残っている: ${jun2023groups.join(" / ")}`);
  // **その会派の議員はちょうど 1 人**（見出しが列に結びついていることの検算）
  const inaomori = jun2023.members.filter((m) => m.group === "草の根運動いが");
  assert.equal(inaomori.length, 1, `草の根運動いが の列 ${inaomori.length}`);
  assert.equal(inaomori[0].nameText, "稲森 稔尚");
});

test("#901 1 列に収まる会派見出しは今までどおり（令和8年6月の 7 会派が 1 バイトも変わらない）", () => {
  // **直し方が「2 列のときだけ効く」ことを、既存の本で固定する**（等価でない変更を混ぜていないこと）
  const groups = new Map<string, number>();
  for (const m of jun.members) groups.set(m.group, (groups.get(m.group) ?? 0) + 1);
  assert.deepEqual([...groups.entries()], [
    ["新政みえ", 19], ["自由民主党", 16], ["自民党県議団", 5],
    ["草莽", 3], ["公明党", 2], ["日本共産党", 1], ["参政党", 1],
  ]);
  assert.equal([...groups.values()].reduce((a, b) => a + b, 0), 47, "母数");
});

/**
 * **`001172850.pdf`（令和6年12月）には `草の根運動いが` の列が無い**（実測。5 会派 46 人）——
 * **その会派が崩れて出ていたのは令和6年の別の月の本である。**
 * **「同じ会期だから同じ会派が並ぶ」と決め打ちしない**（#569。本ごとに数える）。
 * **ここで固定するのは「崩れた並びが 1 つも無いこと」と「全列に会派が付くこと」。**
 */
test("#901 令和6年12月（`001172850.pdf`）は 5 会派 46 人で、崩れた並びが 1 つも無い", () => {
  const gs = [...new Set(dec2024.members.map((m) => m.group))];
  assert.deepEqual(gs, ["自由民主党", "新政みえ", "草莽", "公明党", "日本共産党"], `会派 ${gs.length}`);
  // **崩れた並び（同じ文字の別の順）が 1 つも無い**
  const bag = (x: string) => [...x].sort().join("");
  const known = ["草の根運動いが", "草の根運動みえ", "新しい翼"];
  assert.deepEqual(gs.filter((g) => known.some((k) => bag(k) === bag(g) && k !== g)), [], "崩れた並び");
  // **どの列にも会派が付いている**（見出しを読み直して列との対応が崩れていないこと）
  assert.equal(dec2024.members.filter((m) => m.group === "").length, 0);
  assert.equal(dec2024.members.length, 46, `議員の列 ${dec2024.members.length}`);
});
