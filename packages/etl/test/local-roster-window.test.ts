 ...
 * | | 母数 | 結果 |
 * |---|---|---|
 * | **名簿に無い `memberId`** | **58,057 セル / 1,369 採決 / 11 議会** | **0 件** |
 ...
 *
 * **母数を一緒に持つ**（#757）——**`rollcalls` の合計が 1,369 であることを下で検算する。**
...
test("#928 本番 data/: 11 議会の rosterAsOf と採決日の窓（7 議会が後ろにはみ出し、最大は鳥取 1,156 日）", async () => {
  ...
  assert.deepEqual(got, {
    ...
    "pref-24": { daysAfter: 224, daysBefore: 302, votesAfter: 190, rollcalls: 365 },
  ...
  assert.equal(Object.values(got).reduce((s, x) => s + x.rollcalls, 0), 1_369, "11 議会の採決の合計（#855 の母数と同じ）");
