...
test("#537 新しい議会を足すときのチェックリストがある（満たせない議会は実装しない）", () => {
  ...
  assert.ok(boxes >= 7, `チェックリストの項目が ${boxes} 個しかない（7 個以上あったはず）`);

  // **数だけでは、どの項目が消えたか分からない。**
  // **実際に事故を踏んだ項目は、逐語で押さえる**（#750 で gitleaks が発火した。PR #761）。
  // 数を増やすだけの検査だと、この行を消して別の行を足せば通ってしまう。
  const mustHave: [string, RegExp][] = [
    ["robots.txt を読む", /robots\.txt/],
    ["1 秒以上の間隔・UA・直列", /1 秒以上の間隔/],
    ["出典 URL を meta.json に", /`sources`/],
    ["第三者サービスのトークン（#750 で実際に踏んだ）", /第三者サービスのトークン/],
  ];
  const missing = mustHave.filter(([, re]) => !re.test(md)).map(([name]) => name);
  assert.deepEqual(missing, [], `${CONTRACT} のチェックリストから項目が落ちている。
**実際に事故を踏んだ項目である。** 消すなら、なぜ不要になったかを書いてからこの検査も直すこと。`);
});
