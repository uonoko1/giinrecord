...
test("#901 `--sessions` を渡さないとき、CLI は議会ごとの既定を使う（三重 4 / 徳島 4 / 宮城 11 / 奈良 2）", async () => {
  ...
  assert.equal(await sessionsPassedTo("nara"), 2, "測っていない議会は 2 のまま");
  ...
  assert.equal(await sessionsPassedTo("nara"), defaultSessionsFor("nara"));
});
