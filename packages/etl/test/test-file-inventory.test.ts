...
const ETL_TEST_FILES_MIN = 181; // 実測 2026-09-25（#1017）: readdirSync で 184（− 3。#720 の規則）。#1017 が workflow-needs-resolve.test.ts を足して 183 → 184。**164 のままでは 20 本消しても落ちなかった**——#943 が 167 本の時点で置いてから main が 17 本増えたのに、下限だけが置き去りになっていた（#720 が詰めた「実数から離れたぶんだけ守っていない」がまた開いた）。以前の経緯: **#943 が workflow-data-pr-push.test.ts を足して 166 → 167**（下限 159 → 164）。**#951 が local-session-roster-coverage.test.ts を足して 165 → 166**（下限 159 → 161）。#928 が 156 → 157、**#901 が local-sessions-default.test.ts と local-cli-sessions.test.ts を足して 157 → 159**、高知の kochi-petition-branch.test.ts と kochi-widened-anchors.test.ts で 159 → 161、秋田の akita-vote-alignment.test.ts で 161 → 162。**139 のままだと 18 本消しても落ちなかった**
