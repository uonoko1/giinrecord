 ...
 *
 * 1. **`counts` が 105 行とも無い。** だから **「`○` の数 = `counts.yes`」が書けない**
 ...
 * 2. **`○` と `●` に `mapped` が付かない。** 3,780 票のうち **3,578 票（94.7%）が `mapped` 無し。**
 ...
 * **徳島だけにある形がもう 2 つ:**
 *
 * 3. **`○` U+25CB と `〇` U+3007 が同じ PDF の同じ 20 行に混ざっている**（1064407.pdf）。
 *    **`raw` は原文のまま残す**ので **2 通りの字が `data/` に出る**（#674 / `glyph-variants.ts` の
 *    コメントが「徳島の本番データ 74 件がその形」と名指ししている）。
 * 4. **`committeeResult`（委員会審査結果）が 105 行ともある。** ほかの 10 議会には無い欄。
 */
...
test("#865 本番 pref-36: 採決 105 × 議員 36 = 3,780 セル、抽出不能 0.00%", { skip: !hasData }, () => {
  const m = meta();
  const rcs = rollCalls();
  // **実測 2026-09-14**（直近 2 会期。月次のワークフローは既定の `--sessions 2` で回る）
  assert.equal(m.counts.rollcalls, 105);
  assert.equal(m.counts.members, 36);
  assert.equal(m.counts.cells, 3_780);
  ...
  // **105 行とも 36 セルちょうど**（会期の途中で議員が 1 人静かに消えていない。#705 が滋賀で踏んだ形）
  const perRow = new Map<number, number>();
  for (const r of rcs) perRow.set(r.votes.length, (perRow.get(r.votes.length) ?? 0) + 1);
  assert.deepEqual(Object.fromEntries(perRow), { 36: 105 }, "採決ごとのセル数");
 ...
 */
test("#865 本番 pref-36: counts は 105 行とも無い／○ と ● に mapped が付かない（推定を拒んだ跡）", { skip: !hasData }, () => {
  const rcs = rollCalls();
  assert.equal(rcs.length, 105, "母数");
  ...
  assert.deepEqual(meta().countChecked, { checked: 0, noCounts: 105, rows: 105, unreadableCells: 0 });
  // **`mapped` が付くのは「票を投じていない」4 つの凡例だけ**
  const votes = rcs.flatMap((r) => r.votes);
  assert.equal(votes.length, 3_780, "母数");
  const unmapped = votes.filter((v) => v.value.mapped === undefined);
  assert.equal(unmapped.length, 3_578, "mapped の無い票（3,578 / 3,780 = 94.7%）");
  ...
  assert.equal(votes.length - unmapped.length, 202, "mapped の付く票");
 ...
 */
test("#865 本番 pref-36: 票の内訳と、○ U+25CB / 〇 U+3007 が同じ 20 行に混ざっていること", { skip: !hasData }, () => {
  const rcs = rollCalls();
  const votes = rcs.flatMap((r) => r.votes);
  assert.equal(votes.length, 3_780, "母数（減っていたらこの内訳は意味が無い）");
  ...
  assert.deepEqual(Object.fromEntries([...raw].sort()), { "●": 93, "〇": 74, "○": 3411, "欠": 85, "議": 105, "退": 10, "除": 2 });
  assert.deepEqual(Object.fromEntries([...legend].sort()), {
    "欠席": 85, "委員会審査結果又は議長宣告に起立しなかった者": 93, "委員会審査結果又は議長宣告に起立（賛成）した者": 3485,
    "退席": 10, "議長": 105, "除斥": 2,
  });
  // **2 つの字は別の符号位置**（寄せていない）
  ...
  assert.equal(withMaru2.length, 20, "`〇` の出る採決");
  assert.equal(new Set(withMaru2.map((r) => r.sourceUrl)).size, 1, "`〇` の出る PDF は 1 本");
  assert.deepEqual([...new Set(withMaru2.map((r) => r.sessionId))], ["2026-06"], "`〇` の出る会期");
  assert.deepEqual(withMaru2.filter((r) => !r.votes.some((v) => v.value.raw === "○")).map((r) => r.id), [],
    "**20 行とも `○` と `〇` が同じ行に混ざっている**");
});
 ...
 */
test("#865 本番 pref-36: `議` は 105 採決すべてで 1 人、会期ごとに 須見 一仁 → 井川 龍二", { skip: !hasData }, () => {
  const rcs = rollCalls();
  ...
  assert.deepEqual(Object.fromEntries(perRollCall), { 1: 105 }, "採決ごとの `議` の数");
  ...
  assert.deepEqual(
    ...
    { "2026-02": { "須見 一仁": 85 }, "2026-06": { "井川 龍二": 20 } },
  );
});
 ...
 */
test("#865 本番 pref-36: `欠` 85 件は坂口 誠治 1 人に集中、`退` 10 件は 9 人、`除` 2 件は 2 人", { skip: !hasData }, () => {
  const rcs = rollCalls();
  ...
  const ketsu = by("欠");
  assert.deepEqual(Object.fromEntries(ketsu), { "坂口 誠治": 85 }, "`欠` の内訳");
  // **その 85 件はすべて 2026-02 会期**（もう 1 会期には `欠` が無い）
  const ketsuSessions = new Set(rcs.filter((r) => r.votes.some((v) => v.value.raw === "欠")).map((r) => r.sessionId));
  assert.deepEqual([...ketsuSessions], ["2026-02"]);
  const tai = by("退");
 ...
 */
test("#865 本番 pref-36: 36 人が氏名と 1 対 1（未突合 0、票に空の memberId が無い）", { skip: !hasData }, () => {
  const rcs = rollCalls();
  const votes = rcs.flatMap((r) => r.votes);
  assert.equal(votes.length, 3_780, "母数");
  ...
  assert.deepEqual([...new Set(tokushima.map((m) => m.counts?.rollcalls))], [105], "議員ごとの採決数");
});
 ...
 */
test("#865 本番 pref-36: sourceUrl は 9 本すべて https://www.pref.tokushima.lg.jp、委員会審査結果が 105 行とも", { skip: !hasData }, () => {
  const m = meta();
  const rcs = rollCalls();
  ...
  assert.equal(pdfs.size, 4, "採決の出典 PDF");
  assert.deepEqual([...pdfs.values()].sort((a, b) => b - a), [83, 20, 1, 1], "PDF ごとの採決数");
  assert.equal([...pdfs.values()].reduce((a, b) => a + b, 0), 105, "合計");
  assert.deepEqual([...pdfs.keys()].filter((u) => !u.endsWith(".pdf")), [], "PDF でない出典");
  // **会期は 2 本、内訳は 85 + 20 = 105。採決日は 4 日**
  assert.deepEqual(m.sessions.map((s) => [s.sessionId, s.rollcalls]).sort(), [["2026-02", 85], ["2026-06", 20]]);
  assert.deepEqual(
    Object.fromEntries([...new Set(rcs.map((r) => r.date))].sort().map((d) => [d, rcs.filter((r) => r.date === d).length])),
    { "2026-02-13": 1, "2026-02-20": 1, "2026-03-11": 83, "2026-07-03": 20 },
  );
  ...
  assert.deepEqual(Object.fromEntries([...cr].sort()), { "-": 13, "－": 3, "不採択": 4, "可決": 83, "承認": 1, "採択": 1 });
  assert.equal([...cr.values()].reduce((a, b) => a + b, 0), 105, "委員会審査結果の合計");
  // **全部の採決に日付・件名がある**
...
test("#865 本番 pref-36: 36 人全員に kana・選挙区・会派がある", { skip: !hasData }, () => {
  ...
  assert.equal(meta().rosterAsOf, "2026-09-05");
});
