...
/**
 * **「票の比較キーに y が入っていない」ことを、ソースを読んで固定する**（#1000 の 3 番目と同じ形）。
 *
 * **#999 の揺れは y の丸め誤差である。** 4 県の票の道に y が入った時点で、
 * **上の混ぜる検査を通ってしまう壊れ方が生まれうる**
 * （混ぜても結果が同じなのに、**本物の y のわずかな差で列が入れ替わる**形）。
 * **だから「混ぜて同じ」だけでは足りない。比較キーそのものを見る。**
 *
 * **これは denylist なので「これで全部」ではない**——
 * **`cy` を別名の変数に入れてから引く / 比較を別ファイルに出す、といった形はすり抜ける。**
 * **上の振る舞いの検査が一次で、ここは二次である。**
 */
test("#1002 4 県の readVoteCells の本体に y / cy の比較が無い", () => {
  const targets = [
  ...
  for (const t of targets) {
    ...
    checked++;
    // `a.y - b.y` / `b.cy - a.cy` のような、y を引く比較
    if (/\b\w+\.c?y\s*-\s*\w+\.c?y\b/.test(body)) offenders.push(`${t.pref}: y の差を取っている`);
  }
  assert.equal(checked, 4, "4 県ぶん見ているはず（母数）");
  assert.deepEqual(offenders, [], "票を決める関数の中で y を比較している（#999 の揺れが票に届く）");
});
