...
test("#819 y 方向: 公表された賛成者数・反対者数が、その行の記号帯の ○ / × の数と合う", async () => {
  ...
  assert.equal(judgeable, 501, "**母数**——判定できた行（#901 で秋田に 3 本足して 414 → 418、青森に 2 本足して 418 → 501）");
...
test("#819 y 方向: 記号帯を 1 行ずらすと落ちる（この検算が恒真でないこと）", async () => {
  ...
  assert.equal(judgeable, 497, "**母数**——回して判定できた行（1 行だけの本を除く）");
  ...
  assert.equal(caught, 218, "**1 行ずらして落ちた行**（#819 では 177 / #829 で 176 / #901 で 179 → 218）");
});
