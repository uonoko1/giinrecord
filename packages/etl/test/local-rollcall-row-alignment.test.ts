...
test("#819 y 方向: 公表された賛成者数・反対者数が、その行の記号帯の ○ / × の数と合う", async () => {
  ...
  assert.equal(judgeable, 418, "**母数**——判定できた行（#901 で秋田に 3 本足して 414 → 418）");
...
test("#819 y 方向: 記号帯を 1 行ずらすと落ちる（この検算が恒真でないこと）", async () => {
  ...
  assert.equal(judgeable, 414, "**母数**——回して判定できた行（1 行だけの本を除く）");
  // **#901 で 176 → 179**——**足した `514.pdf` の 3 行が 3 行とも落ちる**
  // （**増えた範囲の行も、この y の検算にちゃんと掛かっている**）
  assert.equal(caught, 179, "**1 行ずらして落ちた行**（#819 では 177 / #829 で 176 / #901 で 179）");
});
