 ...
 *
 * **`counts` 以外に置ける検算は見つかっていない**——
 * 議決日・議案番号・件名・議決結果は**すべて左の欄にある**ので、
 * 記号帯との対応を確かめる材料にならない（左の欄どうしの整合しか見ない）。
 * **記号帯の側にあるのは記号だけである。**
 */
...
/**
 * **今わかっている、公表数と記号の数が合わない行**（#819 の実測。**すべて秋田の同じ 1 つの機序**）。
 *
 * **直っていない。** ここに**実物を並べて固定する**のは、
 *   - **「合わない行は 0 件」と書けないから**（書いたら嘘になる）
 *   - **1 件でも増えたら落ちるようにするため**（新しい機序が混じったら気づける）
 *   - **直したら落ちるようにするため**（直した人が、この表とこのコメントを消すことになる）
 * **「既知だから無視してよい」という意味ではない。** 機序は下の `#819 秋田: ページ番号` に書いてある。
 */
const KNOWN_BAD = [
  "akita/041222hyoketsu.pdf\t議案第209号 生活排水処理事業の運営に係る連携協約を締結する協議について\t42/42\t0/1",
  "akita/060220hyoketsu.pdf\t議案第10号 令和５年度急傾斜地崩壊対策事業に要する経費の一部負担の変更について\t40/40\t0/1",
  "akita/h231202giketu.pdf\t事提出認定第２号 平成２２年度秋田県歳入歳出決算の認定について\t42/42\t2/1",
  "akita/h291222giketu.pdf\t議案第214号 公の施設の指定管理者の指定について\t40/40\t0/40",
  "akita/h291222giketu.pdf\t請願第40号 県立高校図書館の充実を求める請願について\t40/40\t0/40",
];
...
test("#819 y 方向: 公表された賛成者数・反対者数が、その行の記号帯の ○ / × の数と合う", async () => {
  ...
  // **合わない行は既知の 5 行だけ**（すべて秋田のページ番号の機序。下のテストの注を読むこと）。
  // **増えても減っても落ちる。**
...
test("#819 y 方向: 記号帯を 1 行ずらすと落ちる（この検算が恒真でないこと）", async () => {
  ...
  assert.ok(caught >= 150, `1 行ずらして落ちた行 ${caught}（実測 177 / 411。150 を下回ったら検算の効きが落ちている）`);
});
 ...
 *
 * **捏造された値の実物**（フィクスチャ 6 本での実測。**この 3 行は現に壊れている**）:
 * ```
 ...
 *
 * **落ちるほうの実測は 4 行**（`041222` p2 請願第57号 / `080319` p1・p2・p3）。
 * **`080319` は本番に出ている PDF で、本番の秋田で `counts` が欠けている 7 件のうち 3 件がこれ。**
 *
 * **直っていない。** **直す PR が別に要る**（#819 は測定の PBI で、直すのは範囲の外）。
 * **このテストは「壊れている 5 行がちょうどこれだけ」を固定する**——
 * **直したら落ちる**（直した人がこの表を消す）。**増えたら落ちる**（新しい機序に気づける）。
 * **直し方の候補**（どれも測っていないので、直す人が測ること）:
 *   - 錨からの距離に上限を置く（行の高さの n 倍まで）
 *   - 票の行の帯の外（ページ下端）のアイテムを落とす
 * **どちらも 154 本すべてに当たるので、当てて数えてから決めること。**
 */
/** 上の `KNOWN_BAD` と同じ 5 行を、秋田だけの形で書いたもの */
const KNOWN_BAD_AKITA = [
  "041222hyoketsu.pdf 議案第209号 生活排水処理事業の運営に係る連携協約を締結する協議について: 公表 yes=42 no=1 / 記号 ○=42 ×=0",
  "060220hyoketsu.pdf 議案第10号 令和５年度急傾斜地崩壊対策事業に要する経費の一部負担の変更について: 公表 yes=40 no=1 / 記号 ○=40 ×=0",
  "h231202giketu.pdf 事提出認定第２号 平成２２年度秋田県歳入歳出決算の認定について: 公表 yes=42 no=1 / 記号 ○=42 ×=2",
  "h291222giketu.pdf 議案第214号 公の施設の指定管理者の指定について: 公表 yes=40 no=40 / 記号 ○=40 ×=0",
  "h291222giketu.pdf 請願第40号 県立高校図書館の充実を求める請願について: 公表 yes=40 no=40 / 記号 ○=40 ×=0",
];

test("#819 秋田: 公表数が記号の数と合わない行が、既知の 5 行だけである", async () => {
  const bad: string[] = [];
  ...
  assert.ok(withCounts >= 100, `counts のある行 ${withCounts} / 全 ${rows} 行`);
