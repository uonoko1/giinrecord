...
test("#819 y 方向: 公表された賛成者数・反対者数が、その行の記号帯の ○ / × の数と合う", async () => {
  ...
  // **#901 で秋田のフィクスチャを 6 → 9 本にしたので 414 → 418**（**+4 = `514.pdf` 3 行 ＋ `430.pdf` 1 行**。
  // **`05516hyoketsu.pdf` の 6 行は反対者数の欄が PDF で空なので `counts` が無く、判定の外**）
  assert.equal(judgeable, 580, "**母数**——判定できた行（#901 で秋田に 3 本足して 414 → 418、青森に 2 本足して 418 → 501、佐賀に 1 本足して 501 → 580）");
...
test("#819 y 方向: 記号帯を 1 行ずらすと落ちる（この検算が恒真でないこと）", async () => {
  ...
  // **#901 で 411 → 414**（**`514.pdf` の 3 行。`430.pdf` は 1 行なので回転の対象外**）
  assert.equal(judgeable, 576, "**母数**——回して判定できた行（1 行だけの本を除く）");
  // **#901 で 176 → 179**——**足した `514.pdf` の 3 行が 3 行とも落ちる**
  // （**増えた範囲の行も、この y の検算にちゃんと掛かっている**）
  // ## **#901 で 179 → 218**（**青森の `324`（51 行）と `323`（32 行）を足した**）
  // **足した 83 行のうち 39 行が落ち、44 行は落ちない**（**47%**）。
  // **`324` は 51 行中 35 行、`323` は 32 行中 18 行が全会一致（`×` が 0 個）である**（実測）。
  // **落ちた数が「反対票のある行の数」と一致しないのは、この検算が
  // 「隣の行の `counts`」と比べるからである**——**全会一致の行でも、隣が全会一致でなければ落ちる。**
  // **「39 行しか落ちない = 錨が弱い」と読まないこと**（#911。**回転で落ちる行数は
  // その本の反対票の分布で決まるので、錨の強さの尺度にならない**）。
  // **青森の x の錨（`aomori-votes-pdf.test.ts` の #901）は 51 行 2,448 対すべてで落ちる。**
    // ## **#901 で 218 → 245**（**佐賀の `3_95237_272348_up_pqtj45d7.pdf`（令和5年2月定例会・79 行）を足した**）
  // **足した 79 行のうち 27 行が落ち、52 行は落ちない**（**34%**）。
  // **佐賀の 79 行のうち 63 行が全会一致（`×` が 0 個）、16 行に反対票がある**（実測）。
  // **落ちた 27 行は「反対票のある 16 行」とは一致しない**——
  // **上の青森と同じ理由で、この検算は「隣の行の `counts`」と比べるからである**
  // （**全会一致の行でも、隣が全会一致でなければ落ちる。逆に、反対票があっても
  // 隣とたまたま同じ数なら落ちない**）。**27 > 16 なのはそのため。**
  // **「27 行しか落ちない = 錨が弱い」と読まないこと**（#911）。
  // **佐賀の x の錨（`saga-votes-pdf.test.ts` の「記号を 1 列回すと全対がずれる」）は
  // フィクスチャ 5 本 4,514 対すべてで落ちる**（#901 で列を 1 本ずらす変異を当てて確かめた）。
  assert.equal(caught, 245, "**1 行ずらして落ちた行**（#819 では 177 / #829 で 176 / #901 で 179 → 218 → 245）");
});
...
test("#819 秋田: 公表数が記号の数と合わない行（#829 で 5 行とも直り、0 行になった）", async () => {
  ...
  // **#901 でフィクスチャを 6 → 9 本にした**（`514.pdf` 3 行 / `430.pdf` 1 行 / `05516hyoketsu.pdf` 6 行）。
  // **`counts` があるのはそのうち 4 行**——**`05516hyoketsu.pdf` の 6 行は反対者数の欄が PDF で空**
  // （`akita/votes-pdf.ts` の docblock。**「空欄 = 0」と読むと 154 本で 180 行が嘘になる**）
  assert.equal(rows, 223, "**母数**——秋田のフィクスチャ 9 本の全行（#901 で 6 本 213 行から）");
  assert.equal(withCounts, 107, "**母数**——`counts` があり判定できた行（#901 で 103 → 107）");
