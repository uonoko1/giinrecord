...
test("#819 y 方向: 公表された賛成者数・反対者数が、その行の記号帯の ○ / × の数と合う", async () => {
  ...
  assert.equal(judgeable, 525, "**母数**——判定できた行（#901 で秋田 414 → 418、青森 418 → 501、滋賀 501 → 525）");
...
test("#819 y 方向: 記号帯を 1 行ずらすと落ちる（この検算が恒真でないこと）", async () => {
  ...
  assert.equal(judgeable, 521, "**母数**——回して判定できた行（1 行だけの本を除く。#901 の滋賀で 497 → 521）");
  ...
  assert.equal(caught, 235, "**1 行ずらして落ちた行**（#819 では 177 / #829 で 176 / #901 で 179 → 218 → 235）");
});
