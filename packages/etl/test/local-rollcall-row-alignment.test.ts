...
test("#819 y 方向: 公表された賛成者数・反対者数が、その行の記号帯の ○ / × の数と合う", async () => {
  ...
  assert.equal(judgeable, 604, "**母数**——判定できた行（#901 で秋田 414 → 418、青森 418 → 501、滋賀 501 → 525、佐賀 525 → 604）");
...
test("#819 y 方向: 記号帯を 1 行ずらすと落ちる（この検算が恒真でないこと）", async () => {
  ...
  assert.equal(judgeable, 600, "**母数**——回して判定できた行（1 行だけの本を除く。#901 の滋賀で 497 → 521、佐賀で 521 → 600）");
  ...
  // ## **#901 の滋賀で 218 → 235**（**`Kg744`（22 行）・`Kg749`（6 行）・`Kg835`（4 行）を足した**）
  // **足した 32 行のうち 17 行が落ちる**（**53%**）。**増えた範囲の行もこの y の検算に掛かっている。**
  // ## **#901 の佐賀で 235 → 262**（**回転の母数も 521 → 600**）
  // **足した 79 行のうち 27 行が落ちる（34%）。** **増えた範囲の行もこの y の検算に掛かっている。**
  assert.equal(caught, 262, "**1 行ずらして落ちた行**（#819 では 177 / #829 で 176 / #901 で 179 → 218 → 235 → 262）");
});
