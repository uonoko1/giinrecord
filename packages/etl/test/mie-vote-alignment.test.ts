...
test("#835 母数: フィクスチャ 26 本が読め、行と議員の数が実測どおり", () => {
  ...
  // **#999 でさらに 1 本**（`000073636.pdf`（平成25年6月））——
  // **並べ替えが丸め誤差でひっくり返るせいで読めていなかった本**である。
  // **左 8 列の見出しの 1 列目が `案等番号議` になり（正しくは `議案等番号`）、照合で止まっていた。**
  // **足した 26 行は検算A（賛成者数 ↔ ○ の数）と検算B（歴代議長）を両方通る。不明セルは 0 増**（43 のまま）。
  assert.equal(books.length, 26, `読めた本 ${books.length}`);
  ...
  assert.equal(rows, 524, `行 ${rows}`);       // #999 で 498 → 524（+26）
  assert.equal(cells, 25063, `セル ${cells}`); // #999 で 23763 → 25063（+1,300 = 26 行 × 50 人）
...
test("#835 検算A: 公表された賛成者数・反対者数が、その行の記号帯の ○ / × の数と合う", () => {
  ...
  assert.equal(t.judgeable, 481, `判定できた行 ${t.judgeable}（不明を含む ${t.skipped} 行は判定外）`); // #999 で 455 → 481（+26）
...
test("#835 検算B: `議` の列の議員が、県が公表している歴代議長と一致する", () => {
  ...
  assert.equal(t.judgeable, 524, `判定できた行 ${t.judgeable}（歴代議長の表に無い年月 ${t.skipped} 行は判定外）`); // #999 で 498 → 524（+26）
  assert.deepEqual(t.bad, [], `合わない行 ${t.bad.length} / ${t.judgeable}`);
});
...
test("#852 副議長の表で照合すると落ちる（#835 の担当者が実際に踏んだ罠）", () => {
  ...
  assert.equal(t.judgeable, 524, `副議長の表で判定できた行 ${t.judgeable}`); // #999 で 498 → 524（+26）
  ...
  // **#999 で 1 本ぶん 26 行が増えて 492 → 518 になった**（2026-09-24。`000073636.pdf` 平成25年6月）。
  // **増えた 26 行は 26 行とも落ちる**——**新しく読めた本は、議長の表と副議長の表を区別できている。**
  // （平成25年6月の議長は 山本勝（平成25.05 就任）、副議長は 前田剛志（同）。
  // **26 行すべてで `議` が立つのは 山本勝 の列**（実測。他の氏名は 1 行も無い）なので、
  // **副議長の表で照合すると 26 行とも合わない。** これが「どちらの表でも通る」なら、
  // **その本の `議` は外の事実に結ばれていないことになる**ので、ここの数で見張っている。）
  assert.equal(t.bad.length, 518, `副議長の表で落ちた行 ${t.bad.length} / ${t.judgeable}（実測 518）`);
