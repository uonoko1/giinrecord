...
test("#835 母数: フィクスチャ 22 本が読め、行と議員の数が実測どおり", () => {
  ...
  assert.equal(books.length, 22, `読めた本 ${books.length}`);
  ...
  assert.equal(rows, 491, `行 ${rows}`);
  assert.equal(cells, 23412, `セル ${cells}`);
...
test("#835 検算A: 公表された賛成者数・反対者数が、その行の記号帯の ○ / × の数と合う", () => {
  ...
  assert.equal(t.judgeable, 448, `判定できた行 ${t.judgeable}（不明を含む ${t.skipped} 行は判定外）`);
...
test("#835 検算B: `議` の列の議員が、県が公表している歴代議長と一致する", () => {
  ...
  assert.equal(t.judgeable, 491, `判定できた行 ${t.judgeable}（歴代議長の表に無い年月 ${t.skipped} 行は判定外）`);
  assert.deepEqual(t.bad, [], `合わない行 ${t.bad.length} / ${t.judgeable}`);
});
...
test("#835 x 方向: 記号のアイテムの中心と、置いた列の中心が半セル未満（列番号を通さずに測る）", async () => {
  ...
  assert.equal(pairs, 23369, `測った (記号, 列) の対 ${pairs}`);
...
test("#852 副議長の表で照合すると落ちる（#835 の担当者が実際に踏んだ罠）", () => {
  ...
  assert.equal(t.judgeable, 491, `副議長の表で判定できた行 ${t.judgeable}`);
  ...
  assert.equal(t.bad.length, 488, `副議長の表で落ちた行 ${t.bad.length} / ${t.judgeable}（実測 488）`);
  // **通ってしまう 3 行は #852 のときと同じ 3 行のまま**（#867 A-2 で 2 本増えても増えていないことを名指しで固定する）
