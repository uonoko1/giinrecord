...
const NOT_FULLY_PARSEABLE = new Map<string, RegExp>([
  ...
  // **#999 が「許容差の上界」の根拠として置いた本**（平成20年12月）。
  // **`公明党`(y=755.9585124287606) と `県政みらい`(y=755.9573199973106) の差 1.192431e-3 pt が、
  // 実データで「潰してはいけない本物の差」のいちばん小さいものに近い**（`d/h = 2.114e-4`、`tol` の 21.14 倍）。
  // **この本そのものは #999 とは別の理由で読めない**——
  // **1 行目の件名・日付・結果が揃わない**（`incomplete row`）。**#999 では直していない。**
  // **上界の検査は `test/pdf-row-order.test.ts` が座標のところまでで行う。**
  ["000073608.pdf", /incomplete row \(title\/date\/result\)/],
]);
...
test("#835 母数: フィクスチャ 27 本が読め、行と議員の数が実測どおり", () => {
  ...
  // **#999 でさらに 2 本**:
  // **`000073636.pdf`（平成25年6月、26 行）**——**並べ替えが丸め誤差でひっくり返るせいで読めていなかった本。**
  // **左 8 列の見出しの 1 列目が `案等番号議` になり（正しくは `議案等番号`）、照合で止まっていた。**
  // **`000835026.pdf`（令和元年5月、2 行）**——**誤差で x が逆順になる対を 3 つ持つ本**で、
  // **`test/pdf-row-order.test.ts` が下界の根拠として使う**（直す前は `"対日"` の順に出ていた）。
  // **足した 28 行は検算A（賛成者数 ↔ ○ の数）と検算B（歴代議長）を両方通る。不明セルは 0 増**（43 のまま）。
  // （**3 本目の `000073608.pdf` は上界の根拠だが、別の理由で読めないので `NOT_FULLY_PARSEABLE` に入れてある。**）
  assert.equal(books.length, 27, `読めた本 ${books.length}`);
  ...
  assert.equal(rows, 526, `行 ${rows}`);       // #999 で 498 → 526（+26 と +2）
  assert.equal(cells, 25165, `セル ${cells}`); // #999 で 23763 → 25165
...
test("#835 検算A: 公表された賛成者数・反対者数が、その行の記号帯の ○ / × の数と合う", () => {
  ...
  assert.equal(t.judgeable, 483, `判定できた行 ${t.judgeable}（不明を含む ${t.skipped} 行は判定外）`); // #999 で 455 → 483
...
test("#835 検算B: `議` の列の議員が、県が公表している歴代議長と一致する", () => {
  ...
  assert.equal(t.judgeable, 526, `判定できた行 ${t.judgeable}（歴代議長の表に無い年月 ${t.skipped} 行は判定外）`); // #999 で 498 → 526
  assert.deepEqual(t.bad, [], `合わない行 ${t.bad.length} / ${t.judgeable}`);
});
...
test("#835 x 方向: 記号のアイテムの中心と、置いた列の中心が半セル未満（列番号を通さずに測る）", async () => {
  ...
  assert.equal(pairs, 23476, `測った (記号, 列) の対 ${pairs}`); // #999 で 23374 → 23476
...
test("#852 副議長の表で照合すると落ちる（#835 の担当者が実際に踏んだ罠）", () => {
  ...
  assert.equal(t.judgeable, 526, `副議長の表で判定できた行 ${t.judgeable}`); // #999 で 498 → 526
  ...
  // **#999 で 2 本ぶん 28 行が増えて 492 → 520 になった**（2026-09-24）。
  // **増えた 28 行は 28 行とも落ちる**——**新しく読めた本は、議長の表と副議長の表を区別できている。**
  //   `000073636.pdf`（平成25年6月）26 行: 議長 山本勝（平成25.05 就任）/ 副議長 前田剛志（同）。
  //     **26 行すべてで `議` が立つのは 山本勝 の列**（実測。他の氏名は 1 行も無い）
  //   `000835026.pdf`（令和元年5月）2 行: 議長 中嶋年規（令和元.05 就任）/ 副議長 北川裕之（同）。
  //     **2 行とも `議` は 中嶋年規 の列**（実測）
  // **これが「どちらの表でも通る」なら、その本の `議` は外の事実に結ばれていないことになる**ので、
  // **ここの数で見張っている。**
  assert.equal(t.bad.length, 520, `副議長の表で落ちた行 ${t.bad.length} / ${t.judgeable}（実測 520）`);
