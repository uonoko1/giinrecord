...
# **通す結論はこれだけ**（#1021）。`REVIEW_VERDICTS` は「レビューの結論として認める語」の
# allowlist で、**そのうちマージを許すのは 1 つだけ**である。**2 つを混ぜないこと。**
REVIEW_APPROVE=マージしてよい
...
review_heading_re() {
  ...
  #
  # **この形は 2026-09-25 の 1 日で 3 度目である**（PO の指摘。#1002 の denylist /
  # #1008 の正規表現 / これ）。**「一覧が空になったとき何を主張するか」を決めていない検査は、
  # 空になった瞬間に黙る。** 一覧を足すときは、**空のときに何が起きるかを必ず書く。**
  # **この行だけを消す変異は等価**（一覧が空でない限り発火しないので、14 件とも緑）。
  # **一覧を空にする変異と組み合わせると 3 件落ちる**（実測）——
  # 「`## レビュー: ` だけでは通さない」テストが、そこを留めている。
...
assert_reviewed() {
  ...
  if [[ "$hit" == 1 ]]; then
    ...
    # **#1021 の穴（PO 自身のレビューで実測 2026-09-27）**: `REVIEW_APPROVE` が空だと
    # `"$found" != *""*` は**常に偽**になり、**この検査が黙って全部通す**。
    # 実測: 空にして判定「直してから」の PR #1026 に当てると、**レビュー検査を素通りして CI の待ちに進んだ**。
    # **`REVIEW_VERDICTS` には #1014 が同じ守りを入れているのに、こちらには無かった。**
    # **allowlist を痩せさせたら黙って開く**——#858 と同じ向きの失敗である。
    [[ -n "$REVIEW_APPROVE" ]] || die "REVIEW_APPROVE が空です（この道具の不具合）。マージしません。

       空だと「どの結論でも通す」になります（\$found != *\"\"* は常に偽）。
       .claude/agents/reviewer.md が規定している「通す結論」を REVIEW_APPROVE に入れてください。"
    # **#1021: 通すのは `マージしてよい` だけ。**
    # **この検査が答えるべきなのは「レビューされたか」ではなく「レビューが通したか」である。**
    # それまでこの道具は 3 つの結論を区別せず、どれか当たれば通していた。実測（2026-09-24）:
    ...
    if [[ "$found" != *"$REVIEW_APPROVE"* ]]; then
      die "PR #$PR のレビューの結論は「$found」です（コメント $total 件を見ました）。マージしません。

       **この道具が通すのは「$REVIEW_APPROVE」だけです。**

       やること:
         - 指摘を直す → もう一度レビュアーを立てる → 新しい結論を貼る → この道具を流す
         - 指摘が誤っている、または別 PBI にすると判断したなら、**その判断を PR に書いてから**
           レビュアーに再判定してもらう。**PO が代わりに結論を書き換えないこと。**

       **レビュアーが「直してから」と書いたものを PO の判断で押すのは、レビューを無効にします。**
       $URL"
    fi
    return 0
  fi
