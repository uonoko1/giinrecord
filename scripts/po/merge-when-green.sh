...
# **その 3 語のどれかを含むコメントが 1 件以上あること**を要求する。
...
#
# **denylist ではなく allowlist**（#858 と同じ向き）: 3 語のどれも無ければ止める。
