...
#
# **この検査が見ないこと（測って確かめた）**:
#   **フラグの「値」は見ない。綴りだけ。** `gh secret set … --body -` は
#   **`--body` が実在するので通る**（実測: 変異 M12 が生き残った）。だがこれも誤りで、
#   **リテラルの `-` が secret として保存される**（#799 が `--no-store` で確認）。
#   **そちらは `scripts/ci/test/human-tasks.test.sh` の `gh` スタブが弾く**
#   （実測: 同じ変異を人間のテストに当てると **4 本落ちる**）。2 つで別々のものを見ている。
#   短いフラグ（`-b` / `-F`）も見ない——1 文字は誤検出が高くつくので出さない側に倒した。
#
