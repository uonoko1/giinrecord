...
### 証拠 D — git log（**採用の記録が書かれた日を確定できる**）

`design/wireframes/` に関わるコミットは 2 つだけである:

```
$ git log --format='%h %ad %s' --date=short -- design/wireframes/
bb35cf4f 2026-08-23 docs: mandatory security review checklist (OSS, shared VPS) (#131)
01d0bd13 2026-08-22 chore: sprint 0 scaffold — ...
```

内訳:

| コミット | 日付 | design/wireframes/ に入れたもの |
|---|---|---|
| `01d0bd13` | 2026-08-22 | **`.dc.html` 20 件**＋`canvas.json`（artboard 20・`page-logo` 無し） |
| `bb35cf4f` | 2026-08-23 | **`Logo.dc.html` 1 件**＋`canvas.json` の更新（`page-logo` と artboard 1 件を追加） |

**採用の記録は 2026-08-22 に書かれている。断定できる。** 追加時点の `canvas.json` を直接読める:

```
$ git show 01d0bd13:design/wireframes/canvas.json
```

**その時点で既に**「採用：A 台帳 ＋ C ダーク」「不採用案」のページ名と、
`採用（2026-08-22）：A 台帳をライト、C 夜の台帳をダークモードとして採用。` の注記が入っていた。
**決定日（2026-08-22、ユーザー承認）と記載日が一致する。**
つまり注記は後付けの回想ではなく、**決定と同じ日に書かれた記録**である。

**注意**: `.dc.html` 21 件のうち **20 件が `01d0bd13`、`Logo.dc.html` だけが `bb35cf4f`** である。
「21 件すべてが 1 コミット」ではない。**§6 で `AltNewspaper` / `AltWhitePaper` の前後関係を
「時系列では区別できない」としているのは、この 20 件の中での話である。**

コミットメッセージ自体に採用/不採用の記述は無いので、**分類そのものは証拠 A〜C による。**
git log が確定させるのは「**いつ書かれたか**」である。
...
### ここが最も取り違えやすい: **ワイヤーフレームは A を推していたが、選ばれたのは D**

`Logo.dc.html` は各案を評価したうえで、**自分の推奨をはっきり書いている**:

> 評価：A は「判」というサイト内の文法と繋がり、漢字1字で日本のサービスだと分かる。（…）
> **推し：A（判）。次点 D。** 決まったら SVG／PNG（16・32・180・512）・OGP 画像を生成して…

**しかし実装されたのは D である**（`logo.svg` のコメントとテストが示すとおり）。
**推奨は「次点」のほうが採用された。**

> **これは #462 の趣旨そのものである。**
> **「ワイヤーフレームがこう推している」は「こう決まった」ではない。**
> ワイヤーフレームは提案であって決定ではなく、**決定は実装と `canvas.json` にある。**
> 実際にこの README の最初の草稿は、「推し：A」を読んで**ロゴを「A 案採用」と誤記した。**
> 実装 (`logo.svg`) を見て初めて誤りに気づいている。**次の人は同じ間違いをしないこと。**

> **注意 1: この「ロゴ案 A〜D」の A〜D は、色の方向 A〜E とも `DirectionB/C` とも無関係である。**
> 同じ文字を 3 通りの別の意味で使っているので、読むときに取り違えやすい。
> **色の方向は A が採用だが、ロゴは A が推されたうえで不採用、D が採用**という、
> 最も紛らわしい組み合わせになっている。
...
- **`AltNewspaper` / `AltWhitePaper` が A〜E の**前**の探索か、並行の探索か**。
  この 2 件は A〜E と**同じ `01d0bd13` に同時追加**されているので、**git では前後を区別できない。**
  「初期別案」という title を根拠に「前」としたが、これは title の語だけが根拠である。

### 分からない

- **`Logo.dc.html` が推した A ではなく D が選ばれた理由。** 結果は確実だが（実装が D）、
  **判断の理由がどこにも無い。** 色の方向 B/D/E には `canvas.json` に不採用の理由が
  書き残されているのに、**ロゴには推奨を覆した記録が無い。**
  「A/B/C を落とした理由」ではなく「**推された A を採らなかった理由**」が欠けている、
  というのが正確な言い方である。
- **`canvas.json` の注記を誰が書いたか。** **いつ書かれたかは確定している**（§1 証拠 D。
  2026-08-22 の `01d0bd13` 時点で既に入っていた）が、**書いた人は git の author 以上には分からない。**
...
## 8. 機械で守っていること／**守っていないこと**

`apps/web/app/lib/wireframe-refs.test.ts` が守るのは、次の 3 つ**だけ**である:
...
> **docs の参照先が採用案かどうかは、機械では守っていない。人が README を読むこと。**

具体的に、次はすべて**テストが緑のまま通る**（レビューの指摘を受けて、実際に変異を入れて再現した。
4 件とも `Tests 6 passed (6)` のまま）:

- **`typography-judgment.md` の参照先を、実在する別のファイルに差し替える。**
  `Votes.dc.html` → `HomeE.dc.html`、`Search.dc.html` → `BoldSearch.dc.html` に
  書き換えても**落ちない。** どちらも実在し README にも載っているためである。
  **`HomeE` は #456 が懸念した不採用案、`BoldSearch` は §3 で「取り違えるな」と
  警告している `Search` の不採用版**であり、**この README が防ごうとしている失敗そのものが素通りする。**
- **`canvas.json` の artboard を 1 つ消す。**
- **`canvas.json` で `Votes.dc.html` を `page-alt`（不採用）へ移す。**
  採用の記録そのものを書き換えても落ちない。
- **README に実在しない `NotReal.dc.html` の行を足す**（README → ディスクの向きの検査が無い）。

**「採用/不採用」の文言はあえてテストしていない。** 判断は人がするもので、
文字列一致で縛っても README の文面を固定するだけで、正しさは何も守れないためである。

**だからこそ、ここに書いておく。**「機械が守っている」と書いてあることは、
読む人が自分で確かめなくなる。**このリストは、テストを信用してはいけない範囲の一覧である。**
ワイヤーフレームを根拠に何かを決めるときは、**必ずこの README の §2 / §3 を読んで
採用案かどうかを自分で確かめること。**
