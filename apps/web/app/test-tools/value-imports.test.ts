...
describe("entriesReaching: 入口ごとに「違反に辿り着くか」だけを答える（#514）", () => {
   ...
   * `visited` を殺すと `stack` が無限に伸びる。**同期の無限ループなので
   * `vitest.config.ts` の `testTimeout` は効かない**（イベントループごとブロックする）。
   * 緑にはならないので検出はできているが、**メッセージは出ない**。
   *
   * **どう終わるかは環境で変わる。** レビュアーの実測では
   * **656 秒走ってメモリは 330MB**（23GB 環境）で、**OOM には届かなかった**。
   * `ci.yml` に `timeout-minutes` が無いので、**ランナー既定の 6 時間を食う**（#556）。
   * **「OOM で死ぬから検出できている」と読まないこと。**
   *
