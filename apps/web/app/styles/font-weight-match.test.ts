...
describe("ウェイトの判定そのもの（#506）", () => {
  ...
  describe("parseFontShorthand: font ショートハンドの size / weight / family（#517）", () => {
    ...
    it("システムフォント指定 6 語は『システム指定』として返す（サイズが読めたと誤読しない）", () => {
      // **#567 が入って期待値が変わった。** もとは `undefined`（＝読めない）を期待していたが、
      // #567 が **`"system"`（＝家族が自前でない、と読み取れた）**に区別した。
      // **ここで釘打ちたいのは「`small-caption` の `small` に当たってサイズが読めたことにならない」**
      // ことなので、`"system"` を期待する形でも目的は変わらない。
      // **`undefined` のままにすると、rebase は無衝突で通ったうえでこの 1 行だけが赤くなる**（PO が実測）。
      const wrong = SYSTEM_FONTS.filter((v) => parseFontShorthand(v) !== "system").map((v) => `${v}: ${JSON.stringify(parseFontShorthand(v))}`);
