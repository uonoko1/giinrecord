...
const TARGET_LINK_RULES = {
  ...
  散文: {
     ...
     * ## 実害はあった（「まだ起きていない」だけ）
     *
     * `apps/web/app` の `.css` 7 ファイル・セレクタ 284 件を走査すると、
     * **`a$` だけが当たるセレクタが 8 件**ある（`.members-item__kana` / `.list__meta` など。
     * 全部クラス名で、`a` 要素は 0 件）。**いまはどれも `.note` の中に無い**ので偽陽性は出ていないが、
...
describe("forbiddenTargetFixes: 例外に当たるリンクを「直した跡」の判定（#506）", () => {
  ...
  const 散文の非違反: Record<string, string> = {
     ...
     *
     * `selectorLink` に `a$` の枝があると、この 3 つを**リンクだと誤って呼ぶ**。
     * 本番 CSS に現に `__kana` / `__meta` で終わるクラスが **8 件**ある（下の `it` で数える）ので、
     * そのどれかが `.note` の中に置かれた日に**偽陽性**になる。
     */
