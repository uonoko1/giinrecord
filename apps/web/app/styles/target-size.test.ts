...
const TARGET_LINK_RULES = {
  ...
  散文: {
     ...
     * ## 実害はあった（「まだ起きていない」だけ）
     *
     * 起票時（2026-09-06）の実測——`apps/web/app` 配下の `.css` を再帰的に集め、
     * `,` で割ったセレクタを数えた（**7 ファイル / 284 件**）:
     *
     *     `\ba\b` だけが当たる   3 件   `.rollcalls-item a:hover` など。**全部 `a` 要素**
     *     `a$` だけが当たる       8 件   `.members-item__kana` `.list__meta` など。**`a` 要素は 0 件**
     *
     * **いまはどれも `.note` の中に無い**ので偽陽性は出ていないが、
     * そのどれかが `.note` の中に書かれた日に、**WCAG を満たしている実装を違反と呼ぶ**。
     * #413 で「110 箇所が違反」と誤って起票したのと同じ誤り方になる。
     * **この 284 / 3 / 8 は起票時の値で、CSS が変われば動く。**
