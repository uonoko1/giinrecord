 ...
 * **`counts` が出ているのに結果が空である理由**は、**採決ページに毎回ではなく
 * `/about` か `/coverage` に一度だけ書く**（別 PBI）。
 *
 * **ここにあった「174 ページ」は誤りだった**（PO が 2026-09-24 に撤回）。
 * `resultAbsent` を持つ採決は**実測 3 件**（すべて滋賀 `pref-25`）で、
 * 本番の採決ページ**母数 4,599 本を全数 curl して確かめた**。
 * 誤った原因は #993 の型そのもの——`'"resultAbsent":true'`（空白なし）で grep して
 * **0 件**が返り、本番の `data/` が空白付きで整形されていることに気づかなかった。
 * **同じ値の別の綴りを先に潰していれば防げた。**
 *
