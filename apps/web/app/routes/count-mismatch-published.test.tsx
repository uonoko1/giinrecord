...
describe("#826 本番データ: 記号の数と公表値の突き合わせが画面に出る", () => {
  ...
  it("本番の母数と食い違いの件数を固定する（測定の固定。変わったら数え直すこと）", async () => {
    ...
    expect({
      ...
      //
      // ## **#901 で 1,369 → 1,737 / 1,030 → 1,355 / 5 → 48 に動いた**（2026-09-20）
      ...
      // **`noCounts` 334 は動いていない**（三重は 733 / 733 本すべてに `counts` がある）。
      // **食い違いは 0 のまま**——**増えた 325 本も公表値と数が合っている。**
    }).toEqual({ rows: 1737, checked: 1355, noCounts: 334, unreadableCells: 48, mismatches: 0 });
  });
  ...
  it("/coverage に、本番の母数（1,355 件）と食い違い（0 件）と未突合の内訳が出る", async () => {
    ...
    // **突き合わせなかった 334 件と 48 件の内訳も出す**（黙って母数から外さない）
