...
describe("#951 本番データ: 会期ごとの名簿の写り方が画面に出る", () => {
   ...
   *
   * **実測 2026-09-25**: **11 議会 / 119 会期。`seatsChanged` の最大は 5（滋賀。#901 で 2 → 19 会期）で、
   * 印（10 以上）は 0 件。** **#901 が会期を広げると、ここが動いて数え直しを強制する。**
   *
   * ## ⚠ **この docblock は `96 会期` のまま 23 会期ぶん腐っていた**（#1033 の 3 巡目レビューが見つけた）
   *
   * **下の `toEqual` は `sessions: 119` を要求していたので、assert と docblock が食い違っていた。**
   * **#1007 が `local-session-roster-coverage.test.ts` で直したのと寸分違わぬ形が、
   * 「本番 `data/` から数え直しているので安全」と名指しして対象外にした所に在った。**
   * **`packages/etl/test/seats-changed-line-numbers.test.ts` が
   * 「`11 議会 … N 会期` と書いた所すべての N が実測と一致する」を見るようにしたので、
   * 次に腐ったら落ちる。**
   */
  it("本番の母数と印の件数を固定する（119 会期・最大 5・印 0 件）", async () => {
    const metas = await realLocalMetas();
    const local = metas.filter((m) => m.assemblyId.startsWith("pref-"));
