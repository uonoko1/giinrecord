 ...
 */
import { assemblyPaths, localRollCallPaths, memberPaths, rollCallPaths } from "./data-files";
...
export async function prerenderPaths(dataDir: string): Promise<string[]> {
  const [members, assemblies, rollcalls, localRollcalls] = await Promise.all([memberPaths(dataDir), assemblyPaths(dataDir), rollCallPaths(dataDir), localRollCallPaths(dataDir)]);
  return [...STATIC_PATHS, ...members, ...assemblies, ...rollcalls, ...localRollcalls];
}
