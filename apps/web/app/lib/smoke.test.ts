...
describe("checkBuild", () => {
  const staticOnly = fakeBuild({
    ...
    "coverage/index.html": html(["/", "/assets/entry-abc123.js"]),
...
describe("checkSitemap", () => {
  ...
  it("絶対 URL でも相対パスでも、全 <loc> がビルドに存在すれば OK", () => {
    const b = fakeBuild({
      ...pages,
      "sitemap.xml": sitemap(["https://example.test/", "https://example.test/about", "/coverage", "/terms", "/privacy", "/members", "/members/m_1", ...assemblyLocs]),
    });
    const r = checkSitemap(b, { memberIds: ["m_1"], rollCalls: null });
    expect(r.failures).toEqual([]);
    expect(r.checkedUrls).toBe(10);
  });

  it("存在しないページを指す <loc> は失敗", () => {
    const b = fakeBuild({ ...pages, "sitemap.xml": sitemap(["/", "/about", "/coverage", "/terms", "/privacy", "/members", "/members/m_1", ...assemblyLocs, "/members/gone"]) });
  ...
  it("data/ が約束したページが sitemap に無ければ失敗（全議員・全採決・静的ページ）", () => {
    const b = fakeBuild({ ...pages, "sitemap.xml": sitemap(["/", "/about", "/coverage", "/terms", "/privacy", "/members", ...assemblyLocs]) });
  ...
  it("assemblies/index.json の全議会のページが sitemap に要る（#158）。無いときは国会の2議会", () => {
    const b = fakeBuild({ ...pages, "assemblies/pref-04/index.html": "", "sitemap.xml": sitemap(["/", "/about", "/coverage", "/terms", "/privacy", "/members", "/members/m_1", ...assemblyLocs]) });
