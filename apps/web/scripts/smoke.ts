 ...
 * and that no page or fonts/fonts.css loads a resource from another origin (self-hosted fonts, #168),
...
async function listBuild(root: string): Promise<BuildFiles> {
  ...
  for (const e of entries) {
    ...
    const needsContent = rel.endsWith(".html") || rel === "sitemap.xml" || rel.startsWith(`${DISTRICTS_DATA_PATH}/`) || rel === "data/meta.json" || rel === FONTS_CSS;
...
async function fetchAll(origin: string, urls: string[]): Promise<Map<string, ServedResponse>> {
  ...
  for (const url of urls) {
    try {
      ...
      // pages: read the HTML to check for off-site resources (#168); everything else: headers + status only,
      // do not download the 5 MB archive
      const isHtml = (headers["content-type"] ?? "").startsWith("text/html");
      const body = isHtml ? await r.text() : "";
      if (!isHtml) await r.body?.cancel();
