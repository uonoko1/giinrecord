...
**出どころ**: この決定は `docs/` ではなく**プロジェクトのデザインメモ**に残っている
（「デザイン方向『台帳』」の "How to apply"）。**`docs/` 配下には書体の割り当てを定めた文書が無い**
（`docs/ops/fonts.md` は「Shippori Mincho（見出し）と BIZ UDPGothic（本文）」を配信の対象として
挙げるだけで、**どの要素に使うかは書いていない**）。
**リポジトリの中で根拠にできるのは、次のワイヤーフレームである。**
...
- **(a) 現状（ゴシック 700）と (b) ゴシック 400 の差は、ほとんど分からない。**
  **画面に見えている範囲では改行位置も行数も同一**で、変わるのは線の太さだけ。
  <!-- 訂正 2026-09-05（#456 の実測）: 一覧 200 行**すべて**を測ると、**197 行は 1px も動かないが、
       3 行（48 字の長い議案名）が 3 行→2 行になる**。ここで「同一」と書いたのは
       **画面に映っていた範囲だけ**を見た記述だった。はみ出し・切れは前後とも 0 件で、
       3 行目に「付）」だけが残っていたものが解消するので、悪くなる方向ではない。 -->
...
一方 `/rollcalls` の (b) の 10.2% は**ほぼ全ての行が 1px も動いていない**ので、
**ほぼ純粋に線の太さだけの差**である。**同じ 10〜14% でも意味が違う。**
（訂正 2026-09-05: 200 行中 197 行が不動、3 行だけ 3 行→2 行に縮む。#456 の実測。）
...
そして (a) と (b) は**200 行中 197 行が 1px も動かず、線の太さしか変わらない**（§2）。
残る 3 行は長い議案名が 3 行→2 行に縮むだけで、はみ出し・切れは 0 件。
**失う見た目が無い**（正確には「設計が指定していたほうに戻る」）。
（訂正 2026-09-05: 当初「行数も同一」と書いたのは画面に見えていた範囲だけの観察だった。#456 の実測で訂正。）
