...
なお `inline.md` は、フォント読み込みを待たずに測ると `/coverage` の議会名 `h3` 7箇所を
「13px で違反」と誤って数えると書いているが、**この 7箇所は 46 にも 41 にも入っていない**
（どちらでも対象外）ので、+5 の説明にはならない。**+5 の原因は特定できていない。**

**教訓は数の大小ではなく、「数えた数は測るまで確定しない」ということ。**
