...
**取得したのは HTML 5 本・PDF 3 本だけ**（賛否 PDF は 40 本あるが、**3 本しか取っていない**）。
...
1. **`櫛引ユキ子` の「櫛」は `w=0.0` で、列の中心から外れる。**
...
3. **凡例が PDF の中に文章で書かれており、PDF ごとに読む必要がある**（島根と同じ方針）。
4. **左側に 議案番号／件名／表決方法／議決結果、右側に 賛成者数／反対者数 の欄がある**——
   島根 `votes-pdf.ts` の `Columns` とほぼ同じ構え。
5. **罫線はあるが、行の高さと一致しない**（件名が 2 行の議案がある）。
   **島根と同じく「罫線ではなく文字の位置で行を決める」必要がある。**
