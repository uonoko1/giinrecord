...
**なぜ道具に入れたか**: **2026-09-23〜24 に、33 本の PR が 1 件もレビューされずにマージされた**
（実測 `mergedAt`: **PO 31 本 / bot 2 本**。09-23 が 21 本、09-24 が 12 本）**。**
...
33 本のときの PO は「自分で検算したから十分だ」と判断していた。
...
**ただし「誤検出 0」は直近 60 PR でのみ真です。既知の穴があります**（実測。#1009 のレビューが発見）:

**全履歴（PR 634 本のコメント 217 件）に当てると、通る 9 件のうち 3 件が誤検出**です。

| PR | 当たった語 | 実体 |
|---|---|---|
| #224 | 「レビューありがとうございます」＋「『賛成会派／**反対**会派』に…」 | **担当者の返答** |
| #457 | 「## レビュー3点に対応しました」＋「落ちたときの対応が正**反対**になる」 | **担当者の返答** |
| #566 | 「…**賛成者数**／**反対者数**／**表決方法**…」 | 列仕様の説明 |

**3 件とも「担当者がレビューに返答したコメント」です。**
**担当者が「レビューありがとうございます、直しました」と返信し、その中で採決データの
「賛成／反対」や「正反対」に触れた瞬間、レビュアーが一度も走っていなくても検査が通ります。**

**直す方向（別 PBI）**: 報告の見出しを要求する（`^##\s*(敵対的)?レビュー`）。
**ただしそれだけでは足りません**——実測で、**本物の報告 6 件のうち #496 は見出しが無い**
（「**承認します。マージします。**」で始まる）。
**先に `.claude/agents/reviewer.md` に「報告の 1 行目は `## レビュー: <結論>`」を規定する**必要があります。
**現状 `reviewer.md` には報告の綴りの指示が 1 行もありません**（実測: `grep -c レビュー` → **0 件**。
「レビュ**アー**」は役割の説明として 3 か所あるが、報告の書き方ではない）。
...
| **同じ検査を全履歴に当てた結果** | PR 634 本・コメント 217 件で **通る 9 件**（本物 6 / **誤検出 3**） |
