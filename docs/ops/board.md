...
# ここで reviewer サブエージェントを立て、その報告を PR のコメントに貼る（#1006・#1010）
# 報告の 1 行目は  ## レビュー: <マージしてよい|直してから|反対>  でなければ通らない
...
**何を見ているか（#1010 で変わった）**: **PR のコメントの 1 行目がこの形であること。**

```
## レビュー: <結論>
```

`<結論>` は `マージしてよい` / `直してから` / `反対` のどれか 1 つ。
**この綴りは `.claude/agents/reviewer.md` の「報告（日本語）」が規定しています**
——**道具が語を推測するのではなく、仕様が綴りを決める形**です。

### #1006 の検査には欠陥がありました（#1010 で直した）

**#1006 の検査は「レビュー」という語を必須にしていましたが、`reviewer.md` はその語を
1 度も規定していませんでした**（実測 `grep -c 'レビュー[^ア]' .claude/agents/reviewer.md` → **0 件**。
「レビュ**アー**」は役割の説明で、報告の書き方ではない）。
**仕様が何も決めず、道具が語を推測していた**ので、**仕様どおりに書いた報告が弾かれ、
意味の無い 6 文字が通りました**（実測。#1010 で再現し、直した後に再測）:

| 報告の本文 | #1006 | **#1010 の後** |
|---|---|---|
| `## 結論: マージしてよい。変異 3 件を当て直して全部落ちた。` | 止まる | 止まる（`reviewer.md` がこの綴りを規定していないため） |
| `結論を先に: マージしてよい。指摘は 0 件。` | 止まる | 止まる（同上） |
| **`レビュー反対`（6 文字）** | **マージ** | **止まる** |
| `## レビュー: マージしてよい。変異 3 件を当て直して全部落ちた。` | マージ | マージ |

### #1009 が書いた数字のうち 2 つは誤りでした（#1010 で数え直した）
...
| PR | 223 | 224 | 226 | 259 | 429 | **496** | 1000 | 457 | **566** |
|---|---|---|---|---|---|---|---|---|---|
| 一致行数 | 1 | 1 | 1 | 1 | 1 | **2** | 1 | 1 | **0** |

  **1 行目が見出しでないのは本当ですが、本文には在ります。**

**この誤りは 3 人を経由しました**（前回のレビュアー → PO → 担当者）。
**3 人目のレビュアーが全員を正しました。レビュアーの指摘も、疑わずに受け取ってはいけない。**

### なぜ「結論の語をどこかに含む」をやめたか（実測。#1010 の核心）

**本物 8 件のうち 4 件（#223 #224 #226 #259）は、3 つの結論の語をどれも書いていません。**
`REQUEST_CHANGES` / `APPROVE` と書いていました。
**その 4 件が通っていたのは、採決データの語「反対」がたまたま同じコメントに在ったから**です:

| PR | 当たっていた「反対」 |
|---|---|
| #223 | 「投票総数　215　　　賛成票　199　　　**反対**票　16」 |
| #226 | 「列34「成相安信」は請願第30号で唯一の**反対**者（●）」 |
| #259 | 本文中の採決の説明 |
| #224 | **本物の報告の側には結論の語が 1 つも無く**、担当者の返答の「賛成会派／**反対**会派」で通っていた |

**`REVIEW_VERDICTS` から `反対` を外すと、本物 8 件のうち 4 件が落ちます。**
**検査は採決データの語に支えられていました。それは歯止めではなく偶然です。**

### なぜ方向 2（`結論` も許す allowlist）を採らなかったか（実測）

Issue が挙げたもう 1 つの案は、`レビュー` だけでなく `結論` も許すことでした。
**全履歴に当てると、通る PR が 10 件 → 12 件に増え、増えた 2 件は両方とも誤検出**でした
（**#622**「## PO: 追試しました」/ **#945**「## PO が確かめた」——どちらも PO 自身の検算）。
**本物 8 件は 8 件とも既に「レビュー」を含むので、得るものが 0 件で、誤検出だけが 2 → 4 件に増えます。**
**方向 2 は、測ると厳密に悪くなります。**

### なぜ「1 行目」なのか（実測）

**担当者の返答と見分けるため**です。`^##\s*(敵対的)?レビュー` を全履歴に当てると **36 件**
当たりますが、**そのうち 6 件は担当者の返答**でした:
...
**「レビュー」の語だけでは、レビューと「レビューへの返答」が区別できません。**
**「レビュー」の直後に結論の語が来ること**が、その区別になります
——**返答は「レビュー対応」「レビュー指摘」と続き、結論の語では続きません。**

### この形の代償（隠さずに書く）

**新しい規則を全履歴に当てると、通るのは 2 件だけ**（**#1000 と #1009**。**誤検出 0 件**）。
**本物 8 件のうち 6 件は通りません**——**綴りを決める仕様が無かったので、6 人が 6 通りに
書いたから**です。**それが #1010 そのものです。**

| PR | 1 行目 | なぜ通らないか |
|---|---|---|
| #223 | `## 敵対的レビュー: PR #223（…）` | 結論でなく件名 |
...
**落ちる 6 件について、やることはありません。**
**6 件は既にマージ済み**で、**この歯止めは「これからマージする PR」にしか効きません**
（`merge-when-green.sh` は OPEN な PR にしか使えず、MERGED なら手順 1 で止まります）。
**上の表は「もし当てたら」という遡及の計算**であって、**直さなければならない負債ではありません。**

**そして、いちばん誤解されやすい点をはっきり書いておきます**:

> **過去 6 件が落ちるのは、過去に規定が無かったからであって、その 6 件が悪いのではありません。**

**6 件はどれも中身のある敵対的レビューです**（#259 は `REQUEST_CHANGES` を出し、
#429 は「1点直してから」と言い、#496 は PO の数字の誤りを名指しで訂正しています）。
**綴りを決める仕様が無かったので、6 人が 6 通りに書いた。それが #1010 の中身そのものです。**
**`reviewer.md` に綴りが入ったので、今日以降のレビューは揃います。**

**過去の報告に道具を合わせません。** 合わせると、上で見たとおり
**「採決データの語『反対』に支えられた検査」に戻ります。**

**言い換えれば、この検査は「2026-09-25 より前の書き方」を受け付けません。**
**古い形で報告が貼られたら、この道具は止まります。** そのとき PO がやることは 2 つ:
**レビュアーに規定の形で貼り直してもらう**か、**`--no-review <理由>` で理由を残して通す**。
**どちらも記録が残ります。黙って通る道はありません。**

**良し悪しは判定しません**（`pr-closes.sh` と同じ——機械が判断できないことを機械に判断させない）。
**「マージしてよい」だけを探していない**のは意図です: それだと
**「直してから」と言われて直し、再レビューを受けた PR が「レビューが無い」と同じ扱い**になります。
見ているのは「レビュアーが走ったか」であって「レビュアーが許したか」ではありません
（だから当たった結論を必ず読み上げます——**「直してから」のまま押していないか、PO が読む**）。
...
| **#1006 の検査を全履歴に当てた結果** | PR 634 本・コメント 218 件で **通る 10 件**（本物 8 / **誤検出 2**） |
| **#1010 の検査を全履歴に当てた結果** | 同じ母数で **通る 2 件**（#1000 #1009。**本物 2 / 誤検出 0**） |
