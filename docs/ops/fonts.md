...
| ウェイト | スライス数 | 合計 |
|---|---|---|
| **Shippori Mincho 700（サブセット）** | **1** | **194.6KB**（979 字。#477。**#759 で秋田の 41 名が入り 943 → 979 字 / 186.5 → 194.6KB**） |
...
明朝700 はページによらず 1 面 194.6KB を引く（#477 の実測時は 186.5KB / 943 字。**議会が増えると字が増える**）。**残り 3 ウェイトは従来どおりの分割**である。
...
**#759（秋田県議会 pref-05、41 名）で 943 → 979 字になった**（2026-09-13 に作り直し。
**増えた 8 字は `丈憂湯瓜薄邉鉱館`**——**秋田の議員の氏名と選挙区**）。
**`pnpm test` の `font-subset-coverage.test.ts` が落ちて気づいた**——
**議会を足したら、この検査が必ず教えてくれる。**
...
**明朝700 は、使うページなら必ず 1 面 194.6 KB を引く**（現状は 5〜83 面で 63〜2,157 KB と振れる）。
...
> 明朝700 を 5〜7 字しか使わないのに 194.6 KB を引くことになる。
