...
| ウェイト | スライス数 | 合計 |
|---|---|---|
| **Shippori Mincho 700（サブセット）** | **1** | **198.9KB**（1,004 字。#477。**#901 の滋賀で 979 → 1,004 字 / 194.6 → 198.9KB**） |
...
明朝700 はページによらず 1 面 198.9KB を引く（#477 の実測時は 186.5KB / 943 字。**議会が増えると字が増える**）。**残り 3 ウェイトは従来どおりの分割**である。
...
**#901（滋賀を `--sessions 19` に。#741 pref-25）で 979 → 1,004 字になった**（2026-09-23 に作り直し。
...
**明朝700 は、使うページなら必ず 1 面 198.9 KB を引く**（現状は 5〜83 面で 63〜2,157 KB と振れる）。
...
> 明朝700 を 5〜7 字しか使わないのに 198.9 KB を引くことになる。
