...
> ## **議会を足したときだけではない——`--sessions` を広げたときも落ちる**（#901）
>
> **#901 で佐賀を 2 → 13 会期にしたら、`△`（U+25B3）で落ちた**（979 → 1,004 字 / 194.6 → 198.9 KB）。
> **`△` は佐賀の「退席」の記号**で、**広げる前の 23 採決には 1 つも無く、13 会期で 13 票ぶん出た。**
> **議員が増えたのではない**——**同じ 37 人の、それまで出ていなかった表決の記号である。**
>
