...
| ウェイト | スライス数 | 合計 |
|---|---|---|
| **Shippori Mincho 700（サブセット）** | **1** | **199.1KB**（1,006 字。#477。**#901 の滋賀で 979 → 1,006 字 / 194.6 → 199.1KB**） |
...
明朝700 はページによらず 1 面 199.1KB を引く（#477 の実測時は 186.5KB / 943 字。**議会が増えると字が増える**）。**残り 3 ウェイトは従来どおりの分割**である。
...
**#901（滋賀を `--sessions 19` に。#741 pref-25）で 979 → 1,006 字になった**（2026-09-23 に作り直し。
**増えた 25 字は `―`（U+2015）・`■`・`倭` `動` `号` `否` `字` `未` `氏` `突` `簿` `載` など**——
**`―` は滋賀の賛否 PDF のセルの記号で、凡例が引けない会期に 9 個出る**）。
...
> #### **議会を足したときだけではない——`--sessions` を広げたときも落ちる**（#901。**2 県が続けて踏んだ**）
>
> | 県 | 足りなかった字 | 何の記号か | 字数 |
> |---|---|---|---|
> | **島根**（#967） | **`−` U+2212**（MINUS SIGN） | PDF の凡例に在る | 990 → **1,003** |
> | **佐賀**（#968） | **`△` U+25B3** | **退席** | 1,003 → **1,004** |
>
> **どちらも議員が増えたのではない**——**同じ議員の、それまで出ていなかった表決の記号である。**
> **佐賀の `△` は広げる前の 23 採決には 1 つも無く、13 会期で 13 票ぶん出た。**
>
> **表決の記号は凡例ごとに県で違う**（`△` 退席 / `除` 除斥 / `欠` 欠席 / `−` …）ので、
> **会期を広げると「その県で初めて使われた記号」が出うる。**
> **一次資料の字を `-` や `^` の近い字に寄せて回避しないこと**——
> **寄せると凡例が引けなくなる県が出る**（#901 奈良の `―` U+2015 ↔ `-` U+002D。`local-assemblies.ts`）。
> **サブセットを作り直すのが正しい。**
>
> **#901 で県を広げる担当者は、`pnpm test` まで通してから PR を出すこと**——
> **`pnpm --filter @seiji-kiroku/etl test` だけでは気づかない**（この検査は `apps/web` にある）。
>
> #### ⚠ **2 県が同時に広げると、このファイルが衝突する**（#967 ↔ #968 で実際に起きた）
>
> **`.woff2` はバイナリなので、3-way マージの結果を信用してはいけない。**
> **`git checkout --ours` / `--theirs` で片付けると、採らなかった側の字が消えて本番で豆腐（□）になる。**
>
> **正しい直し方**: **先にマージされた版を土台に rebase し、手順 B で作り直す。**
> **検算は「増える方向にしかならないこと」**——
> **減る方向の数字が出たら、どちらかの字を落としている。**
> **`font-subset-coverage.test.ts` が「今の `data/` に出る字が 1 字残らず入っているか」を見るので、
> 作り直したあとに必ず走らせること。**
>
> **作り直すときは `NODE_OPTIONS=--max-old-space-size=8192` を忘れないこと**（手順 B）——
> **rebase の後は衝突を解くことに気を取られて素で走らせやすい。**
> **素で走らせると `exit 137` で途中で消え、エラーメッセージは出ない**（**OOM だと気づけない**）。
>
> **3 つとも衝突する**（**バイナリだけではない**）。
> **2026-09-24 に、島根（#967）と佐賀（#968）のフォント変更だけを持つ枝を
> 共通の親から 2 本合成して `git merge-tree` に掛けた実測:**
>
> ```
> CONFLICT (content): apps/web/public/fonts/fonts.css
> CONFLICT (content): apps/web/public/fonts/shippori-mincho-700.subset.txt
> warning: Cannot merge binary files: ...subset.woff2
> CONFLICT (content): apps/web/public/fonts/shippori-mincho-700.subset.woff2
> ```
>
> **`.txt` は全 1,000 字あまりが 1 行に並ぶ 1 行のファイル**なので、
> **どの県が字を足しても、動くのは同じ 1 行である。**
> **`fonts.css`（2,970 行）も、動くのは明朝700 の `unicode-range` の 1 行だけ**
> （**島根 ↔ 佐賀 の差分は両ファイルとも `1 insertion / 1 deletion`。実測**）。
> **「テキストだから 3-way マージが助けてくれる」は、このファイルには当てはまらない**——
> **3-way マージが解けるのは、2 人が別の行を触ったときだけである。**
> **3 つとも、1 回の再生成で同時に書き換わる。どれか 1 つでも手で解いたらやり直す。**
> **検査は `.txt` の主張ではなく woff2 の cmap を実際に読む**ので、
> **`.txt` だけ正しくしても通らない**（#477/#520。**これは守りであって、不便ではない**）。
...
**明朝700 は、使うページなら必ず 1 面 199.1 KB を引く**（現状は 5〜83 面で 63〜2,157 KB と振れる）。
...
> 明朝700 を 5〜7 字しか使わないのに 199.1 KB を引くことになる。
...
2. `NODE_OPTIONS=--max-old-space-size=8192 PYFTSUBSET=.venv/bin/pyftsubset pnpm --filter web font-subset`
   （`public/fonts/` の woff2・字一覧・`fonts.css` を書き換える）

   > **`NODE_OPTIONS` を付けないと、ページ数によっては OOM で落ちる**（#901 佐賀で実測）。
   > **素のまま走らせると `exit 137`（SIGKILL）で 3,800/5,527 ページあたりで消える。**
   > **エラーメッセージは出ない**——`[ERR_PNPM_RECURSIVE_RUN_FIRST_FAIL] ... Exit status 137` だけである。
   > **「途中で止まった」ようにしか見えないので、OOM だと気づきにくい。**
   > **`data/` が増えるとページ数も増える**ので、**#901 で会期を広げるたびに閾値に近づく**
   > （**5,527 ページは 2026-09-23 の実測。三重を広げた #886 の時点では 1,466 ページだった**）。
