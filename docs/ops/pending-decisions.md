...
## **人間に打ってもらうものは 1 本のスクリプトにまとめる**（2026-09-13）

**「あれとこれを打ってください」と 4 回言うより、1 回で済むほうがよい**（ユーザーの依頼）。

```sh
bash scripts/human-tasks.sh                          # 何をするかだけ出す（dry-run）
bash scripts/human-tasks.sh --yes --host <VPS の IP> # 実行する
```

**入っているのは、対話が要らず・権限だけで止まっている 2 つ**:

| # | 何を | なぜ PO ができないか |
|---|---|---|
| 1 | **`site.conf` を本番に反映**（#610 / #654） | **PO の端末に接続先が定義されていない**（VPS の IP はリポジトリに書けない） |
| 2 | **`git stash` 2 件を drop**（#543） | **`git stash drop` が権限で止められている** |

**入れていないもの**（**人間の判断が要るので、スクリプトにしてはいけない**）:
**PAT の設置**（#550 / #155 / #547。GitHub の設定画面）、
**Sponsors / 広告 / NDL 照会**（#53 / #48 / #250。外部に届く。方針も要る）。
