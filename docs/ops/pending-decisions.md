...
**さらに「sudo パスワード以外の情報を私に用意させないで」**——**IP も鍵もスクリプトが自分で解決する。**
...
**どの鍵を出すかが決まらず Permission denied になる**。`docs/ops/deploy.md`）。
**dry-run のログに IP は出さない**（このログが貼られても漏れないように）。

### **入れていないもの**

| | なぜ入れないか |
|---|---|
| **#543 の退避された作業 2 件の破棄** | **`scripts/ci/forbidden-patterns.sh` が該当する git のサブコマンドを全面的に禁止している**（#542 / #557。2026-09-06 に 3 回、担当者の未コミットの作業が消えた）。**規則を作った側がスクリプトで破るのは筋が通らない。** 手順は下の「3.」にある |
