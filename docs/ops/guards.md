...
| **push される前に rebase された枝が、main の行を黙って消す**（`merge-base` が既に main の先端なので上の検査は `ok` と言う） | `scripts/ci/stale-base.sh --net-deletions`（ファイルが残ったまま base の行が差し引きで減っていないかを見る）。**直近 60 PR で 4 件に火が点き、うち 2 件が実地の事故**（#832 の 17 行・#761 の 12 行）。**証拠ではなく合図なので、意図した削除なら PR 本文に理由を書く**（#836） |
