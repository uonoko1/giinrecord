...
| **一般選挙の境をまたいで、票が今の名簿に黙って寄る**（地方。**再選した本人でも同姓同名の別人でも出力が 1 バイトも違わない**。#569 の重いほう） | `packages/etl/src/local-assemblies.ts` の `sessionRosterCoverageOf` が **11 県すべてが通る 1 か所**で会期ごとに数え、`meta.json` の `sessionRosterCoverage` に出す（#951）。**`seatsChanged: Math.min(Math.max(0, rosterSize - s.seen.size), s.unmatched.size)`**——「名簿に居るのにこの会期に居ない人」と「この会期に居るのに名簿に無い氏名」が**同時にある分だけ**席の持ち主が変わっている、という事実だけを言う。**誰が誰に替わったかは書かない**（推定）。**止めない**——止めると #901 が進まなくなるので、`SEATS_CHANGED_FLAG`（10）以上を `/coverage` で名指しするだけ。**既存の 4 つの守りは全部黙る**（#928 は境の直前なら 1 任期の内側／`unmatched` は寄ってしまうので出ない／`lossyNameMatches` は字が落ちていない／`sourceConflict` は食い違っていない）。テストは `packages/etl/test/local-session-roster-coverage.test.ts` と `apps/web/app/routes/session-roster-coverage-published.test.tsx`。**実測 2026-09-25: 11 議会 119 会期で `seatsChanged` の最大は 5（滋賀）、10 以上は 0 件**（#951）。**ただし「印 0 件」は「境が無い」ではない**——**#990 が 11 県の境を測り直したところ、測れた 8 県のうち 2 県（秋田 9・佐賀 4）の境がこの線の下にある。** **線は止める道具ではなく記録である**（`SEATS_CHANGED_FLAG` の docblock） |
