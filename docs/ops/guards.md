...
| **ETL が落ちたときに「どの県が」「どのセル値で」が分からない**（メッセージが痩せても例外は飛ぶので気づけない） | `packages/etl/test/local-legend-error-message.test.ts` の `assertMentions` が 7 県 × 2 層の 13 か所で `label` と `raw` と凡例が含まれることを見る（#679。直す前は同じ変異 25 件すべてが素通り、直した後は 26 件すべて捕まえる） |
...
| **別の任期の議会の名簿を、その採決に当てる**（地方。`--sessions` を広げて任期をまたぐと起きる） | `packages/etl/src/local-assemblies.ts` の `rosterWindowOf` と `LOCAL_TERM_DAYS`（**地方自治法 93 条 1 項の 4 年 = 1,461 日**。`rosterAsOf` からそれより遠い採決は**間に必ず選挙がある**ので違反にする）。テストは `packages/etl/test/local-roster-window.test.ts` の `1 任期の違反を 1 件も出さない`。**地方の名簿は掲載日しか持たず任期が無い**ので、国会側の在職確認（上の行）に当たる検査は書けない——**「窓がどれだけ開いているか」までしか言えない**。**実測 2026-09-20: 7 議会が `rosterAsOf` より後にはみ出し、最大は鳥取 1,156 日（残り 305 日）**（#928） |
| **一般選挙の境をまたいで、票が今の名簿に黙って寄る**（地方。**再選した本人でも同姓同名の別人でも出力が 1 バイトも違わない**。#569 の重いほう） | `packages/etl/src/local-assemblies.ts` の `sessionRosterCoverageOf` が **11 県すべてが通る 1 か所**で会期ごとに数え、`meta.json` の `sessionRosterCoverage` に出す（#951）。**`seatsChanged: Math.min(Math.max(0, rosterSize - s.seen.size), s.unmatched.size)`**——「名簿に居るのにこの会期に居ない人」と「この会期に居るのに名簿に無い氏名」が**同時にある分だけ**席の持ち主が変わっている、という事実だけを言う。**誰が誰に替わったかは書かない**（推定）。**止めない**——止めると #901 が進まなくなるので、`SEATS_CHANGED_FLAG`（10）以上を `/coverage` で名指しするだけ。**既存の 4 つの守りは全部黙る**（#928 は境の直前なら 1 任期の内側／`unmatched` は寄ってしまうので出ない／`lossyNameMatches` は字が落ちていない／`sourceConflict` は食い違っていない）。テストは `packages/etl/test/local-session-roster-coverage.test.ts` と `apps/web/app/routes/session-roster-coverage-published.test.tsx`。**実測 2026-09-25: 11 議会 119 会期で `seatsChanged` の最大は 5（滋賀）、印が付く会期は 0 件**（#951）。**ただし「印 0 件」は「境が無い」ではない**——**#990 が 11 県の境を測り直したところ、測れた 8 県のうち 2 県（秋田 9・佐賀 4）の境がこの線の下にある。** **線は止める道具ではなく記録である**（`SEATS_CHANGED_FLAG` の docblock） |
