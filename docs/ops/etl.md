...
| 22 回次（第200〜221回） | 約 8,000（回次別 `numberOfRecords` の実測 797,651 件からの外挿） | 約 4 時間 26 分 | **約 6 時間 = `timeout-minutes: 360` を超える** |

**22 回次ぶんは 1 回の dispatch では流せない。** #219 のバックフィルと同じく回次を分ける。
間隔を縮めるのではなく chunk を分けて対処する（1 秒に戻しても 8,000 ページで約 3 時間 46 分で、timeout への余裕が小さい）。
