...
- 全回次の取り直しになるので初回は 77 分前後（上の計測）。`.cache/` が残っていれば短くなる。
...
### 委員会発言（#242）を入れるときの所要時間（#231 の間隔での試算）

`docs/research/individual-records.md:63` の実測ページ数（両院 1 回次で本会議 15 ページ・**委員会込み 700 ページ**）から:

| 範囲 | ページ数 | `NDL_API_INTERVAL_MS` = 2 秒での待ち | 応答 0.7 秒/req を足した実時間 |
|---|---|---|---|
| 第221回だけ | 約 700 | 約 23 分 | 約 32 分 |
| 22 回次（第200〜221回） | 約 8,000 | 約 4 時間 26 分 | **約 6 時間 = `timeout-minutes: 360` を超える** |

**結論: #242 の 22 回次ぶんは 1 回の dispatch で流せない。** 回次を分けるのは #219 のバックフィルと同じやり方でよく（10〜15 回次ずつ）、間隔を縮めるのではなく **chunk を分ける**ことで対処する。参考までに間隔を 1 秒に戻しても 8,000 ページで約 3 時間 46 分（応答込み）で、timeout に対する余裕は小さい。**取得量が桁違いになるのは間隔ではなく委員会を足すことが原因**なので、そこは #242 側の設計で決める。

（ページ数は `numberOfRecords` からの推定で、全量取得は #263 でまだ実測していない。実測が出たらこの表を置き換える。）
