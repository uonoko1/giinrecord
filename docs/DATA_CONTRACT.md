...
### 字が落ちたまま名簿に寄った氏名を画面に出す（Issue #800。**`unmatched.json` とは別物**）

**何が問題だったか**: #778（PR #794）で `meta.lossyNameMatches` を 11 県すべての `meta.json` に入れたが、**誰にも見えていなかった**。`grep -rn "lossyNameMatches" apps/web` は **0 件**、`OPS_DATA_FILES`（`apps/web/app/lib/smoke.ts`）に地方の `meta.json` は入っておらず、`data/assemblies/{id}/meta.json` は **`/data/` で配信されず、`import.meta.glob` のバンドルにも入らない**。**`git clone` した人しか読めなかった。** 「記録した」は「見えるようにした」ではない。

**`unmatched.json` と `lossyNameMatches` は別物である**（**同じ場所にまとめない**）:

| | `unmatched.json` | `meta.lossyNameMatches` |
|---|---|---|
| 何が起きたか | **突き合わなかった** | **字が落ちたまま突き合わせた** |
| 票はどこに付くか | **誰にも付かない** | **本人に付く**（寄せ方は変えない。#569） |
| 利用者から見えるか | **見える**（その票が議員ページに出てこない） | **見えない**（普通の表決として出る） |
| 重さ | 「記録が出ない」 | **「別人の記録が出る」に近い**（#569。**後者は利用者から検出できない**） |

**だから出す先も違う。** `unmatched` は「出てこない票がある」という**収録範囲の話**なので `/coverage` だけでよい。`lossyNameMatches` は**特定の議員のページに票が出ている**話なので、**その議員のページにも書く**。

**決めたこと（両方に出す）**:

| 誰向け | どこ | 何を出すか |
|---|---|---|
| **利用者** | 議員ページ `/members/{id}` の冒頭（`LossyNameNotice`） | 表決結果に**印字されていた氏名の原文**・**議員名簿の氏名の原文**・その氏名で突き合わせた表決の**件数**・**このページの表決の総数（母数）**・**一次資料へのリンク**。該当しない議員には**何も出さない** |
| **運用・読者** | `/coverage` の「字が落ちたまま名簿に突き合わせた氏名」節（`LossyNameSection`） | 議会ごとの**人数・件数・その議会の表決の総数（母数）**、**印字されていた氏名と名簿の氏名の対**、議員ページへのリンク、**一次資料へのリンク**。**該当 0 件の議会は行を出さず、合計を「0 名・0 件」と母数つきで書く** |

**`OPS_DATA_FILES` に地方の `meta.json` を足す案は採らなかった。** 11 県ぶんのファイルが `/data/` に増えるうえ、**読むのが運用者だけなら「見えるようにした」ことにならない**——#800 で起きていたのは「利用者に見えない」ことである。**ビルド時に Node で読んで画面に焼き込む**（`readLocalAssemblyMetas`）方が、配信ファイルを増やさず利用者にも届く。

**書くのは事実だけ**（絶対原則）:

- **印字されていた氏名・名簿の氏名は原文のまま出す。** 落ちた字が何だったかを**推定しない**（`西川` から `均` を復元して見せない）。
- **寄せ方が正しいかどうかを書かない。** 「たぶん正しい」も「誤りかもしれない」も**推測であり評価である**。我々にも確かめる手立てが無い、という事実がこの節の理由そのものである。
- **一次資料へのリンクを必ず添える**（`meta.sources`。表決結果の PDF と議員名簿）。**利用者が自分で確かめられなければ、書いた意味が無い。**
- **母数を必ず添える**（#757）。「0 件」と「1 件も読めていない」を同じ出力にしない——**`meta.json` を 1 つも読めていないとき（`readLocalAssemblyMetas` が null）は節ごと出さない**。

**票は 1 件も動かさない**（#800 の「やらないこと」）。**判定も変えない**——どれが lossy かを決めるのは ETL（`packages/etl/src/local-assemblies.ts` の `lossyNameMatchesOf`、判定は `sources/local/name-match.ts` の `isLossyName`）で、**鳥取で 35 人全員が鳴るのを避けるために形を選んである**（#778）。`apps/web/app/lib/lossy-name.ts` は `meta.json` が既に書いた行を引くだけである。

**本番データで検査する**（`apps/web/app/routes/lossy-name-published.test.tsx`）。fixture だけだと、**ETL が書いた欄と画面のつなぎが切れても緑のままになる**——それが #800 そのものだった。2026-09-13 時点の本番は **11 県中 1 県（奈良 `pref-29`）に 2 名・162 件**、残り 10 県は 0 件。

### 記号の数と公表された賛成者数・反対者数の突き合わせ（Issue #826。**食い違いで止めない**）

賛否 PDF には、議員ごとの記号の列とは**別の欄**として、行ごとの**賛成者数・反対者数**がある。同じ一次資料の別の場所なので、**数が合うかを突き合わせれば、記号の取りこぼし・二重取りが検出できる。** 5 県（宮城・鳥取・島根・佐賀・高知）がこれを取得の検算に使ってきた。

**何が問題だったか**: **5 県とも `assert.equal` を行ごとに撃つ形で、「議長の行を除く」条件は 1 つも無かった**（#825 の実測、PO が佐賀のコードで確認）。ところが**食い違いには「取りこぼし」以外の原因がある**:

| | 山梨県議会（#782 / #811） | 佐賀県議会（#825 の実測） |
|---|---|---|
| 事象 | 知事の再議・三分の二未満で否決 | **同じ事象の行が実在する** |
| 公表の賛成者数に議長の記号を | **入れていない**（`row9` 18 / 数えた `〇` 19、`row6` 17 / 18） | **入れている**（505 行中 1 行。数は合う） |

**つまり「規則」ではなく「議会ごとの慣行」で、同じ事象でも数え方が違う。** **5 県のどれかに山梨と同じ行が出れば、そのテストは落ちる。** 落ちれば**月次 ETL が通らず、正しい記録まで出なくなる**（#569 の「記録が出ない」側）。**落ちた人が「テストを緩める」誘惑に駆られるのが一番悪い。**

**決めたこと（#811 の PO の判断）**: **食い違いを例外で止めず、`unknownCells` / `unmatched.json` と同じ「件数」として出す。** **記録は出たうえで、人が見に行ける。**

| どこ | 何を出すか |
|---|---|
| `meta.countChecked` | **母数**（#757）。`rows`（採決の総数）・`checked`（突き合わせた数）・`noCounts`（`counts` の欄が PDF に無い）・`unreadableCells`（凡例の引けないセルがある）。**食い違いが 0 でも必ず出す** |
| `meta.countMismatches` | 食い違った採決の `rollCallId`・**数えた数**・**公表された数**。食い違いが無ければ省略 |
| `/coverage` の「記号の数と公表された賛成者数・反対者数の突き合わせ」節 | 母数・食い違いの件数・未突合の内訳、議会ごとの表（採決へのリンク付き） |

**共通層（`buildLocalAssembly` の `countMismatchesOf`）に置く**（#800 / #778 と同じ理由）。判定に要るのは `rollCalls[].counts`（公表値）と `rollCalls[].votes[].value.mapped`（凡例から引いた意味）だけで、**どちらも 11 県すべてがそこを通る。** **`input.countMismatches` は受け取らない**——受け取ると「県が渡さなければ出ない」に戻る。

**生の字では数えない。** 本番の実測で、**賛成は `○`（10 県）と `〇`（徳島）、反対は `×`（8 県）と `●`（島根・徳島）**に分かれている。**島根の `●` を `×` で数えれば、島根の全 112 件が「反対 0」になり、偽の食い違いが 112 件出る。**

**母数から外すもの（外した数も出す。#757）**:

- **`counts` の欄が無い行**（本番 **341 件**。奈良 125・徳島 105・高知 104・秋田 7）——**突き合わせる相手が公表されていない。**
- **凡例の引けないセルがある行**（本番 **5 件**。滋賀 4・鳥取 1）——`mapped` が無いセルは賛成とも反対とも数えられない。**滋賀の 4 行は生の字では公表値と合っている**（`legend` が `抽出不能` なだけ）ので、**母数に入れると「票が食い違った」という偽の件数になる**（食い違っているのは凡例の読みであって、票ではない）。

**議長の行は検算から外さない**（#811 で PO が退けた案）。`議` は `mapped: "投票なし"` なので賛成にも反対にも数えず、**自然に合う。** **議長が投票した行は一番壊れやすいので、そこを検算の外に出さない。**

**書くのは事実だけ**（絶対原則）: **数えた数と公表された数の両方**と**母数**と**一次資料**。**どちらが正しいかは書かない**——**我々には確かめる手立てが無い、という事実がこの節の理由そのものである。** **`counts` は数え直さない**（`rollcalls/{id}.json` の `counts` は公表値のまま）。

**検算は緩めていない。** 県ごとのテスト（佐賀を #826 で最初に直した）は**食い違いが 1 行でもあれば落ちる。** 変わったのは**落ちたときに食い違った行が全部見える**ことで、行ごとの `assert.equal` は**最初の 1 行で止まっていた**（**実測: 72 行のうち 3 行を食い違わせても、落ちたメッセージに出たのは 1 行だけだった**）。**`1 行だけ数え方が違う` と `表の復元が壊れて 26 行ずれた` は、前者が正しい記録・後者が偽の記録なのに、「テストが赤い」では同じに見える。** **残る 4 県（宮城・鳥取・島根・高知）は別 PBI。**

**2026-09-14 時点の本番**: 採決 **1,369 件**のうち母数 **1,023 件**、**食い違い 0 件**。

### 一般選挙の境をまたいだ寄せに痕跡を残す（`meta.sessionRosterCoverage`、Issue #951）

**何が問題だったか**: **地方の名簿は、どの県も「今の名簿」1 枚しか公表していない**（`LocalMemberTerm` は `asOf` しか持たず、任期の開始日も終了日も持たない）。**`--sessions` を広げて一般選挙の境をまたぐと、その頃の議員の氏名が今の名簿に当たりうる。** **#950 が奈良で実測した**——**境の向こうの 4 本 72 行 / 2,952 票のうち 1,656 票（56.1%）が 2026 年の名簿に寄る**（23 人ぶん）。

**「再選した本人」でも「同姓同名の別人」でも、出力が 1 バイトも違わない。** **これは #569 の「別の記録が出る」側でいちばん重い形である**（利用者から検出できない）。

**既存の守りは 4 つとも黙る**（#951 で実測した）:

| 守り | 何を見るか | この形を捕まえるか |
|---|---|---|
| **#928**（`rosterWindowOf` / `LOCAL_TERM_DAYS`） | 名簿と採決の日付が 1 任期（1,461 日）を超えて離れているか | **捕まえない**（**奈良の境は 1,290 日で窓の内側**） |
| **`unmatched.json`** | 名簿に寄らなかった氏名 | **捕まえない**（寄ってしまうので出ない） |
| **`meta.lossyNameMatches`** | 字が落ちたまま寄せた氏名 | **捕まえない**（字は落ちていない） |
| **`sourceConflict`** | 一次資料どうしが氏名で食い違う | **捕まえない**（食い違っていない） |

**決めたこと（#951 の PO の判断）**: **止めない。印を付けて数えるだけ。** **止めると #901（会期を広げる）が進まなくなる**ので、まず `data/` に痕跡を残して後から数えられるようにする。

| どこ | 何を出すか |
|---|---|
| `meta.sessionRosterCoverage` | **会期ごと**に `sessionId` / `date` / **母数**（`rollcalls` / `votes`）/ `rosterSeen` / `rosterAbsent` / `unmatchedNames` / `unmatchedVotes` / `seatsChanged`。**会期が 1 つでも、入れ替わりが 0 でも必ず出す**（#757。`countChecked` と同じ扱い） |
| `/coverage` の「古い会期に今の議員名簿を当てていないか」節 | 母数（会期数）・いちばん大きい入れ替わり・印の付いた会期の表 |

**`seatsChanged` は `min(rosterAbsent, unmatchedNames)`**——**「名簿に居るのにこの会期に居ない人」と「この会期に居るのに名簿に無い氏名」が同時にある分だけ、席の持ち主が変わっている。** **片方だけでは証拠にならない**: `rosterAbsent` だけなら PDF の列がその会期だけ少ないのかもしれず、`unmatchedNames` だけなら字が壊れているだけかもしれない（#680 の `□`）。**どちらも既存の守りが見ている。**

**なぜ会期ごとか**: **議会全体で数えると混ざって消える。** 奈良が境をまたいだとき、**新しい会期は名簿と完全に一致し、古い会期だけが半分ずれる**ので、ひとまとめに数えると「40 人中 40 人が居る」になる。**会期は県の会期ページ（一次資料）が決めた単位で、我々が切ったものではない。**

**書くのは事実だけ**（絶対原則）: **数だけ。** **誰と誰が入れ替わったかは書かない**（推定であり #569 が禁じる側）。**「選挙があった」とも書かない**——**選挙かまとまった辞職かは、表決 PDF と名簿からは区別できない。** **選挙の日付を外から持ち込まない。**

**`SEATS_CHANGED_FLAG`（10）は「ここから先は選挙だ」という線ではない**——**今までに実際に起きた幅の外、という線だけ。** **実測 2026-09-21（本番 `data/` の 11 議会 / 67 会期 / 3,036 採決 / 135,686 票）: `seatsChanged` の最大は 4（三重 r05-2 / r06。母数 47 人）で、10 以上は 0 件。** **線を実測の内側に引けば今すぐ赤くなり、直せない赤はやがて誰も見なくなる**（#785）。

**境をまたいだ形は 2 県で測った**（**1 県だけで線を引いていない**）:

| 県 | 境 | 入れ替わり | **`seatsChanged`** | **#928**（1,461 日） | **名簿に黙って寄る票** |
|---|---|---|---:|---|---:|
| **奈良**（#950 が測った境） | 2022-10-12 | 41 名中 17 名 | **17** | **鳴らない**（1,290 日） | **56.1%** |
| **宮城**（#953 が測った境） | 2023-07-12 | 58 名中 18 名 | **16** | **鳴らない**（1,163 日） | **69.0%** |

**どちらも `--sessions` を 1 つ広げれば届く位置にある。** **#950 も #953 も、担当者が氏名の集合の不連続を見て手前で止めた**——**機械が止めたのではない。** **この欄はその「止めた根拠」を `data/` に残すためのものである。**

**共通層（`buildLocalAssembly` の `sessionRosterCoverageOf`）に置く**（#800 / #778 / #826 と同じ理由）。要る材料は `rollCalls[].sessionId` / `.date` / `.votes[].memberId` / `.nameText` と `members` だけで、**11 県すべてがそこを通る。** **`input.sessionRosterCoverage` は受け取らない**——受け取ると「県が渡さなければ出ない」に戻る。
...
- **滋賀県議会の取得先**（Issue #741。すべて `https://www.shigaken-gikai.jp`。assembly id `pref-25`、`pnpm etl:local shiga`）: **このサイトは Shift_JIS**（`PoliteFetcher.textShiftJis`）。名簿は `/g07_giinlistP.asp`（五十音順、1 ページ。議員 1 人ぶんが 1 セルで、氏名（`g07_giinlistS.asp?SrchID=197` へのリンク）・ふりがな・期数・所属会派・選挙区が `<br>` で並ぶ）。議員 id は `p_25_{SrchID}`（例 `p_25_197`）。**名簿ページに掲載日が 1 つも無い**ので `asOf` / `meta.rosterAsOf` は**取得日（JST）**（徳島と同じ扱い）。会期は 年の一覧 `/voices/g07_Congress.asp?YMSel=9999` → 年ページ → 会期の賛否ページ `/g07_gian_sanpi.asp?KaigiID=256`（**KaigiID は年ページから拾う。連番に見えて飛ぶので組み立てない**）→ 賛否 PDF（`/voices/GikaiDoc/attach/Congress/*.pdf`。**1 会期に複数本**）。**年ページは 2 通りあり、両方読む**——`Tmode` 無しは**年度**（4月〜翌3月）、`Tmode=0` は**暦年**で、片方だけでは会期が落ちる（実測: 2026 年度版にしか 7月定例会議が無い）。`sessionId` は `{西暦}-{月2桁}`（臨時会議・招集会議は `-rinji`）、`sessionLabel` は年ページの h2 の原文（「令和8年 7月定例会議」）。**PDF に議案番号の欄が無い**（議案等番号の欄が件名を兼ね、「議第105号から議第109号まで」のように範囲で書かれる）ので `number` は空にし、id は件名から作る（同じ議決日で同じ件名なら `-1`, `-2`）。`kind` は `議案等`（議案種別の欄が無いので推定しない）。**年は PDF に無い**ので会期の年から補い、会期の月より 6 か月以上前の月は翌年にする。**表決方法の欄は無い**ので `method` は書かない。
  - **記号帯の読み方**（#689 / #694 / #705 / #718 の実測に従う）: 行の中の**末尾が記号で終わるアイテムを全部足し合わせ**、その総数が議員の列の数と一致するときだけ置く（合わなければ全セル `不明`）。**記号 10 個以上のアイテムだけを行と見てはいけない**——8 本 32 行で記号帯が 2 アイテムに割れ、割れ目に `議`（議長）が 1 文字だけ立つ（落とすと**議長の記録が丸ごと消え、合計の辻褄は合うので気づけない**）。記号 k の x は、**1 文字だけのアイテムは実 `cx`**（推定しない）、**複数記号のアイテムはその右端からセル幅ずつ左へ**（左端 + 等分は、議決結果が混ざる行で破綻する）。**2 ページ目の氏名帯は「空の列だけ」1 ページ目から補う**（「氏名帯が空のときだけ」では直らない）。
  - **凡例は PDF ごとに読む**（2 種類ある。`「○」は賛成を、「×」は反対を、「議」は議長（表決権なし）、「－」は表決に参加していないことを表す。` と `「-」欠席を、「議」は議長（表決権なし）、「退」は退席を表す。`）。**同じ `-` の意味が会期で違う**ので、`mapped` は**記号ではなく凡例の意味**で引く。**凡例の文が 1 つも無い PDF がある**（実測 147 本中 19 本）ので、そのときは `legend` を `抽出不能` にして `mapped` を付けない（**例外にしない**——例外にするとその本の読めた票まで全部消える）。
  - **読めない一次資料**: **文字層の無い画像 PDF が 3 本ある**（`Kg265_250424`・`Kg274_250628`・`Kg337_sanpi-261127`）。**読めないのが正しい結果**で、会期ごと落とさずに `meta.unreadableSources`（URL と理由）に記録する。**書かないと「その日の採決は無かった」と読めてしまう。**
  - **氏名の字が壊れる**: `辻正隆` の `辻` が 2 本の PDF で `□`（U+25A1）に化ける（同じ会期の別の本では化けない）。**`辻` に戻さない**（推定であり別人の記録を作る側。#569／#674）。`unmatched.json` に `reason: "brokenGlyph"` として落ちる。
...
- 発言の**突合先の名簿は両院ぶん**である（#313）。会議録は会議の院（`nameOfHouse`）で分かれるが、**発言者はその院の議員とは限らない**: 参議院の会議には衆院議員が大臣・副大臣として答弁に立ち、連合審査会にも出る（逆も同じ）。院ごとに名簿を分けて突合していたため、参議院の会議録に出た衆院議員の発言が丸ごと落ちていた（`data/unmatched.json` の 692 行はすべて参議院の会議録で、うち第221回の 391 行・37 名分がこれに当たる）。**名簿を足しても在職の確認は緩まない**: 衆院名簿が覆わない回次（第217・219回の 301 行）は `tenureVerified` が候補を落として `unmatched` に残り、両院に同姓同名がいれば会派で 1 人に絞れないかぎり紐づけない（#230 と同じ厳しさ）。
- 発言の**取得範囲**は院で違う（突合先の名簿とは別の話）。参院は取得回次（`targets`）を取得する。衆院（#73 / #242）は衆院名簿が「現在」の1回次分しか無いので、議案の名寄せと同じく名簿が覆う回次（`meta.sessions` の最大）だけ取得・突合し、過去回次の衆院の発言は取得しない（名簿に無い旧議員を同名の現職に紐づけない。#71 で回次ごとの名簿が入れば広がる）。**これはサイズの都合ではなく原則**なので、委員会を足しても変わらない。その回次が引き継ぎ（carried）のとき（過去回次だけの手動実行・バックフィルの chunk）でも**取得する**（#236）。取得をやめると衆院の発言が前回出力の引き継ぎ頼みになり、引き継ぎが1度でも欠ければ（#103 以前の `session` の無い行、名簿から消えた memberId）0 のまま自力では戻らないため。引き継ぎとの二重行は「取得した `speechId` の引き継ぎ行を落とす」ことで防ぐ（`dropCarriedSpeeches`。残すのは今回の名簿で名寄せし直した取得分。取得が空なら何も落とさない）。
...
## 一次資料の取得と再公開（#537、2026-09-13 決定）

**`docs/research/local-assemblies.md`（#128）は「ETL 化する議会には事前に事務局へ問い合わせる」と定めていたが、
既存 7 県はいずれも問い合わせずに実装され、本番で公開されている**（#537 が起票した事実）。

**2026-09-13、人間の判断で「案 3: データの性質で切り分ける」を採った。**
**照会はしない。** 以下がその整理である。
...
| | 公開しているか | 法的な扱い |
|---|---|---|
| **採決の賛否**（「議員 A が議案 B に賛成した」） | **している** | **著作物ではない**（下記） |
| **議案名・会期名・議員名** | している | 事実の表示。創作的表現ではない |
| **PDF のレイアウト・組版** | **していない** | 著作物でありうる。**再現しない** |
| **議事録の文章表現** | **していない** | 著作物でありうる。**引用の範囲を超えて載せない** |

### なぜ照会が要らないか

1. **採決の賛否は著作物ではない。**
   著作権法 2 条 1 項 1 号は著作物を「**思想又は感情を創作的に表現したもの**」と定義する。
   **「議員 A が議案 B に賛成した」は事実であって創作的表現ではない。** 著作権はそもそも及ばない。

2. **公開しているのは抽出した事実だけで、複製にあたらない。**
   **PDF の表現（レイアウト・組版・文章）を再現していない。** 事実を取り出して別の形で並べている。

3. **取得の過程も著作権法 30 条の 4 で適法。**
   同条は「**著作物に表現された思想又は感情の享受を目的としない利用**」（情報解析を含む）を
   許諾なく認める（2018 年改正）。**ETL は表現を享受せず、記載された事実を取り出すだけである。**

4. **実測でも禁止されていない**（#128 / #221 / #529 / #616 / #710 で確認）:
   - **robots.txt: 候補ホスト 18 件を取得し、賛否ページを `Disallow` しているものは 0 件**
   - **利用規約: 取得した 4 サイトのトップに、機械取得や複製を禁じる文言は無い**
   - **`Disallow` があるホストには取りに行っていない**（愛知の会議録システムは `Disallow: /` なので 1 行も読んでいない。#688 / #710）

**調査文書の「これは『禁止されていない』であって『許可されている』ではない」という一文は、
法的判断ではなく「安全側に倒す」ために置かれたものだった**（書いた時点で上の整理をしていない）。
**整理した結果、照会は法的には不要である。**

### それでも守ること（**この整理は取得の作法を緩めない**）

- **`robots.txt` を先に読み、従う。** `Disallow` の下には取りに行かない。
  **照合は RFC 9309 §2.2.2 に従う**（`polite-fetch.ts` の `isAllowedByRobots`）——
  **`*` は 0 文字以上の任意の並び、末尾の `$` は行末への固定、それ以外はリテラル。
  照合の対象はパスとクエリ。** **`Allow` は見ない**（＝保守的に「拒否」側へ倒す）。
  **#894 まで、途中に `*` がある `Disallow`（`/koujisoutatu*.pdf` 等、11 県中 13 件）を素通りさせていた。**
  **「取ってはいけない」と書かれた URL を「取ってよい」と判定する、危険側の誤りだった。**
  **確かめ方が間違っていれば、確かめたことにならない。**
- **1 秒以上の間隔を空け、UA を名乗り、同時並列で叩かない**（`polite-fetch.ts`）。
  **相手が WAF で空を返してきたら、それは「負荷をかけている」という合図である**（#710 の愛知。2 秒で当たった）。
- **一次資料リンクを全行に付ける**（このプロジェクトの原則）。**利用者が原典に到達できること。**
- **PDF の表現を再現しない。** 記号・氏名・議案名という事実だけを取り出す。
- **賛否以外に手を広げるときは、この節に戻って評価し直す**——
  **議事録の本文をそのまま載せる**ような使い方は、上の整理の範囲外である。

### 新しい議会を足すときのチェックリスト

- [ ] **`robots.txt` を取得して読んだ**（URL・取得時刻・内容を調査文書に書く）
- [ ] **賛否ページが `Disallow` の下でないことを確かめた**
- [ ] **利用規約のページを開いて、機械取得を禁じる文言が無いことを確かめた**（原文を引用する）
- [ ] **取得は 1 秒以上の間隔・UA 明記・直列**（`polite-fetch.ts` を使う）
- [ ] **出典 URL を `meta.json` の `sources` に全件記録した**
- [ ] **PDF の表現ではなく、抽出した事実だけを `data/` に書いている**
- [ ] **HTML をフィクスチャに保存するなら、第三者サービスのトークンが混ざっていないか確かめた**
      （**青森で実際に踏んだ。2026-09-13、PR #761**）

**HTML フィクスチャの注意**（#750 で `gitleaks` が発火した）:

**自治体のページには、第三者サービスへの公開リンクが `?token=…` 付きで入っていることがある。**
**青森県議会のページには電話リレーサービスのリンクがあり、`gitleaks` が `generic-api-key` として検出した。**

**秘密ではないが、消すこと。** 理由は 2 つ:

1. **うちが管理できないものを、うちの履歴に残さない**（第三者のトークンである）
2. **`.gitleaks.toml` に除外を書いて緑にするのは逆**——**除外を増やすと、次に本物が入ったときに通る確率が上がる**

**フィクスチャは「ETL が読む部分が正しく取れるか」を見るためのもので、ページ全体をそのまま持つ必要は無い。**
**消したら「テストが弱くなっていないか」を確かめること**（同じ関数が同じ結果を返すか）。

**既存 8 県のフィクスチャには 1 件も無い**（2026-09-13 に PO が
`grep -rlE "(token|apikey|api_key)=[A-Za-z0-9_-]{12,}" packages/etl/test/fixtures/` で確認）。
**青森が初めてである。** **今後 HTML を足すたびに起きうる。**

**この節を根拠にするので、チェックリストを満たせない議会は実装しない。**
