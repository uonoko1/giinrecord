...
**`SEATS_CHANGED_FLAG`（10）は「ここから先は選挙だ」という線ではない**——**今までに実際に起きた幅の外、という線だけ。** **実測 2026-09-25（本番 `data/` の 11 議会 / 119 会期 / 4,599 採決 / 198,221 票）: `seatsChanged` の最大は 5（滋賀 2023-05臨時 〜 2024-09 の 8 会期。母数 42 人）で、10 以上は 0 件。** **線を実測の内側に引けば今すぐ赤くなり、直せない赤はやがて誰も見なくなる**（#785）。 **⚠ この最大は #901 の進み方で動く**——**2026-09-21 の時点では 79 会期 / 最大 4（三重）だった。** **数は `packages/etl/test/local-session-roster-coverage.test.ts` が本番 `data/` から数え直しており、ここの数字がそれとずれると `packages/etl/test/seats-changed-line-numbers.test.ts` が落ちる**（#1007）。

**境をまたいだ形は 3 県で測った**（**1 県だけで線を引いていない**。#961）:
...
| **青森**（#959 が測った境） | 2023-03-08 | 44 名中 11 名が名簿に無い | **11** | **鳴らない**（1,174 日） | **75.0%** |

**3 県とも `--sessions` を 1 つ広げれば届く位置にある。** **#950 / #953 / #959 の担当者が、いずれも氏名の集合の不連続を見て手前で止めた**——**機械が止めたのではない。** **この欄はその「止めた根拠」を `data/` に残すためのものである。**

**青森は #928 との関係を露わにした**（#959 の要点。#961 の出どころ）: **一般選挙の境は 14 ↔ 15 会期だが、`LOCAL_TERM_DAYS` が初めて超えるのは 19 会期目**——**境より 4 会期も後ろである。** **`--sessions 15`（境の向こう）ですら 1,174 / 1,461 日で #928 は鳴らない。** **この 4 会期ぶんの隙間が、この欄の要る理由そのものである。**

**線（10）と実測の距離は上下で違う**: **上側は今の 11 県の最大 5 との差が 5。** **下側は「余裕」ではなく取りこぼしである**——**#990 が 11 県すべての境を測り直した結果、いちばん小さい境は青森 11 ではなく佐賀 4（線の下 6）で、線の下にあるのは 秋田 9・佐賀 4 の 2 県である。** **線 10 のままでも、測れた 8 県のうち 2 県の境を既に取りこぼしている**（母数 11 県 / 測れた境 8 / 測れない 3）。

**⚠ 「いちばん小さい境は青森 11」は 2026-09-23（`3da4b9d3` / #954）から 2026-09-25 まで此処に書かれていた誤りである**（#1007 で訂正）。 **青森の 11 は #951 が `rosterAbsent` 13 / `unmatchedNames` 11 と仮置きして出した値で、#990 が本番 `data/` の最古の会期から組み立て直すと 16 になる**（`rosterAbsent` 18 / `unmatchedNames` 16）。 **11 県の境の表と、その数の出どころは `packages/etl/src/local-assemblies.ts` の `SEATS_CHANGED_FLAG` の docblock と `packages/etl/test/local-seats-changed-boundaries.test.ts` にある**（**此処に書き写さない**——#1007 が直したのは「同じ数字が 2 か所にあって片方だけ直った」ことだからである）。
