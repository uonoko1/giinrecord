...
- **【既知の未解決】在職確認のない氏名一致による紐づけ（#230）**: 第142〜199回の票のうち、**現行名簿と氏名が一致する少数は `m_` に紐づく**（第142回では 252 票中 3 票。中曽根 弘文・橋本 聖子・山崎 正昭）。名簿には `termEnd` はあるが**在職開始日にあたるフィールドが無い**ため、その議員がその回次に在職していたことを一次資料から確認できない。つまり**これは推定を含む紐づけ**であって、確認された事実ではない。`resolveMember` が「正規化氏名の候補が1人なら、その回次を名簿が覆っているかを見ずに採用する」ために起きる。同じことが第215回以前で広く起きており（第200〜215回で約 18,401 票）、上の「第215回以前は…手元の名簿（第216回以降）で突合する」がそれを許容している。一律の解消は**影響（公開中データの紐づけが大量に外れ、`lostVoteMatches` が ETL を止める）を測ってから #230 で独立に行う**。それまでこの契約は現状を追認する（隠さず、推定を含むことを明記したうえで）。氏名だけから Member を作らない原則は変わらない。
