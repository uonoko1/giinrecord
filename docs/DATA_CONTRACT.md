...
**`SEATS_CHANGED_FLAG`（10）は「ここから先は選挙だ」という線ではない**——**今までに実際に起きた幅の外、という線だけ。** **実測 2026-09-21（本番 `data/` の 11 議会 / 79 会期 / 3,534 採決 / 159,590 票）: `seatsChanged` の最大は 4（三重 r05-2 / r06。母数 47 人）で、10 以上は 0 件。** **線を実測の内側に引けば今すぐ赤くなり、直せない赤はやがて誰も見なくなる**（#785）。
...
**線（10）と実測の距離は上下で違う**: **上側は今の 11 県の最大 4 との差が 6、下側はいちばん小さい境（青森 11）との差が 1。** **線を 12 以上にすると青森を取りこぼす。**
