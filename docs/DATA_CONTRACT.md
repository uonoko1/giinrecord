...
**`SEATS_CHANGED_FLAG`（10）は「ここから先は選挙だ」という線ではない**——**今までに実際に起きた幅の外、という線だけ。** **実測 2026-09-25（本番 `data/` の 11 議会 / 119 会期 / 4,599 採決 / 198,221 票）: `seatsChanged` の最大は 5（滋賀 2023-05臨時 〜 2024-09 の 8 会期。母数 42 人）で、10 以上は 0 件。** **線を実測の内側に引けば今すぐ赤くなり、直せない赤はやがて誰も見なくなる**（#785）。 **⚠ この最大は #901 の進み方で動く**——**2026-09-21 の時点では 79 会期 / 最大 4（三重）だった。** **数は `packages/etl/test/local-session-roster-coverage.test.ts` が本番 `data/` から数え直しており、ここの数字がずれると `docs-numbers.test.ts` が落ちる。**
