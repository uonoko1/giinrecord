...
**線（10）と実測の距離は上下で違う**: **上側は今の 11 県の最大 5 との差が 5。** **下側は「余裕」ではなく取りこぼしである**——**#990 が 11 県すべての境を測り直した結果、いちばん小さい境は青森 11 ではなく佐賀 4（線の下 6）で、秋田 9（線の下 1）もこの線の下にある。** **線 10 のままでも、測れた 8 県のうち 2 県の境を既に取りこぼしている**（母数 11 県 / 測れた境 8 / 測れない 3）。
